"""적합성 필터 — LLM 진입 최전선 (방법서 §12-1).

2단 게이트로 비용을 방어한다:
  1차 규칙(무료): 관계 키워드 無 or 시드 미언급 → 즉시 컷
  2차 제로샷 LLM(gpt-4o-mini): {"is_relevant": bool}만 판정

실측(2026-07-27): RSS 578건 중 1차 통과 68건(11.8%). 1차만으론 부동산·거시경제
기사가 섞이므로 2차 라우터가 필요하다.
"""
...
def rule_filter(title: str, body: str, *, track: str = "relation") -> RuleResult:
    ...
    # ★키워드 요구를 없애는 안을 실험했다가 **되돌렸다** (2026-07-31).
    #
    #   동기: 리스크는 어휘가 열려 있어 목록으로 못 잡는다.
    #     「SK하이닉스 화수분 노릇도 불안」 「신뢰 무너질라 위기감」이 탈락하고 있었다.
    #   그리고 이 필터는 시드 언급분의 87%를 이미 통과시켜 거의 무력해 보였다.
    #
    #   실험 결과: 시드 언급만으로 통과시키면 436건이 새로 들어오는데
    #     1순위(시드2개↑) 33 · 2순위 5 · 3순위 3 · 기타 395
    #   1순위로 올라간 33건이 **주가 시황·증권사 추천주·인물 소개**였다:
    #     "삼성전자·SK하이닉스 주가 신바람… S7 테마 훈풍 무슨일?"
    #     "[SK證 주간추천주] OCI·이노션·피에스케이홀딩스"
    #   얻은 리스크 기사 3건보다 오염된 1순위 33건의 손해가 크다.
    #   밀도가 낮을수록 1순위 오염이 치명적이므로 필터를 유지한다.
    #
    #   놓치는 문제는 **키워드 보강**으로 푼다(아래 RISK_SIGNAL_KEYWORDS).
    if track == "risk":
...
# ★리스크 트랙 — 관계 트랙과 판정 기준이 **정반대인 지점**이 있다.
# 관계 트랙은 "특정 기업만 다루고 상대가 없는 기사"를 컷하는데, 화재·파업·과징금은
# 바로 그 형태다. 실제로 「삼성전자 총파업에 전 세계 긴장」 같은 기사가 관계 트랙에서
# 탈락하고 있었다. 리스크 추론에는 이런 단일 기업 사건이야말로 핵심이다.
_SYSTEM_RISK = (
