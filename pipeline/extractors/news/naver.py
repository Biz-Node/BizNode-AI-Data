...
⚠️ 약관 7.3③ 준수: API 응답(description 등)을 **DB에 적재하지 않는다.**
...
# ★검색용 키워드 — 실측 수율로 확정 (2026-07-27, 21개 후보 × 3개사 × LLM 판정)
#
# 1차 규칙 필터의 85개와 역할이 다르다. 규칙 필터는 "받은 것을 거르는" 무료 작업이라
# 넓을수록 좋지만, 검색은 호출당 비용이 있어 **수율 높은 것만** 남긴다.
#
# 실측 수율(= LLM이 실제 관계로 판정한 비율):
#   공급계약 90.5% · MOU 76.0% · 계약체결 50.0% · 납품/합작/공급 40.0%
#   합병/수주 36.7% · 제휴 30.8% · 매각 25.0% · 소송 20.0% · 인수 16.7%
#   ✗ 제외: 제재 9.1% · 지분 6.7% · 협력 6.7% · 협약 4.2% · 경쟁 3.3% · 리콜 0%
#
# 교훈: **구체적 행위 명사**("공급계약")는 높고, **포괄적 수식어**("협력"·"경쟁")는
# 시황·칼럼에 흔히 쓰여 노이즈가 된다. 흔한 단어는 더 구체적인 복합어로 바꾼다.
# 복합어 대체 검증(2차 실측): 특허침해 65.2%(←특허 10.0%) · 지분인수 30.0%(←지분 6.7%)
#   · 지분매각 25.0% / ✗ 경쟁사 4.3%(←경쟁 3.3%, 개선 없음) · 손해배상 11.8%(←소송 20%)
SEARCH_KEYWORDS = (
    # 거래·공급 (최고 수율대: 40~90%)
    "공급계약", "계약체결", "납품", "공급", "수주",
    # 협력 (구체 형태만: 31~76%)
...
# ★리스크·사건 검색 키워드 — 관계(SEARCH_KEYWORDS)와 **목적이 다르다.**
#
# 위 15개는 "두 기업 사이의 관계"를 찾는 키워드다. 그래서 제재(9.1%)·리콜(0%)처럼
# 관계 수율이 낮은 것을 뺐다. 그런데 화재·파업·과징금은 **관계가 아니라 사건**이라
# 관계 수율로 재면 당연히 낮다 — 잣대가 틀렸던 것이다.
#
# 리스크 추론("이 기업에 어떤 악재가 있었나", "공급망에 무슨 일이 있었나")에는
# 사건 이력이 필요하고, 그건 이 축으로만 모인다.
#
# ★검색 커버리지 실측(2026-07-28) — 좁은 구문일수록 **과거를 깊게** 본다:
#     "삼성전자"        1,000건 → 0일   (상한에 걸려 하루치만)
#     "삼성전자 공급계약"    52건 → 7,506일(20년)
#     "삼성전자 과징금"      43건 → 6,655일
#     "SK하이닉스 화재"     201건 → 4,665일
#   네이버는 start 상한이 1,000이라, 결과가 많은 쿼리일수록 최근만 잘린다.
#   그래서 **기업명 단독 검색은 쓰지 않는다** — 대형주에서 하루치밖에 못 가져온다.
RISK_KEYWORDS = (
    # 사고·생산중단 (공급망 리스크의 핵심)
