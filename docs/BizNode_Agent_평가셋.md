...
|---:|---|---|---|---|---:|---|
| 1 | `query-relations-supplies` | 삼성전자에 납품하는 기업은? | query | 호출 | 5 | PASS |
...
| 9 | `query-multi-company-relation` | 한미반도체와 SK하이닉스 관계 알려줘 | query | 호출 | 3 | PASS |
| 10 | `query-multitool-invest-and-price` | 레인보우로보틱스 최근 투자 상황이랑 주가도 같이 알려줘 | query | 호출 | 2 | PASS |
| 11 | `query-event-info-leak` | 현대오토에버 정보유출 사건 | query | 호출 | 1 | PASS |
...
| 도구 호출 총계 | 37 |
| 케이스당 도구 호출 | 최소 1 · 중앙 2 · 최대 5 (상한 12) |
| ★**Agent 루프가 예산으로 잘린** 케이스 | 0 / 20 |
| 최종 `budget_exhausted` 플래그 | 9 / 20 |
| 임베딩 호출 | 33회 · 캐시 적중 1157 · 빗나감 15 |
...
|---|---:|---:|---:|
| `tool_calls_used` | 12 | 5 | 0 |
| `events_used` | 40 | 20 | 0 |
| `propagations_used` | 12 | 303 | 9 |
| `hops_used` | 6 | 0 | 0 |
...
- `propagations_used` — 상한 **12** 인데 최대 **303** 을 썼습니다 (**25배**).

이건 상한이 낮다는 뜻이 **아닙니다.** 자르는 단위와 세는 단위가 다르다는 뜻입니다 — `fetch_propagation` 은 **입력(파급을 계산할 사건 수)** 을 잘라 놓고 **출력(파급 행 수)** 을 씁니다. 사건 하나가 수십 행을 내므로 잘라도 카운터는 상한을 훌쩍 넘습니다. 그래서 「막는다」고 적힌 예산이 실제로는 막지 못하고, 루프가 끝난 뒤 플래그만 켭니다.

★**이번 단계에서 고치지 않았습니다.** Phase 8 은 현재 동작을 고정한 채 재는 단계이고, 이 표가 그 측정 결과입니다. 고치면 무엇을 쟀는지가 흐려집니다.

★캐시가 **15건 빗나갔습니다** — 그만큼 이 실행에서 직접 계산했고, 그 값은 실행마다 흔들립니다(현황서 §8-13). 기준선으로 쓰려면 `EMBED_CACHE_STRICT=1` 로 다시 재세요.

### 도구별 호출 빈도

| 도구 | 호출 | 쓴 케이스 | 인용 가능 | 거부 |
|---|---:|---:|---|---:|
| `get_relations` | 7 | 3 | 불가 | 0 |
| `get_events` | 11 | 9 | 불가 | 0 |
| `search_news` | 4 | 4 | 가능 | 0 |
| `search_dart` | 2 | 2 | 불가 | 0 |
| `get_business_overview` | 2 | 2 | 불가 | 0 |
| `get_market` | 5 | 4 | 불가 | 0 |
| `get_filings` | 6 | 5 | 불가 | 0 |
...
관계 kept **126** · cut **903** · 링 없는 근거 인용 **34**건(사건·뉴스 근거에는 링이 없습니다).

### 이 표에서 읽히는 것 — ★판정이 아니라 관측입니다

- **Ring R2 는 본 것이 있는데 상한에 하나도 못 남았습니다** — 안쪽 링이 먼저 자리를 채우고 끝났다는 뜻입니다. 링 순서가 의도대로 동작한 결과일 수도, 상한이 너무 낮은 것일 수도 있습니다.
- **Ring R3 는 재료로 남았지만 한 번도 인용되지 않았습니다** — 링 순서가 프롬프트까지는 갔는데 답변까지는 안 갔다는 뜻입니다.
- **최종 인용 39건 중 34건이 관계가 아닙니다**(사건·뉴스 근거). 링 랭킹을 손대도 인용의 대부분은 안 움직인다는 뜻입니다.
...
| 실제 도구 | `get_relations`×5 (호출 5) |
| 재료 | 관계 65 · 사건 0 · 근거 80 · 최종 인용 5 |
| 링 | 본 것 R0:3 · R1:177 · R2:44 · R3:26 / 인용 R0:1 · R1:4 |
| 임베딩 | 2회 (적중 15 · 빗나감 1) |
| 예산 | 루프가 잘림 아니오 · 최종 플래그 아니오 (호출 5/12 · 사건 0/40 · 파급 0/12) |
| 소요 | 18032ms |
...
| 링 | 본 것 없음 / 인용 없음 |
| 임베딩 | 3회 (적중 74 · 빗나감 0) |
| 예산 | 루프가 잘림 아니오 · 최종 플래그 예 (호출 3/12 · 사건 10/40 · 파급 149/12) |
| 소요 | 10743ms |
...
| 링 | 본 것 없음 / 인용 없음 |
| 임베딩 | 3회 (적중 36 · 빗나감 1) |
| 예산 | 루프가 잘림 아니오 · 최종 플래그 예 (호출 3/12 · 사건 10/40 · 파급 36/12) |
| 소요 | 11206ms |
...
| 재료 | 관계 50 · 사건 19 · 근거 83 · 최종 인용 3 |
| 링 | 본 것 R0:3 · R1:415 · R2:115 · R3:37 / 인용 없음 |
| 임베딩 | 3회 (적중 147 · 빗나감 3) |
| 예산 | 루프가 잘림 아니오 · 최종 플래그 예 (호출 2/12 · 사건 19/40 · 파급 138/12) |
| 소요 | 8977ms |
...
| 실제 anchor | query · ['SK하이닉스'] |
| 실제 도구 | `get_events`×1 · `get_market`×1 · `get_relations`×1 (호출 3) |
| 재료 | 관계 10 · 사건 10 · 근거 21 · 최종 인용 1 |
| 링 | 본 것 R0:3 · R1:162 · R2:44 / 인용 없음 |
| 임베딩 | 3회 (적중 68 · 빗나감 0) |
| 예산 | 루프가 잘림 아니오 · 최종 플래그 예 (호출 3/12 · 사건 10/40 · 파급 303/12) |
| 소요 | 5938ms |
...
| 실제 도구 | `get_events`×1 (호출 1) |
| 재료 | 관계 0 · 사건 10 · 근거 12 · 최종 인용 4 |
| 링 | 본 것 없음 / 인용 없음 |
| 임베딩 | 3회 (적중 25 · 빗나감 0) |
| 예산 | 루프가 잘림 아니오 · 최종 플래그 예 (호출 1/12 · 사건 10/40 · 파급 48/12) |
| 소요 | 8394ms |
...
| 링 | 본 것 없음 / 인용 없음 |
| 임베딩 | 3회 (적중 188 · 빗나감 2) |
| 예산 | 루프가 잘림 아니오 · 최종 플래그 예 (호출 2/12 · 사건 19/40 · 파급 183/12) |
| 소요 | 7830ms |
...
| 링 | 본 것 없음 / 인용 없음 |
| 임베딩 | 3회 (적중 189 · 빗나감 1) |
| 예산 | 루프가 잘림 아니오 · 최종 플래그 예 (호출 2/12 · 사건 16/40 · 파급 130/12) |
| 소요 | 13269ms |
...
| 재료 | 관계 0 · 사건 19 · 근거 21 · 최종 인용 5 |
| 링 | 본 것 없음 / 인용 없음 |
| 임베딩 | 4회 (적중 204 · 빗나감 5) |
| 예산 | 루프가 잘림 아니오 · 최종 플래그 예 (호출 2/12 · 사건 20/40 · 파급 262/12) |
| 소요 | 10392ms |
...
| 실제 anchor | workspace · ['삼성전자', 'SK하이닉스'] |
| 실제 도구 | `get_events`×2 (호출 2) |
| 재료 | 관계 0 · 사건 16 · 근거 65 · 최종 인용 6 |
| 링 | 본 것 없음 / 인용 없음 |
| 임베딩 | 4회 (적중 207 · 빗나감 2) |
| 예산 | 루프가 잘림 아니오 · 최종 플래그 예 (호출 2/12 · 사건 20/40 · 파급 219/12) |
| 소요 | 12229ms |
