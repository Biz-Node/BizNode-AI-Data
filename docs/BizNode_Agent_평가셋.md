...
| 모델 | 호출 | 입력 | 출력 | (그중 추론) |
|---|---:|---:|---:|---:|
| `gpt-4o-mini-2024-07-18` | 43 | 113,017 | 7,908 | 0 |
| `gpt-5.6-sol` | 18 | 142,427 | 9,101 | 3,481 |
...
★캐시가 **34건 빗나갔습니다** — 그만큼 이 실행에서 직접 계산했고, 그 값은 실행마다 흔들립니다(현황서 §8-13). 기준선으로 쓰려면 `EMBED_CACHE_STRICT=1` 로 다시 재세요.
...
관계 kept **110** · cut **898** · 링 없는 근거 인용 **33**건(사건·검색히트·뉴스 근거에는 링이 없습니다 — **정상**) · 관계인데 링을 못 찾은 인용 **0**건(**0 이어야 정상**).
...
- **최종 인용 40건 중 33건이 관계가 아닙니다**(사건·뉴스 근거). 링 랭킹을 손대도 인용의 대부분은 안 움직인다는 뜻입니다.
...
### 4-1. 연결성 — `_STRIP_UNLINKED_CLAIMS` 를 켤 수 있나

주장 **42**건 중 연결 없음 **6** · 판정 불가 **10**건입니다.

- **연결 없음**(`intent_linked is False`) — 든 근거가 질문이 지목한 사건 종류에서 오지 않았습니다. 플래그를 켜면 **이것들이 답변에서 지워집니다**.
- **판정 불가**(`None`) — 질문이 사건 종류를 하나도 지목하지 않았거나 근거의 출처를 모릅니다. ★**차단 대상이 아닙니다** — 「연결 없음」과 섞으면 관계 질의가 통째로 막힌 것처럼 보입니다.

#### 연결 없음으로 판정된 주장 — ★사람이 읽고 오탐을 세는 자리

플래그를 켜기 전에 **이 표의 문장이 정말 질문과 무관한지** 한 건씩 봐야 합니다. 멀쩡한 문장이 여기 있으면 그것이 오탐이고, 오탐이 많으면 켜서는 안 됩니다.

| 케이스 | 질문 | 지워질 주장 | 든 근거 |
|---|---|---|---|
| `query-news-citable` | 파두 실적 논란 어떻게 됐어? | 회사와 주관사가 실적을 고의로 숨겼다는 의혹은 소송과 금융당국 조사로 이어졌습니다. | ev_455dee0b5c040f5e |
| `query-news-citable` | 파두 실적 논란 어떻게 됐어? | 2025년 11월 보도 기준 주주들이 파두를 상대로 낸 증권집단소송은 1년 8개월째 진행이 지연되고 있었습니다. | ev_494599f9e0a0810a |
| `query-news-citable` | 파두 실적 논란 어떻게 됐어? | 2026년 7월 보도된 자본시장법 위반 혐의 2차 공판에서는 실적이 기대에 못 미친 원인을 두고 파두와 검찰이 공방을 벌였습니다. | ev_204dd13c09644227 |
| `query-multi-company-suit` | 삼성전자와 SK하이닉스 둘 다 관련된 소송 있어? | 2022년 보도에 따르면 삼성전자·SK하이닉스·마이크론은 미국에서 D램 가격 담합 의혹으로 피소됐으나, 약 4년간의 공방 끝에 최종 승소했습니다. | ev_1aa810f59e590f35 |
| `ws-relationship-regulator` | 최근 규제당국 조사 동향 | 2026년 2월 미국 현지 로펌이 SK하이닉스의 솔리다임 지분 취득 과정에 대한 조사에 착수했습니다. | ev_a00a445e309a3c4f |
| `ws-relationship-regulator` | 최근 규제당국 조사 동향 | 해당 솔리다임 지분 조사는 규제당국 조사가 아니라 민간 로펌 조사입니다. | ev_a00a445e309a3c4f |
...
| 재료 | 관계 50 · 사건 19 · 근거 79 · 최종 인용 3 |
| 링 | 본 것 R0:3 · R1:415 · R2:115 · R3:37 / 인용 R0:1 · R1:1 |
| 임베딩 | 3회 (적중 149 · 빗나감 1) |
