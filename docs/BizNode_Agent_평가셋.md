...
> ★**이 수치는 머지 전 코드로 만들어졌습니다** — `worktree-model-knob-and-instruments`
> 에서 Phase 13(배열 근거 2차 조회)이 살아 있고 workspace-less(`context_keys`)도 없던
> 시점입니다. 머지 후 코드와 **맞지 않습니다.** 다시 만들어야 정본이 됩니다 —
> LLM 실호출이라 비용이 들어(1패스 ≈2분 15초 · ≈$0.045) **사용자 확인 뒤에** 돌립니다.
...
| 지표 | 값 |
|---|---|
| 도구 호출 총계 | 35~36 · 평균 35.8 · 편차 0.447214 (n=5) |
| 입력+출력 토큰 | 256512~263147 · 평균 261390 · 편차 2746.92 (n=5) |
| uncited 비율 | 0.0%~2.9% · 평균 2.2% · 편차 1.3% (n=5) |
| 임베딩 캐시 빗나감 | 3~10 · 평균 7.2 · 편차 2.77489 (n=5) |
| ★**도구 조합 안정성** | 0.97 (1.00 = 매번 같은 조합) |

★**빗나감을 나란히 보세요.** 0 이 아니면 그 패스는 임베딩을 직접 계산했고 그 값은 실행마다 흔들립니다(현황서 §8-13) — 위 변동폭 중 얼마가 Agent 때문인지 그만큼 못 가릅니다. `EMBED_CACHE_STRICT=1` 은 이걸 0 으로 만드는 장치가 **아닙니다**(Evaluation §10-7).

★**두 줄을 갈라 읽으세요.** 총계의 폭만 보면 안 됩니다 — 도구 **조합**이 통째로 바뀌었는데 합계는 같을 수 있습니다(`get_relations×2 + get_events×1` 과 `get_relations×1 + search_news×2` 는 둘 다 3입니다). 「모델을 바꿔 도구 선택이 안정됐나」에 답하는 것은 **안정성** 쪽입니다.

| 케이스 | 조합 안정성 | 서로 다른 조합 |
|---|---:|---:|
| `query-multi-company-relation` | 0.60 | 2 |
| `query-news-citable` | 0.80 | 2 |
| `query-dart-evidence` | 1.00 | 1 |
| `query-event-capital-smallcap` | 1.00 | 1 |
| `query-event-info-leak` | 1.00 | 1 |
| `query-events-labor` | 1.00 | 1 |
| `query-filings-list` | 1.00 | 1 |
| `query-market-no-evidence-id` | 1.00 | 1 |
| `query-multi-company-suit` | 1.00 | 1 |
| `query-multitool-invest-and-price` | 1.00 | 1 |
| `query-overview-context-only` | 1.00 | 1 |
| `query-relations-supplies` | 1.00 | 1 |
| `unresolved-gibberish` | 1.00 | 1 |
| `unresolved-unknown-company` | 1.00 | 1 |
| `ws-market-across-workspace` | 1.00 | 1 |
| `ws-relationship-regulator` | 1.00 | 1 |
| `ws-semantic-capital-trend` | 1.00 | 1 |
| `ws-semantic-collusion` | 1.00 | 1 |
| `ws-semantic-quality` | 1.00 | 1 |
| `ws-semantic-strike` | 1.00 | 1 |

★**이 표는 판정이 아닙니다.** 낮은 안정성이 곧 결함이 아니라, 도구 선택이 LLM 몫이라 구조적으로 결정론이 아니라는 사실의 크기입니다. 모델을 바꿀 때 **이 값과 비교**하라고 둡니다.
...
| 모델 | 호출 | 입력 | 출력 | (그중 추론) |
|---|---:|---:|---:|---:|
| `gpt-4o-mini-2024-07-18` | 61 | 251,034 | 12,113 | 0 |
...
★캐시가 **6건 빗나갔습니다** — 그만큼 이 실행에서 직접 계산했고, 그 값은 실행마다 흔들립니다(현황서 §8-13). 기준선으로 쓰려면 `EMBED_CACHE_STRICT=1` 로 다시 재세요.
...
관계 kept **110** · cut **898** · 링 없는 근거 인용 **32**건(사건·검색히트·뉴스 근거에는 링이 없습니다 — **정상**) · 관계인데 링을 못 찾은 인용 **0**건(**0 이어야 정상**).
...
- **최종 인용 38건 중 32건이 관계가 아닙니다**(사건·뉴스 근거). 링 랭킹을 손대도 인용의 대부분은 안 움직인다는 뜻입니다.
