...
| 21 | `ctx-detail-page-no-workspace` | 이 회사에 노무 관련 리스크가 있었나? | context | 호출 | 3 | PASS |
| 22 | `ctx-beats-workspace` | 이 회사 최근에 무슨 일이 있었어? | context | 호출 | 1 | PASS |
| 23 | `gate-no-starting-point` | 무슨 일이 있었는지 알려줘 | — | 미호출 | 0 | PASS |
...
| 항목 | 값 |
|---|---|
| Agent 가 불린 케이스 | 20 / 23 |
| 도구 호출 총계 | 37 |
| 케이스당 도구 호출 | 최소 1 · 중앙 2 · 최대 3 (상한 12) |
| ★**Agent 루프가 예산으로 잘린** 케이스 | 0 / 23 |
| ★**예산이 막아 실행 안 한** 호출 | 0 |
| 최종 `budget_exhausted` 플래그 | 0 / 23 |
| 임베딩 호출 | 35회 · 캐시 적중 1171 · 빗나감 51 |
...
| 모델 | 호출 | 입력 | 출력 | (그중 추론) |
|---|---:|---:|---:|---:|
| `gpt-4o-mini-2024-07-18` | 45 | 110,685 | 7,980 | 0 |
| `gpt-5.6-sol` | 20 | 153,377 | 12,107 | 5,517 |
...
★캐시가 **51건 빗나갔습니다** — 그만큼 이 실행에서 직접 계산했고, 그 값은 실행마다 흔들립니다(현황서 §8-13). 기준선으로 쓰려면 `EMBED_CACHE_STRICT=1` 로 다시 재세요.
...
관계 kept **110** · cut **898** · 링 없는 근거 인용 **40**건(사건·검색히트·뉴스 근거에는 링이 없습니다 — **정상**) · 관계인데 링을 못 찾은 인용 **0**건(**0 이어야 정상**).

### 3-1. 워크스페이스 유무별 링 분포 — ★이번 실행에서는 갈라 볼 것이 없습니다
...
- **최종 인용 47건 중 40건이 관계가 아닙니다**(사건·뉴스 근거). 링 랭킹을 손대도 인용의 대부분은 안 움직인다는 뜻입니다.

## 4. 주장(claim) — 관측만 합니다

| 항목 | 값 |
|---|---|
| `check_claims` 를 지난 케이스 | 20 / 23 |
| 주장 총계 | 45 |
| ★**uncited**(근거를 안 단 주장) | 0 · **0.0%** |
| no_text(근거 원문을 못 찾음) | 0 |
| unlinked(질문 의도와 연결 없음) | 5 |
...
주장 **45**건 중 연결 없음 **5** · 판정 불가 **18**건입니다.
...
| 케이스 | 질문 | 지워질 주장 | 든 근거 |
|---|---|---|---|
| `query-news-citable` | 파두 실적 논란 어떻게 됐어? | 파두 실적 논란은 아직 종결된 것으로 확인되지 않습니다. | ev_204dd13c09644227 |
| `query-news-citable` | 파두 실적 논란 어떻게 됐어? | 2025년 11월 보도 기준 주주들이 제기한 증권집단소송은 1년 8개월째 진행이 정체돼 있었습니다. | ev_494599f9e0a0810a |
| `query-news-citable` | 파두 실적 논란 어떻게 됐어? | 2026년 7월 24일 보도 기준 자본시장법 위반 혐의 등의 2차 공판에서 파두와 검찰이 실적 부진 원인을 두고 공방을 벌였습니다. | ev_204dd13c09644227 |
| `query-multi-company-suit` | 삼성전자와 SK하이닉스 둘 다 관련된 소송 있어? | 2022년 보도에 따르면 미국에서 D램 가격 담합 의혹으로 피소된 삼성전자·SK하이닉스·마이크론이 약 4년의 공방 끝에 최종 승소했습니다. | ev_1aa810f59e590f35 |
| `ws-relationship-regulator` | 최근 규제당국 조사 동향 | 2026년 2월에는 SK하이닉스의 솔리다임 지분 취득 과정에 대한 미국 현지 로펌의 조사가 보도됐지만, 이는 규제당국 조사로 확인된 사안은 아닙니다. | ev_a00a445e309a3c4f |
...
| 워크스페이스 · 보고 있는 기업 | ['00126380', '00164779'] · 없음 |
| 기대 Agent 호출 | 예 |
...
| 실제 anchor | query · ['SK하이닉스'] |
| 실제 도구 | `get_events`×1 · `get_relations`×1 · `search_news`×1 (호출 3) |
| 재료 | 관계 10 · 사건 10 · 근거 31 · 최종 인용 0 |
| 링 | 본 것 R0:3 · R1:162 · R2:44 / 인용 없음 |
| 임베딩 | 1회 (적중 64 · 빗나감 0) |
| 예산 | 루프가 잘림 아니오 · 최종 플래그 아니오 (호출 3/12 · 사건 10/40 · 파급 6/12) |
| 소요 | 10398ms |
...
| 재료 | 관계 0 · 사건 10 · 근거 12 · 최종 인용 1 |
| 링 | 본 것 없음 / 인용 없음 |
| 임베딩 | 3회 (적중 16 · 빗나감 0) |
| 예산 | 루프가 잘림 아니오 · 최종 플래그 아니오 (호출 1/12 · 사건 10/40 · 파급 7/12) |
| 소요 | 7477ms |
...
### 21. `ctx-detail-page-no-workspace` — **PASS**

| | |
|---|---|
| 질문 | `이 회사에 노무 관련 리스크가 있었나?` |
| 기대 anchor_source | context |
...
| 실제 도구 | `get_events`×1 · `search_dart`×1 · `search_news`×1 (호출 3) |
| 재료 | 관계 0 · 사건 8 · 근거 13 · 최종 인용 4 |
| 링 | 본 것 없음 / 인용 없음 |
| 임베딩 | 3회 (적중 12 · 빗나감 10) |
| 예산 | 루프가 잘림 아니오 · 최종 플래그 아니오 (호출 3/12 · 사건 8/40 · 파급 6/12) |
| 소요 | 14499ms |

### 22. `ctx-beats-workspace` — **PASS**

| | |
|---|---|
| 질문 | `이 회사 최근에 무슨 일이 있었어?` |
| 기대 anchor_source | context |
| 워크스페이스 · 보고 있는 기업 | ['00126380', '00164779'] · ['00164742'] |
| 기대 Agent 호출 | 예 |
| 끌어오려는 도구 | `get_events`, `get_relations` |
| 무엇을 검증하나 | ★**담아 둔 기업이 있어도 보고 있는 기업이 먼저다.** 워크스페이스로 가면 「현대자동차 페이지를 보며 물었는데 삼성전자로 답하는」 것이 되고, 그건 §14-3 이 막으려는 오답과 같은 종류다. ★앞 케이스와 **워크스페이스만 다르다** — 링이 R3 일색이 아니라 정상 분포로 나와야 하므로, 두 케이스가 §3-1 의 두 줄이 된다 |
| 커버 분기 | anchor:CONTEXT, agent:호출됨, topic:이벤트 탐색 |
| 실제 anchor | context · ['현대자동차'] |
| 실제 도구 | `get_events`×1 (호출 1) |
| 재료 | 관계 0 · 사건 8 · 근거 8 · 최종 인용 4 |
| 링 | 본 것 없음 / 인용 없음 |
| 임베딩 | 3회 (적중 14 · 빗나감 8) |
| 예산 | 루프가 잘림 아니오 · 최종 플래그 아니오 (호출 1/12 · 사건 8/40 · 파급 6/12) |
| 소요 | 19611ms |

### 23. `gate-no-starting-point` — **PASS**

| | |
|---|---|
| 질문 | `무슨 일이 있었는지 알려줘` |
| 기대 anchor_source | **없음** (게이트에서 끝 — 앵커 판정 자체가 없다) |
| 워크스페이스 · 보고 있는 기업 | 없음 · 없음 |
| 기대 Agent 호출 | **아니오** |
| 끌어오려는 도구 | 없음 |
| 무엇을 검증하나 | ★**검색조차 하지 않는다**(설계서 §16-2). 담은 것도 보고 있는 것도 없으면 「무엇에 대한 인사이트인가」가 정해지지 않는다. ★`expected_anchor_source` 가 **`None`** 인 유일한 케이스다 — `guard_workspace` 가 `resolve_anchor` 앞에서 끊으므로 앵커 판정 자체가 없다. 응답에는 `unresolved` 가 실리는데, 그 비대칭을 `test_case` ⑦ 이 명시한다. ★LLM 을 안 부르므로 비용이 0 이다 |
| 커버 분기 | gate:출발점 없음, agent:미호출 |
| 실제 anchor | — · 없음 |
| 실제 도구 | 없음 (호출 0) |
| 재료 | 관계 0 · 사건 0 · 근거 0 · 최종 인용 0 |
| 링 | 본 것 없음 / 인용 없음 |
