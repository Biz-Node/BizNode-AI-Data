...
| **Workspace context** | `POST /ask` 의 **필수** `workspace_keys` 가 주는 **현재 분석 문맥** |
...
### ★이번에 확정하지 **않는** 것 — 두 층을 구분한다 `[DECIDE]`

| | 층 | 이번 개정 |
|---|---|---|
| 1 | Workspace 를 **질문의 대상 문맥으로 해석한다** | ✅ **확정** — 답변이 무엇에 관한 것인지의 **정의** |
| 2 | Workspace 기업을 **Retrieve 의 material anchor 로 직접 쓴다** | 🔶 **`[DECIDE]`** — Retrieve 가 **어떤 키로 재료를 모으는가**의 구현·설계 문제 |

1 이 정해졌다고 2 가 따라오지 않습니다. Workspace 기업을 실제 material anchor 로 쓸지,
쓴다면 어떤 방식으로 재료를 모을지는 **확정하지 않습니다**
([현황서 §7](BizNode_Search_Layer_현황서.md#7-결정해야-할-것--decide)).
...
Question
  ...
  ├─⑥.5 Material Consistency 재료끼리 어긋나는지 본다      ← ★신설 (검사 규칙 미확정)
...
### ★먼저 용어 — 「앵커 없음」이 두 가지로 읽히지 않게 (2026-08-23)
...
POST /ask 는 workspace_keys 를 **필수**로 받는다
    → /ask 에는 **Workspace context 가 항상 있다.**
    → query anchor 가 없으면 Workspace context 를 질문의 대상 문맥으로 해석한다.

「대상 문맥 자체가 없는 경우」는 /ask 밖이다
    → 홈 통합검색 · AI 인사이트. 아래 ⓐ 가 그것이고 이미 이관했다.
```

아래 표의 ⓑ·ⓒ 는 **문제 정의가 그대로**이고, 용어만 위 정의에 맞췄습니다.

기업명이 안 나오는 질문은 **한 종류가 아니라 세 종류**이고, 셋이 서로 다르게 망가집니다.

| 경우 | 예 | 지금 가는 길 | `match_type` | 무엇이 문제인가 |
|---|---|---|---|---|
| **ⓐ 주제 질의** — query anchor 도 Workspace context 도 없음 *(→ `/ask` 밖으로 **이관**)* | 「메모리 가격 담합」·「반도체 업계 파업 위험」 | `edge_types` 없음 → 해소 실패 → **SEMANTIC** | `SEMANTIC` | 무관 기업이 `companies[]` 를 채운다. `intent_of()` 가 지울 앵커가 없어 **질문 전체가 의도**가 된다 |
| **ⓑ query anchor 없는 관계 질의** — Workspace context 는 있음 | 「납품 단가 압박」·「최근 인수 사례」 | `edge_types` 있음 → anchorless → source 5 + target 5 슬롯을 **점수순으로 아무거나** | 🔴 **`EXACT`** | `RELATIONSHIP` → `EXACT` 매핑이라 프롬프트가 「정확히 일치한 결과」라고 말한다. **헤지가 아예 안 걸린다** — ⓐ보다 나쁘다 |
| **ⓒ query anchor 는 있는데 못 뽑음** | 「NAVER에 …」 (별칭·조사 문제) | 앵커 실패 → ⓐ 로 떨어짐 | `SEMANTIC` | **다른 문제다** — [현황서 §4](BizNode_Search_Layer_현황서.md#4-알려진-결함--검색) `AnchorExtractor` 항목 |
...
**ⓑ·ⓒ 는 `/ask` 에서 그대로 발생합니다** — Workspace context 가 있어도 질의 **문장 안에**
query anchor 가 없거나(ⓑ) query anchor 가 있는데 못 뽑는(ⓒ) 경우이기 때문입니다. 아래
서술은 그 둘에 대한 것이고 **문제 정의는 변경 없습니다.**

### ★B4 이후 — 「없음」과 「못 뽑음」이 전보다 확실히 갈렸다 (2026-08-23 실측)
...
★**이 수치를 근거로 새 anchor 정책이나 Workspace material anchor 정책을 추가로 확정하지
않습니다.** 위는 **분리가 선명해졌다는 관찰**이고, material anchor 는
[3절의 `[DECIDE]`](#이번에-확정하지-않는-것--두-층을-구분한다-decide) 그대로입니다.
