...
### 13-6. ★이후 재검증 — 최종 아키텍처는 기술부채 문서 9절에 있다

§13 은 `/ask` 를 **처음 만들 때의** 설계(2026-08-22)입니다. 그 뒤 Step 1~4a 를 거치며
기업별 evidence 오염·사건 상한 부재·claim 단위 검증·한국어 토큰화 같은 문제가 실측으로
드러났고, 2026-08-23 에 **「`/ask` 가 정확히 어떤 시스템인가」를 코드와 독립적으로 다시
정의**했습니다.

그 결과는 [기술부채/설계검토 문서 9절](BizNode_기술부채_설계검토.md#9-ask-최종-아키텍처-및-책임-범위--재검증-2026-08-23)에
있습니다 — `/ask` 의 3단 정의(Graph QA → Evidence-grounded RAG → Insight), 10단계 flow 의
단계별 책임과 금지사항, Relationship 과 Event 를 가르는 이유, claim 5종, 사실/인과/전망/
Insight 4등급, 앵커 없는 질의 정책, 개발 순서.

**§13 의 내용 중 무엇도 철회되지 않았습니다.** 9절은 §13 위에 계층 구분을 얹은 것이고,
특히 아래 셋은 9절이 **유지 대상으로 재확인**했습니다.

- §13-1 의 조립 순서(`retrieve()` 를 먼저 부른다) — 9절 3단 구조를 코드가 이미 강제한다
- §13-2 의 구조적 인젝션 방어 + 화이트리스트 2차 방어선 — 사실 등급의 유일한 서버 강제
- §13-5 의 「LLM-judge 없음」 — 9절 claim 5종 중 넷은 결정론적 대조로 풀린다
