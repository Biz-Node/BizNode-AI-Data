...
| `propagations_used` | 12 | `fetch_propagation` — ★**세기만 하지 않고 입력을 실제로 자릅니다** |
...
|---|---:|---:|---:|---|
| `tool_calls_used` | 12 | **5** | 0 | 한 번도 안 뭅니다. 12 는 넉넉합니다 |
...
0건**입니다 — 플래그는 루프가 끝난 뒤에 켜졌고 그 뒤로 아무도 읽지 않아 지금은 무해합니다.
다만 「막는다」고 적힌 예산이 실제로는 막지 못하고 있습니다.

★**이번 단계에서 고치지 않았습니다.** Phase 8 은 현재 동작을 고정한 채 재는 단계이고,
고치면 무엇을 쟀는지가 흐려집니다.

### 9-2. 링(ring) 순서 — ★관측만 했습니다 (2026-08-28)

`ranking 정책은 이 단계에서 바꾸지 않습니다.` Agent 평가셋 20 케이스 누적입니다.

> ★**이 절의 수치는 결함이 있는 계측으로 잰 것입니다.** 아래 「무엇이 틀렸나」를
> 먼저 읽으세요. 계측은 고쳤지만(`app/core/observe.py`) **재측정은 아직 안 했습니다** —
> LLM 실호출이라 비용이 들어 다음 실행 때 함께 갱신합니다.

| 링 | a7925ae | 41bb1bb | | 본 관계 | 상한에 남은 것 | 최종 인용 |
|---|---|---|---|---:|---:|---:|
...
kept 126 → 110 · cut 903 → 898. 최종 인용 39건 중 **33건이 관계가 아닙니다**(사건·뉴스 근거).

#### ★무엇이 틀렸나 — 계측이 관계가 아니라 「호출 × 관계」를 세고 있었다

`observe.record_rings()` 는 `get_relations` **호출마다** 불리는데 중복을 안 접었습니다.
Agent 가 같은 기업으로 도구를 두 번 부르면 같은 관계가 두 번 세어집니다. 그런데
**몇 번 부를지는 LLM 이 정합니다** — 실측에서 `get_relations` 호출이 한 실행은 7회,
다른 실행은 3회였습니다.

그래서 위 표의 **개수 변화(R1 109→101 · R3 8→0 · kept 126→110)는 41bb1bb 의 효과가
아닙니다.** 그럴 수가 없습니다:

    ordered = [R0 블록] + [R1 블록] + [R2 블록] + [R3 블록]   ← 링 순서 고정
    kept    = ordered[:limit]                                  ← 프리픽스 컷

`relation_selector.order()` 는 `ordered = list(rows)` 뒤 `sort()` 만 합니다 — **길이를
보존하는 순열**이라 블록 크기를 못 바꿉니다. 프리픽스 컷에서 링별 kept **개수**는
블록 크기와 `limit` 만으로 정해지므로, 링 **안** 순서는 개수에 영향을 줄 수 없습니다.
(교차 확인: 다른 세션의 실측에서도 정렬 ON/OFF 간 링별 kept 개수가 11/11 동일했고,
달라진 것은 **어떤 엣지가 남나** 뿐이었습니다.)

즉 41bb1bb 가 바꾸는 것은 **「몇 개가 남나」가 아니라 「무엇이 남나」**이고, 위 표는
그것을 보여주지 못합니다.

★**이것은 [§8-13](#8-13-기준선-비결정성의-뿌리--임베딩-값-드리프트-2026-08-28--phase-175) 이
임베딩에 대해 막아 둔 것과 같은 종류의 귀속 문제입니다.** 2차의 완료 기준이 평가셋
점수인데, 점수 차이를 랭킹에 귀속시킬지 도구 선택에 귀속시킬지 못 가르면 기준 자체가
성립하지 않습니다. 임베딩 쪽에는 영속 캐시라는 장치가 있었지만 **도구 선택 쪽에는
없었습니다.**

**고친 것** — `record_rings` 가 `edge_id` 로 중복을 접습니다. 이제 `ring_seen`·`ring_kept`
는 「관계 몇 개」이고 호출 횟수에 흔들리지 않습니다. `tests/graph/test_observe_rings.py`
8건이 그것을 못 박습니다(특히 `test_call_count_does_not_change_ring_numbers` — 같은 입력을
1회 넣든 5회 넣든 링 수치가 같아야 한다). ★`cut` 만은 **안 접습니다** — 같은 관계가 한
호출에서 남고 다른 호출에서 잘릴 수 있어 「어느 쪽이 참인가」가 없고, 자른 **횟수**로
읽어야 하는 값입니다.

#### 그래도 남는 결론 — R2 는 0 이다

계측 결함과 **무관하게** 성립하는 것이 하나 있습니다. R2 는 두 실행 모두, 그리고 중복
계수와 상관없이 **kept 가 0** 입니다. 이유가 구조적이기 때문입니다 — 정렬은 링 **안에서만**
순서를 바꾸고 자르기는 링 **순서대로** 먹는데, R1 이 혼자 700건대라 R0+R1 이 `limit` 을
다 채우고 끝납니다. **R2 는 애초에 차례가 오지 않습니다.**

→ [§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo) 은 41bb1bb 로
**닫히지 않았습니다.** 링 *안* 순서가 아니라 링 *사이* 배분 문제이고, 링별 최소
할당(fair-share) 같은 배분 규칙이 없으면 정렬을 어떻게 바꿔도 R2·R3 는 0 입니다.

★**Ring ranking 변경 실험이 필요한가**에 대한 답은 「링 안 순서가 아니라 **링 사이
배분**을 봐야 한다」입니다. 그 결정은 아직 하지 않았습니다.

★인용의 85%(33/39)가 관계가 아니라는 것도 그대로 유효합니다 — 링 랭킹을 손대도 인용의
대부분은 안 움직입니다.
