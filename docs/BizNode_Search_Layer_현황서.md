...
· 테스트 **581개** (579 passed · 2 xfailed = 알려진 결함 [§4-5](#4-5-동음이의-사명은-원리적으로-못-가른다)·[§5-15](#5-15-사명의-부분-토큰이-실존-기업으로-해소된다-todo))
...
|---|---|
| 관련 | [§5-4](#5-4-관계에는-의도-필터가-없다-사건에는-있는데-todo) `relation_selector` 와 **같은 자리에서 풀릴 가능성**이 높다 |
...
| 수정 대상 | `answer_service._fact_lines()` → 블록 분할 · 프롬프트 규칙 문구(§5-9) · `role`·`symmetric`·`press` 노출(§5-5) · 보간값 개행 제거(§5-10) · ★**워크스페이스 소속 표기**([설계서 §12](BizNode_Search_Layer_설계.md#12-사실과-추론의-경계--4등급)) |
...
| 완료 조건 | 20개 대표 질문의 claim 이 타입별로 갈려 분포가 나온다. **④ Evidence-derived 만의 분포**가 처음으로 보인다 · ★claim ⑤ 에 **부분 검증 원천**(인사이트 문장의 주어·영향 대상이 워크스페이스 기업인가 — **집합 확인**)이 붙는다. **여전히 판정하지 않는다** |
...
| 무엇 | 왜 사람이 정하나 | 관련 |
|---|---|---|
| **⑥.5 의 검사 규칙과 불일치 시 처리 방식** | B2 는 **§5-14 유형**의 확정 사례 **24/938** 을 쟀지만 이는 **라벨↔근거 불일치 전체의 분모가 아니다.** §5-12 유형처럼 **연도·배경절 표지어 없이도** 발생하는 불일치가 확인됐다. 전체 규모와 유형을 더 파악한 뒤 규칙·처리 방식을 정한다. ★지금은 **「`/ask` 완충 vs 추출 파이프라인 재적재」를 판단하지 않는다** | [설계서 §10](BizNode_Search_Layer_설계.md#10-ask-전체-flow--10단계) · [§5-12](#5-12-event-라벨이-근거와-정반대인데-낱말-겹침이-못-잡는다-todo)·[§5-14](#5-14-기사의-배경절이-event-라벨이-된다-추출-파이프라인-measure) · [§6-3](#6-3-병행-가능한-독립-작업) B2-2 |
