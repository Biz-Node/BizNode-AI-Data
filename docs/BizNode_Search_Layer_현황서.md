...
★관계 쪽에는 이미 링(ring) 정렬이 있습니다(`_ring_of`). **사건 쪽에만 없습니다** —
...
**A-2. ★`AnchorExtractor` 오탐이 전역 경로를 통째로 막는다** `[TODO]`
...
**A-3. 사건 라벨에 마케팅 이벤트가 섞여 있다** `[MEASURE]`
