...
★**그래서 §9-2 의 링별 kept 개수 변화(R1 109→101 · R3 8→0)는 이 수정의 효과로 읽으면
  안 됩니다.** 위 성질상 개수는 이 수정으로 바뀔 수 없으므로, 그 차이는 **입력 모집단이
  달라진 것**(본 것 754→746 · 63→50)에서 옵니다 — 도구를 고르는 쪽이 LLM 이라 실행마다
  호출 구성이 흔들립니다(총 호출 33~37). §8-13 이 임베딩에 대해 막아 둔 것과 **같은
  종류의 귀속 문제**이고, 도구 선택 쪽에는 아직 그 장치가 없습니다.
