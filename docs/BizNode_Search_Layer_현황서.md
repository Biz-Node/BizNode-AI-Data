...
> 설계 근거·아키텍처는 [설계서](BizNode_Search_Layer_설계.md)를,
> 앞으로 무엇을 언제 결정·처리할지는 [기술 부채 및 설계 검토 사항](BizNode_기술부채_설계검토.md)을 보세요.
...
**현재 상태 한 줄** — 질문을 받아 답까지 돌려주는 데까지 **끝났습니다.**
백엔드는 `POST /ask` 를 부르면 되고, 추론 담당은 `app.services.answer_service` 를
직접 import 하면 됩니다. 남은 것은 **Redis 캐싱·N+1 최적화·랭킹 품질 평가** 등입니다.
