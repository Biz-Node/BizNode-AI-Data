...
### 9-2. 링(ring) 순서 — ★관측만 했습니다 (2026-08-28 · 41bb1bb 이후 재측정)

`ranking 정책은 이 단계에서 바꾸지 않습니다.` Agent 평가셋 20 케이스 누적입니다.

★**측정 시점이 중요합니다.** 처음 잰 것은 `a7925ae`(링 안 의도 정렬이 **꺼진** 상태)였고,
`41bb1bb` 이 그것을 되살린 뒤 **다시 쟀습니다**([§8-18](#8-18-링-정렬-상속-조사--세-단계-중-두-단계만-남았다-2026-08-28--phase-2)).
아래는 **정렬이 켜진 뒤**의 값이며, 두 시점을 나란히 둡니다.

| 링 | 본 관계 (전 → 후) | 상한에 남은 것 (전 → 후) | **최종 인용** (전 → 후) |
|---|---:|---:|---:|
...
관계 kept **126 → 110** · cut **903 → 898**. 최종 인용 39건 중 **33건이 관계가 아닙니다**
(사건·뉴스 근거).

**읽히는 것 넷 — 판정이 아니라 관측입니다.**

1. ★**의도 정렬 복구는 R2 를 살리지 못했습니다.** 41bb1bb 착수 전 예상은 「정렬이 켜지면
   `IS_EXECUTIVE_OF`·`REGULATES`·`HAS_EVENT` 가 R2 안에서 앞으로 올라온다」였는데, **R2 는
   전후 모두 0** 입니다. 이유는 구조적입니다 — **정렬은 링 *안에서만* 순서를 바꾸고, 자르기는
   링 *순서대로* 먹습니다.** R1 이 혼자 746건이라 상한을 다 채우고 끝나므로 R2 는 애초에
   차례가 오지 않습니다. 즉 [§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo)
   은 이 수정으로 **닫히지 않았습니다.** 별개의 구조 문제입니다
2. **R3 는 8 → 0 이 됐습니다.** 정렬이 켜지며 안쪽 링이 상한을 더 확실히 채웠습니다
3. **kept 가 126 → 110 으로 줄었습니다.** 자르는 규칙은 그대로인데 **무엇이 살아남는지가
   바뀌었습니다** — 이것이 41bb1bb 의 실제 효과입니다(개선이 아니라 1.5차 동작 복구)
4. **인용의 85%(33/39)가 관계가 아닙니다** — 링 랭킹을 손대도 인용의 대부분은 안 움직입니다

★**Ring ranking 변경 실험이 필요한가**에 대해 이 표가 주는 답은 「링 안 순서가 아니라
**링 사이 배분**이 문제」라는 것입니다. R1 이 상한을 독식하는 한 R2·R3 는 정렬을 어떻게
바꿔도 0 입니다. 링별 최소 할당(fair-share) 같은 **배분 규칙**이 없으면 안 바뀝니다.
그 결정은 아직 하지 않았습니다.

★도구 호출 분포는 41bb1bb 로 **안 바뀝니다** — `get_relations` 인자가 그대로라
도구 스키마·시스템 프롬프트가 동일합니다. 실측 총 호출은 36 · 33 · 37 · **34** 로,
같은 코드에서도 흔들리는 노이즈 범위 안입니다.
