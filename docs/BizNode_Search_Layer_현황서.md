...
· 테스트 **488개** (487 passed · 1 xfailed = 알려진 결함 [§4-5](#4-5-동음이의-사명은-원리적으로-못-가른다))
...
| **`workspace_keys` 는 필터가 아닙니다** | **순서만** 정합니다. 워크스페이스 밖 기업도, 사건·인물·기관·제품도 그대로 나옵니다. ★단 **`/ask` 에서는 필수 값입니다**(2026-08-23 확정) — 그래프 안에서 인사이트를 만드는 챗봇이라 워크스페이스 없이 부를 수 없습니다. 스키마 강제는 [§6-2](#6-2-순서--목적에서-역산한-6단계) ② 단계에서 붙습니다 |
...
★**Workspace context 와의 관계는 관찰로만 적습니다.** 「그 기업이 워크스페이스에 없다」는
사실을 **추가 신호로 쓸 여지는 있으나**, 이것을 anchor resolution 의 규칙으로 쓸지,
material anchor 결정에서 어떻게 쓸지는 **확정하지 않았습니다** —
[§7-0](#7-0-architecture-freeze-가-남긴-것--decide-3건-2026-08-23) 의 **Workspace material
anchor `[DECIDE]`** 와 묶여 있습니다.
...
★**2026-08-23 — 이 결함은 `/ask` 밖으로 이관됐습니다.** `POST /ask` 는 `workspace_keys` 를
**필수**로 받아 기업 컨텍스트가 전혀 없는 질의가 오지 않습니다
([설계서 §14 ⓐ](BizNode_Search_Layer_설계.md#14-앵커-없는-질의)). 결함 자체는 남습니다 — **홈 통합검색·
AI 인사이트의 문제**이고, `/ask` 에 남는 것은 ⓑ(앵커 없는 **관계** 질의 · §5-3)뿐입니다.
...
[설계서 §14](BizNode_Search_Layer_설계.md#14-앵커-없는-질의) 는 이 문제를 **앵커 없는 질의**로만 다뤘습니다.
...
★**2026-08-23 에 결정 하나가 확정되어 범위가 줄었습니다** — `POST /ask` 는 `workspace_keys` 를 **필수**로
받습니다. 그래프 안에서 인사이트를 만드는 챗봇이라 **기업 컨텍스트가 전혀 없는 질의가
`/ask` 에 오지 않습니다.** 그 결과 [설계서 §14](BizNode_Search_Layer_설계.md#14-앵커-없는-질의) 의
**ⓐ 주제 질의는 홈 통합검색·AI 인사이트로 이관**되어 아래 `[DECIDE]` 범위에서 빠졌습니다.

### 7-0. ★Architecture freeze 가 남긴 것 — `[DECIDE]` 3건 (2026-08-23)

이번 freeze 는 **정의와 자리**를 확정했고, **규칙과 구현**은 남겼습니다. 셋 다 사람이
정해야 다음이 진행됩니다.

| 무엇 | 왜 사람이 정하나 | 관련 |
|---|---|---|
| **Workspace context 의 기업을 Retrieve 에서 material anchor 로 직접 쓸까** | 「Workspace 를 **질문의 대상 문맥으로 해석한다**」(설계서 §3 **확정**)와 「Retrieve 가 **어떤 키로 재료를 모으는가**」(**미확정**)는 **다른 층**이다. 구현 방식이 정해져야 [§6-2](#6-2-순서--목적에서-역산한-6단계) ② 가 착수된다. ★[§4-5](#4-5-동음이의-사명은-원리적으로-못-가른다)(동음이의)와도 관련되므로 **함께 검토한다** | [설계서 §3](BizNode_Search_Layer_설계.md#3-워크스페이스는-필터가-아니라-랭킹-문맥이다) · [§4-5](#4-5-동음이의-사명은-원리적으로-못-가른다) |
...
| 앵커 없는 **관계** 질의에 「재료가 약하다」를 응답으로 내릴까 | (a) 별도 필드 (b) `match_type` 확장 (c) 답변 문장으로만 | [§5-3](#5-3-앵커-없는-관계-질의가-match_typeexact-로-나간다-decide) ★ⓐ(주제 질의 · §5-2)가 홈으로 이관돼 **결정이 단순해졌다** — 남은 것은 ⓑ 뿐이다 |
