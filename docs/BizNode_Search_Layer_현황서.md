...
`ranking 정책은 이 단계에서 바꾸지 않습니다.` 20 케이스 누적:

★**이 수치를 읽기 전에 [§8-18](#8-18-링-정렬-상속-조사--세-단계-중-두-단계만-남았다-2026-08-28--phase-2) 을 먼저 보세요.**
같은 날 조사에서 **링 안의 의도 정렬이 무력화돼 있음**이 확인됐습니다 —
`agent_tools.get_relations(keys)` 가 `edge_types`·`direction` 을 안 넘겨 `matched` 가 비고
`relation_selector.order()` 가 그대로 돌려줍니다. 즉 아래 숫자는 **「링 분류 + 링 순
줄세우기 + 정렬 뒤 자르기」는 살아 있고 「링 안 의도 정렬」만 꺼진 상태**에서 잰 것입니다.
둘은 같은 실행을 두 각도에서 본 것이라 **함께 읽어야** 합니다.

| 링 | 도구가 본 관계 | 상한에 남은 것 | **최종 인용** |
|---|---:|---:|---:|
| R0 양끝 모두 워크스페이스 안 | 9 | 9 | 1 |
| R1 워크스페이스 ↔ 바깥 기업 | 754 | 109 | 4 |
| R2 워크스페이스 ↔ 비-Company | 203 | **0** | 0 |
| R3 워크스페이스와 안 닿음 | 63 | 8 | 0 |

관계 kept **126** · cut **903**. 최종 인용 39건 중 **34건이 관계가 아닙니다**(사건·뉴스 근거).

읽히는 것 셋 — **판정이 아니라 관측입니다**:

1. **R2 는 203건을 봤는데 하나도 안 남았습니다.** 안쪽 링이 먼저 자리를 채우고 끝났습니다
2. **R3 는 재료로 남았지만 한 번도 인용되지 않았습니다** — 링 순서가 프롬프트까지는 갔는데 답변까지는 안 갔습니다
3. **인용의 87%(34/39)가 관계가 아닙니다** — 링 랭킹을 손대도 인용의 대부분은 안 움직입니다

★Ring ranking 변경 실험이 필요한지는 **이 수치를 보고 정합니다.**
