...
마지막 갱신 **2026-08-23** · 기준 커밋 `27c4e25`(브랜치 `yun`)
...
즉 **극성 반전은 답변 단계에서 생긴 것이 아닙니다.** 추출 파이프라인이 근거와 반대되는
라벨을 만들었고, `_fact_lines()` 가 그 라벨을 `[사실]` 블록에 실었으며, LLM 은 규칙대로
**그 「사실」을 그 근거와 함께** 옮겼을 뿐입니다. 프롬프트 규칙을 어긴 것이 아니라 **지킨
결과**입니다.

★**[§5-14](#5-14-기사의-배경절이-event-라벨이-된다-추출-파이프라인-measure) 와 같은 뿌리입니다** —
둘 다 「Event 라벨이 자기 근거 원문과 어긋난다」이고, 둘 다 `/ask` 안에는 잡을 지점이 없습니다.
★`is_risk=True`(「위험사건」)까지 같은 줄에 실려 부정 프레임이 한 번 더 강화됩니다.
...
| 완료 조건 | (ⓐ) 「삼성전자가 납품하는 기업」에서 `relations[]` 가 `SUPPLIES_TO` 를 위에 싣는다 (ⓑ) 「SK하이닉스 노조」 사건 선택이 회귀하지 않는다 (ⓒ) 앵커 유무가 **하나의 값**으로 판정되어 아래 단계가 읽을 수 있다 — 이 판정은 **①b(② Search 이후)의 산출물**이고 질의 파싱(①a)에서는 확정할 수 없다([설계서 §10](BizNode_Search_Layer_설계.md#10-ask-전체-flow--10단계)) (ⓓ) 잘라낸 관계 개수가 로그에 남는다 |
...
★단, **워크스페이스를 「재료 앵커」로도 쓸 것인가**는 아직 결정하지 않았습니다
([설계서 §3](BizNode_Search_Layer_설계.md#3-워크스페이스는-필터가-아니라-랭킹-문맥이다) `[DECIDE]`).
