...
마지막 갱신 **2026-08-27** · 브랜치 `yun-phase1`
...
### 5-28. 그래프 경로를 직접 보는 테스트가 라우트 위임 검사 하나뿐이다 `[TODO]`
...
★**Phase 1 에서 테스트를 늘리지 않았습니다.** 완료 기준이 「705 passed ·
  2 xfailed · 7 failed 유지」라 개수를 움직이지 않는 쪽을 골랐습니다. 어느
  쪽으로 갈지는 사람이 정할 일입니다 — ⓐ 13건을 그래프 경로로 옮긴다(기준선을
  잃는다) ⓑ 그래프용 테스트를 따로 더한다(같은 것을 두 벌 검사한다)
  ⓒ 대조 스크립트를 CI 에 건다(DB·임베딩이 필요하다).
