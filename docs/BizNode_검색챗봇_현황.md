...
| **고치지 않은 이유** | 규칙을 더 얹으려면 **실측이 먼저**다([규칙 5](BizNode_Search_설계.md#1-4-이-문서-전체를-관통하는-규칙-여섯)). 토큰 병합·구간 커버리지 중 무엇이 맞는지 재 본 적이 없다 |
...
1차 앵커 이름의 부분 문자열인 토큰을 빼서 `SK`·`하이닉스` 를 거릅니다. §5-15 를 전면
해결하지는 않습니다 — 합성 사명(「TSMC반도체홀딩스코리아」)은 그대로 남습니다.
