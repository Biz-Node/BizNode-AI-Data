...
**A-10. 별칭이 별도 노드면 같은 회사가 앵커 둘로 실린다** `[TODO]` (2026-09-05 · A-7 구현이 드러냄)
...
아닙니다. 뿌리는 두 노드가 **엣지 0건으로 남남인 데이터 결함**(노드 병합 누락)이고,
A-7 의 2차 앵커가 그것을 드러냈을 뿐입니다. 재료가 안 바뀌므로 급하지 않지만,
