...
| └ 비-Company 관계가 Ring 2 로 밀려 잘린다 | 🟡 **반만 해소** | 화이트리스트는 **A-11**(2026-09-06)이 닫았고 **링은 남았다** — 워크스페이스가 있으면 여전히 0/10 → [§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo) | — |
...
**그건 이 변경이 아니라 [§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo)
...
| 무엇 | 상태 |
|---|---|
| ★**계약 변경 3건 백엔드 통보** — `Event.company` 추가 · 앵커 없는 질문의 `match_type` `EXACT`→`SEMANTIC` · **`Anchor.label` 추가**(2026-09-10 · A-8. 기본값 `Company` · `required` 아님 · OpenAPI 대조에서 바뀐 스키마는 `Anchor` 하나이고 경로는 전부 동일) | 가이드에는 적었다([연동 가이드 §10](BizNode_백엔드_연동_가이드.md)) · **통보 안 함** |
...
| 2026-08-26 | **신규 결함 3건** — 부분 토큰 오탐([§5-15](#5-15-사명의-부분-토큰이-실존-기업으로-해소된다--해소-2026-09-10)) · 비-Company 엣지 재료 0([§5-16](#5-16-비-company-엣지-질의는-재료가-0-이-된다--해소-2026-08-26), **해소**) · Ring 2 잘림([§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo)) | §5-16 만 고쳤다. 나머지 둘은 **표시만** — 규칙을 얹으려면 실측이 먼저다. §5-15 는 `xfail(strict=True)` 회귀 케이스로 고정 |
