...
> 설계 근거는 [검색·챗봇 설계](BizNode_Search_설계.md) 와 [Agent 설계](BizNode_Agent_설계.md) 입니다.
...
마지막 갱신 **2026-09-13** · 브랜치 `yun`

> ★**이 문서는 검색과 Agent 두 계층을 함께 다룹니다.** 계층별로 가르지 않은
> 이유는 §12 의 2026-09-05 항목에 있습니다.
...
Global Event Search ─── 완료          2026-09-02 신설. §8-24
   ...
   └ 못 타던 질문 ────── 해소          2026-09-05. 그래프에 없는 사명이 원인. §6-0 A-2
앵커 확장 ───────────── 🟡 반만 해소   2026-09-10. Person·Organization·Product 를 대상으로
   └ 재료 경로 ──────── 완료          앵커 → 1홉 기업(Path B). Coverage 23 → 36 → 40/48. §6-0 A-8
   └ 토큰 관문 ──────── 완료          기관·제품의 일반명사 이름도 후보로 올린다(단계 5)
   └ 앵커 엣지 진입 ──── 완료          비-Company 앵커를 잇는 관계를 링 위로. 26 → 38/40. §5-17 ③
   └ 허브 절단 ──────── 🔴 미결        공정위 1홉이 76곳 — 상위 5로 자른 결과가 맞는지 못 잰다
   └ 다중 앵커 재료 ──── 완료          2026-09-13. 「A와 B」의 두 대상이 전부 재료 기업. §6-0 A-9
   └ 별칭 stub 병합 ──── 완료          2026-09-13. 현대차·네이버 등 21 stub 을 정본 노드로 접음. §6-0 A-10
...
**처음 오신 분은** [설계서 §0 오리엔테이션](BizNode_Search_설계.md#0-처음-오신-분께--10분-오리엔테이션)을
...
| └ 질문이 지목한 대상은 전부 재료 기업 (A-9) | ✅ **2026-09-13** | `with_named_anchors()` → [§6-0 A-9](#6-0-지금-기준--여기부터-읽으세요-2026-09-02) | 4 |
| └ 별칭 stub 이 정본과 갈려 한 회사가 앵커 둘로 실림 (A-10) | ✅ **2026-09-13** · 보류 2 | `batch/repair/company_merge.py` (데이터 수리 · 21건 병합) → [§6-0 A-10](#6-0-지금-기준--여기부터-읽으세요-2026-09-02) | 58 |
| └ 링 확장 임계값 · 앵커 기업 수 상한 | 🔴 `[DECIDE]` | 정렬 방식이라 임계값 없이 도는 중 → [§7-4](#7-4-2026-08-25-개정이-새로-연-decide-2건) | — |
| └ 비-Company 관계가 Ring 2 로 밀려 잘린다 | 🟡 **반만 해소** | 화이트리스트는 **A-11**(2026-09-06)이 닫았고 **링은 남았다** — 워크스페이스가 있으면 여전히 0/10 → [§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo--앵커-축은-해소-2026-09-12) | — |
...
| Agent Tool 연동 | ✅ **2026-08-28** (Phase 2) — ★도구 7종 · `agent ⇄ run_tools` 루프. **재료 범위는 서버가 고정한다**(A-12) | `app/tools/agent_tools.py` · `app/graph/nodes/agent_loop.py` | 28 |
...
결과는 [설계서 3부](BizNode_Search_설계.md#3부--챗봇-답변을-쓴다) 입니다.

| 무엇이 확정됐나 | 어디에 |
|---|---|
| `/ask` 는 Graph QA → Evidence-grounded RAG → Insight **3단이고 순서가 고정** | [설계서 §8](BizNode_Search_설계.md#8-ask-는-무엇인가--셋의-결합이고-순서가-정해져-있다) |
| 데이터 계층 책임 — Company/Relationship/Event/metadata/Evidence 가 각각 무엇을 대답하나 | [설계서 §9](BizNode_Search_설계.md#9-데이터-계층의-책임--무엇이-무엇을-대답하나) |
| 10단계 flow — 단계별 입력·출력·책임·**금지사항** | [설계서 §10](BizNode_Search_설계.md#10-ask-전체-flow--10단계) |
| Relationship 과 Event 는 **다른 파이프라인**이어야 한다 | [설계서 §11](BizNode_Search_설계.md#11-relationship-과-event-는-다른-문제다) |
| 사실 / 관측된 인과 / 계산된 전망 / Insight **4등급** | [설계서 §12](BizNode_Search_설계.md#12-사실과-추론의-경계--4등급) |
| claim **5종**과 타입별 검증 원천 | [설계서 §13](BizNode_Search_설계.md#13-claim-5종과-검증) |
| 앵커 없는 질의는 **Query Understanding 에서** 푼다 | [설계서 §14](BizNode_Search_설계.md#14-앵커-출처--무엇을-대상으로-답하는가) |
...
| 무엇 | 판정 | 어디에 |
|---|---|---|
| `/ask` 요청 계약 (`question` + `workspace_keys`) | **무변경 — 유지** | [설계서 §3](BizNode_Search_설계.md#ask-는-workspace_keys-를-받는다-2026-08-25-정정) |
| Search Layer (`search/` 전체) 의 `workspace_keys` 계약 | **무수정 — 유지** | [§6-4](#6-4-파일별-책임--최종-아키텍처에서-적절한가) |
| 워크스페이스 정책 (hard filter 아님 · ranking context) | **무변경** | [설계서 §3](BizNode_Search_설계.md#3-워크스페이스는-필터가-아니라-랭킹-문맥이다) |
| **A-3** Workspace 기업을 **material anchor** 로 쓴다 · **링(ring)** 순서 수집 | ✅ **확정** | [설계서 §3](#workspace-기업을-material-anchor-로-쓴다-2026-08-25-확정) |
| **B-2** 해소 실패는 워크스페이스로 폴백하지 않는다 — LLM 미호출 · 대안 제안 | ✅ **확정** | [설계서 §14-4](BizNode_Search_설계.md#14-4-해소-실패의-처리--200--재료-없음-2026-08-25-확정) |
| **C-3** `anchor_source` 로 답변 형태를 가른다 (query=서술 · workspace=목록) | ✅ **확정** | [설계서 §14-6](BizNode_Search_설계.md#14-6-답변-형태를-anchor_source-로-가른다-2026-08-25-확정) |
| **E** 식별은 `corp_code` → `norm_name`. `name` 은 표시·해석용 | ✅ **확정** | [설계서 §16](BizNode_Search_설계.md#16-워크스페이스-기업-식별--workspace_keys) |
| §14 의 ⓐ 행 **삭제** · **앵커 출처 3분법**(`anchor_source`) 도입 | ★신설 | [설계서 §14](BizNode_Search_설계.md#14-앵커-출처--무엇을-대상으로-답하는가) |
...
([설계서 §16](BizNode_Search_설계.md#16-워크스페이스-기업-식별--workspace_keys)).
...
| ★**`unresolved` 는 실패가 아닙니다** | `failed=false` · `sources=[]` · HTTP 200 입니다. 대상을 못 찾았을 때 **워크스페이스로 갈아타 답하지 않습니다** — 그게 조용한 오답이기 때문입니다([설계서 §14-4](BizNode_Search_설계.md#14-4-해소-실패의-처리--200--재료-없음-2026-08-25-확정)) |
...
([설계서 §3](BizNode_Search_설계.md#3-워크스페이스는-필터가-아니라-랭킹-문맥이다)). 여기서 깨지는 것은
...
([설계서 §14-7](BizNode_Search_설계.md#14-7-두-경우가-실제로-어떻게-나아지나)).
...
★stub 은 약하게 쓴다        norm_name key 는 corp_code key 보다 약한 prior
                            ([설계서 §16-1](BizNode_Search_설계.md#16-1-key-는-corp_code-또는-norm_name-이다))
```
...
([설계서 §12](BizNode_Search_설계.md#12-사실과-추론의-경계--4등급)).
...
그래서 [설계서 §14](BizNode_Search_설계.md#14-앵커-출처--무엇을-대상으로-답하는가) 의
...
([설계서 §14-6](BizNode_Search_설계.md#14-6-답변-형태를-anchor_source-로-가른다-2026-08-25-확정)).
재료도 「점수순으로 아무거나」가 아니라 **워크스페이스 기업을 앵커로 링 순서**로 모읍니다
([설계서 §3](BizNode_Search_설계.md#재료-수집-순서--링ring-으로-확장한다)).
...
의도별 우선순위냐는 아직 `[DECIDE]`([§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo--앵커-축은-해소-2026-09-12))이고
...
([설계서 §11](BizNode_Search_설계.md#11-relationship-과-event-는-다른-문제다)).
...
([설계서 §13](BizNode_Search_설계.md#13-claim-5종과-검증)).
...
([설계서 §14-5](BizNode_Search_설계.md#14-5-semantic-의-지위가-바뀐다-2026-08-25)).
...
#### ★첫 채점 표본 둘 — 사람이 읽고 DB 로 대조했다 (2026-09-13)

A-9 를 손으로 돌려 본 김에 답변 둘을 근거·사건 노드와 **전건 대조**했습니다.
자동 채점이 아니라 사람이 읽은 첫 기록입니다.

```text
① 「LG전자의 최근 리스크가 있어?」
   답변 주장 3건(적자 전환 01-13 · 사무직 노조 집회 04-30 · MS사업본부 영업손실 12-01)
   → 사건 노드 3/3 · 근거 원문 3/3 · 날짜 3/3 일치. 지어낸 것 0
   ★재료 10건 중 3건만 골랐다 — 「최근」이라 2025-12~2026-04 를 고른 건 맞고,
     01-09 「4Q 영업손실」은 01-13 과 같은 사안이라 빠진 것도 자연스럽다.
     2025-08 튀르키예 과징금(규제수사)을 「최근」에서 뺀 것이 맞는지는 취향 문제

② 「LG전자와 삼성전자 중 누가 더 위험해?」   ★A-9 뒤
   답변 항목 7건(LG 영업손실·갑질·제품결함 / 삼성 압수수색·기술유출·특허소송·방사선 피폭)
   → 사건 노드 7/7 · 근거 원문 7/7 · 귀속 오류 0. A-9 전에는 삼성 쪽 4건이 나올 수 없었다
   🟡 **「누가 더」에 판정을 안 낸다** — 양쪽을 나열하고 끝난다
```

★**②의 판정 부재는 회피가 아니라 프롬프트 규칙에 충실한 결과**입니다
(`app/llm/prompt.py` 확인).

```text
최상위 원칙      「서로 다른 근거를 임의로 결합하여 새로운 사실을 만들지 않는다」
                 → 두 기업의 사건을 합쳐 우열을 내는 것이 곧 근거 결합
[인사이트 생성]  「부정적 영향 … 은 [사실]에 명시적으로 계산된 결과가 있을 때만」
                 → [사실] 블록에 기업 단위 집계가 **없다** (fact_lines 는 사건별
                   이름·유형·보도일·위험여부·role 만 싣는다)
[최종 검증] 1·4  「각 문장이 [사실]·[근거]로 뒷받침되는가」
                 → "삼성이 더 위험하다"는 어느 근거로도 뒷받침이 안 된다
```

비교 질의를 어떻게 답할지는 **`[DECIDE]`** 입니다 — 셋 중 하나.

| 안 | 무엇 | 비용·위험 |
|---|---|---|
| 그대로 | "각각 확인된다"로 나열 | 코드 0. 물은 것(우열)에는 답하지 않는 셈 |
| [사실]에 기업별 집계 | `삼성전자: 위험사건 10건(최근 12개월 4건 · 유형 6종)` 한 줄을 **계산해** 싣는다 → LLM 이 인용하며 비교 가능 | 「사건 수 = 위험도」라는 **지표 정의가 먼저**다(A-4 와 같은 종류). 사건 수는 뉴스 수집량(대기업 편향)을 그대로 반영한다 |
| 프롬프트에 비교 규칙 | 「비교 질문이면 두 대상을 나란히 놓고 최근성·유형 차이를 서술하되 우열은 근거가 있을 때만」 | 근거 정책 무변경. 판정은 여전히 없지만 대조는 좋아진다 |

★추천은 **셋째를 먼저, 둘째는 지표 정의가 선 뒤**입니다. 둘째는 「위험도」를 숫자로
정의하는 결정이 선행이라 A-4 와 묶여야 합니다.
...
> ([설계서 §13-3](BizNode_Search_설계.md#13-3-낱말-겹침lexical-grounding의-제자리)),
...
[설계서 §12](BizNode_Search_설계.md#12-사실과-추론의-경계--4등급) 의 「**LLM 은 블록 이름을 믿는다**」는
...
[설계서 §10](BizNode_Search_설계.md#10-ask-전체-flow--10단계)) — 답변을 쓰기 전에 두 재료를 대조할
...
[설계서 §13-3](BizNode_Search_설계.md#13-3-낱말-겹침lexical-grounding의-제자리) 이 「의미가 맞는가는
...
([설계서 §12](BizNode_Search_설계.md#12-사실과-추론의-경계--4등급) 4등급 **② 관측된 인과** —
⚠ 프롬프트 규칙뿐인 등급입니다).
...
| [설계서 §9-1](BizNode_Search_설계.md#9-1-원칙-넷과-실제-구현-대조) 「원문은 우리가 요약하지 않은 것」 | 제목은 **우리가 쓴 요약이 아니라 기사 자신의 제목**이다. 다만 `[근거]` 블록 한 덩어리 안에 **추출된 본문 문장**과 **기사 전체의 제목**이라는 서로 다른 단위가 섞여 있고 프롬프트가 둘을 가르지 않는다. 이 상태를 어떻게 볼지는 **결정 사항이라 여기서 결론내지 않는다** |

### 5-13. ★앵커가 있는 질의에서도 재료가 질문 프레임에 굴절된다 `[TODO]`

**신규 발견 (2026-08-23 · 같은 질의).**

[설계서 §14](BizNode_Search_설계.md#14-앵커-출처--무엇을-대상으로-답하는가) 는 이 문제를 **앵커 없는 질의**로만 다뤘습니다.
...
「**근거가 없으면 없다고만 하고 끝낸다**」([설계서 §15-2](BizNode_Search_설계.md#15-2-답변-규약--지키게-하려는-것)
...
| 무엇 | 왜 무결한가 |
|---|---|
| 검색 · scope | SK하이닉스는 이 소송의 **실제 원고**이므로 `HAS_EVENT.role=subject` 가 맞다. 남의 기사가 섞인 것도 아니다 — 기업별 evidence scope([설계서 §6-2](BizNode_Search_설계.md#6-2-근거는-그-기업의-엣지에서-꺼낸다))는 제 일을 했다 |
...
★**flow 에 ⑥.5 Material Consistency 를 신설했습니다**(2026-08-23 ·
[설계서 §10](BizNode_Search_설계.md#10-ask-전체-flow--10단계)) — ③ 이 만든 라벨과 ⑥ 이 만든 근거
...
### 5-15. ~~사명의 **부분 토큰**이 실존 기업으로 해소된다~~ — **해소 (2026-09-10)**

**발견 (2026-08-25 · 단계 3 스모크).** 실재하지 않는 이름을 물었는데 그 안에 든
조각이 실존 기업명이라 앵커가 붙었습니다.

```text
질의   「TSMC반도체홀딩스코리아는 어떤가?」          ← 실재하지 않는 이름
  Kiwi 토큰                → 'TSMC'(SL) · '코리아'    ★합성 사명을 쪼갠다
  find_by_names(['TSMC' …]) → 실존 TSMC 에 매칭
  → anchor_source=query · key=tsmc               🔴 사용자가 묻지 않은 기업이 대상이 된다
```

★**전보다 나빴습니다.** 전에는 SEMANTIC 으로 가서 최소한 헤지가 걸렸는데,
`anchor_source=query` 라 **헤지 없이** 나갔습니다.

#### 실측 — 규칙 다섯을 같은 코퍼스에 재고 골랐다 (2026-09-10)

「토큰 병합·구간 커버리지 중 무엇이 맞는지 재 본 적이 없다」가 이 항목이 열려 있던
이유였습니다. **질의 758건**으로 쟀습니다 — 그래프 Company 3,451곳에서 뽑은 실존
사명 58곳(대표 18 + 무작위 40) × 질의틀 7 + 합성 사명 348 + 허구 4.

★질의틀은 **띄어쓰기와 조사를 일부러 갈랐습니다** — 「{n} 주가 알려줘」와
「{n}주가 알려줘」가 다른 결과를 내는 것이 이 문제의 본체이기 때문입니다.

```text
                        A 실존 정답률   A미탐   A오답    B 합성 오탐률
  R0 현행                   62.3%    124     29      35.6%
  R1 병합 스팬               76.8%     80     14       3.4%
  R2 병합 + 조각 폴백          82.0%     57     16      35.6%   ← 오탐이 그대로다
  R3 구간 커버리지             57.1%    160     14       3.4%   ← 조사 붙은 사명이 죽는다
  R4 ★고유명사를 품은 스팬                                      ← 채택
```

★**R2 가 답이 아닌 이유를 재서 알았습니다.** 조각으로 되돌리는 폴백을 넣으면 A 정답률은
가장 높지만 **오탐이 하나도 안 줄어듭니다** — 그게 §5-15 그 자체입니다.

★**R3(구간 커버리지)는 조사에 죽습니다.** 「SK하이닉스에」에서 `SK`·`하이닉스` 중
어느 것도 덩어리 전체를 못 덮어 **둘 다 버려집니다.**

#### ★R1 을 그대로 쓰면 앵커리스가 통째로 죽는다

병합 스팬을 `_name_tokens` 에 그대로 얹었더니 **앵커 없는 질의 12건이 12건 다**
`unresolved` 로 뒤집혔습니다. Global Event Search 가 통째로 막힙니다.

```text
「최근 반도체 업계 주요 이슈가 뭐야?」   anchorless → unresolved[named=최근]
「메모리 가격 담합 관련 소식」          anchorless → unresolved[named=가격]
「납품 단가 압박」                   anchorless → unresolved[named=납품]
```

덩어리에는 「최근」·「업계」·「가격」 같은 **일반명사도 담기기** 때문입니다.
`_NAME_TAGS`(NNP·SL)가 하던 일이 바로 그것을 거르는 것이었습니다.

#### 해소 — 덩어리를 만들되 **고유명사를 품은 것만** 남긴다
...
★**앵커리스가 보존되는 근거가 여기 있습니다.** 덩어리는 고유명사를 품어야 남으므로
**「고유명사가 없다」가 전과 같은 뜻**입니다 — 질의 774건 전수에서 새 규칙과 옛 규칙의
**공집합 동치가 한 건도 어긋나지 않았습니다.**

★**붙이는 규칙은 한 벌입니다.** `token_overlap._merge_adjacent()` 가 이미 이 문제를
위해 있었는데(그 함수의 머리말이 「Kiwi 가 사명을 쪼갠다」다) **글자만** 돌려주어
품사로 거를 수가 없었습니다. `merged_spans()` 로 품사를 함께 내보내고
`_merge_adjacent()` 를 그 위에 다시 세웠습니다 — `sentence_tokens` 와
`batch/audit/grounding.py` 의 `_GROUND_THRESHOLD = 0.34` 가 그 동작에 묶여 있어
**바뀌면 안 됩니다.** 시험이 두 출력을 마주 세워 묶어 둡니다.

```text
①b 2단 사정권 539건            오탐 121 → 12       A 정답 111 → 110 (미탐 +1)
앵커리스 12건                  12 / 12 유지        뒤집힘 0
「대상을 명시했으나 못 찾음」 4건    전부 그대로        ★TSMC 합성만 query → unresolved
전체 시험                      1,143 passed        실패 0 · 1 xfailed
```

#### ★곁가지 — A-7 의 2차 앵커가 틀려 있었다

같은 실측이 [A-7](#a-결함--만든-기능의-값어치를-지금-깎고-있는-것) 의 숨은 오답을
드러냈습니다. **비교 질의의 2차 앵커가 엉뚱한 회사**였습니다.

```text
「삼성전자와 SK하이닉스는 무슨 관계야?」   2차 앵커 = SK(SK주식회사 00144155)  → SK하이닉스
「SK하이닉스와 한미반도체 거래 관계」      1차 앵커 = SK                     → SK하이닉스
「한미반도체와 SK하이닉스 관계 알려줘」    2차 앵커 = SK                     → SK하이닉스
```

`_second_anchor()` 의 부분 문자열 차단은 **1차 앵커 이름의 조각**만 봅니다 —
**2차 쪽이 쪼개진 것은 못 막았습니다.** 덩어리로 보면 애초에 안 쪼개집니다.

#### 남은 것 — 붙여 쓴 「사명 + 일반명사」

```text
「삼성전자주가 알려줘」   덩어리 '삼성전자주가' → 그래프에 없다 → unresolved
```

합성 사명(「삼성전자글로벌인베스트먼트」)과 **구별할 신호가 없습니다.** 잔여 길이로
가르는 것은 실측 없는 임계값이라 얹지 않았습니다. 다만 이 형태 58건 중
**26건은 ①b 1단(`corp_code`)이 흡수**합니다 — `AnchorExtractor` 가 붙여 쓴 것도
뽑아내기 때문입니다.

★**랭킹 기준선은 무영향입니다.** `batch/audit/ranking_baseline.py` 는
`query_understanding` 을 **한 번도 부르지 않고**(import 0) 기업 key 를 직접 받습니다 —
OpenAI 임베딩을 태워 다시 뜨지 않았습니다.

★[§8-5](#8-5-anchor_source-판정-신호-2026-08-25) 가 「새 규칙으로 못 고친다」고 적은
2건은 **그대로입니다** — 「존재하지않는기업」(형태소 분해)은 여전히 `anchorless` 이고,
「대상」(동음이의)은 §4-5 입니다.
...
| 지킨 것 | 왜 |
|---|---|
| `companies` 는 **여전히 Company 만** 담는다 | Person·Organization·Event 를 넣으면 조용히 빈 결과가 되어 「사건이 없다」로 잘못 읽힌다([설계서 §9](BizNode_Search_설계.md#9-데이터-계층의-책임--무엇이-무엇을-대답하나)) |
...
### 5-17. ★비-Company 관계가 Ring 2 로 밀려 상한에서 잘린다 `[TODO]` — 🟡 **앵커 축은 해소 (2026-09-12)**
...
#### ★정정 — 원인이 둘이었고, 그중 하나는 링이 아니었다 (2026-09-06)

위 「미달성」 두 칸의 사정이 서로 달랐습니다.

```text
「삼성전자를 규제한 기관」  REGULATES 는 화이트리스트 **안**에 있었다  → 순수한 링 문제
「삼성전자 임원」          IS_EXECUTIVE_OF 는 화이트리스트 **밖**이었다 → 링과 무관하게 0건
```

실측(2026-09-06) — 워크스페이스를 안 주면 모든 관계가 Ring 3 한 칸에 모이므로 **링이
아예 작동하지 않는데도** 「삼성전자 임원」은 `OWNS_STAKE_IN ×10` 이었습니다. 위에 적힌
「`relations_of('삼성전자')` 안에 Person 끝 21건이 실재한다」는 맞습니다 — 다만 그 21건은
`OWNS_STAKE_IN`·`SUES` 로 들어온 것이고 **임원은 한 건도 없었습니다.**

[A-11](#6-0-지금-기준--여기부터-읽으세요-2026-09-02) 이 화이트리스트 쪽을 닫았습니다
(Person 끝 21 → 43 · Product 끝 10 → 154). **링 쪽은 그대로 남습니다.**

```text
"삼성전자 임원이 누구야?"   워크스페이스 없음   0/10 → **10/10**   A-11 이 닫음
                            워크스페이스 있음   0/10 →   0/10     ★여기가 §5-17
                            링 분포 {0:3, 1:535, 2:281} — Ring 1 이 상한 10 을 다 먹는다
```

★**후보 ② 를 고를 근거가 하나 생겼습니다** — A-7 이 `_RING_BOTH_ANCHOR = -1` 로 「질문이
명시적으로 지목한 두 대상」을 워크스페이스 링 위에 뒀습니다. 「질문이 명시적으로 지목한
관계 유형」도 같은 종류의 신호입니다. 다만 `_SHALLOW_KEYWORDS` 가 `[미확정/저신뢰]` 라
**오탐 비용을 먼저 재야 합니다** — 「개발」이 스쳐 지나간 질의가 `DEVELOPS` 를 워크스페이스
관계 위로 올리는 것이 얼마나 잦은지가 아직 0 측정입니다.
...
| ★안 한 것 | 링별 quota · 의도별 관계 우선순위 **둘 다 결정하지 않았다** — 실측 없이 숫자를 정하지 않는다([규칙 5](BizNode_Search_설계.md#1-4-이-문서-전체를-관통하는-규칙-여섯)) |
...
#### ③ 앵커 자신을 잇는 관계는 링 위로 — 비-Company 앵커에서만 (2026-09-12)

후보 ①②와 **다른 지렛대**입니다. 질의 의도(키워드)에 기대지 않고 **앵커**만 봅니다 —
A-7 이 `_RING_BOTH_ANCHOR = -1` 로 「앵커 ↔ 앵커」를 올린 것과 같은 종류의 신호라
`_SHALLOW_KEYWORDS` 오탐 측정이라는 선행 조건이 **없습니다.**

★**왜 지금 생겼나** — A-8 이 연 Person·Organization·Product 앵커에서는 재료 기업이
앵커 자신이 아니라 **1홉 이웃**입니다. 그래서 「앵커 ↔ 재료 기업」 엣지가 곧 질문이 물은
것인데, 상대가 비-Company 라 Ring 2 로 밀려 잘렸습니다.

```text
「정의선과 관련된 기업은 어디야?」
  재료 기업  현대모비스 · 현대오토에버 · 현대자동차 · 보스턴다이내믹스 · 기아   ✅
  관계 50건  ★정의선 → … 엣지 0건                                        🔴
             (재료 기업 5곳의 **다른** 관계 50건이 들어왔다)
```

실측(2026-09-11 · A-8 검토) — Coverage 를 통과한 40건 중 **14건**이 이랬습니다. Company
앵커는 11/11 있었고(앵커가 곧 재료 기업이라 모든 관계가 앵커를 잇는다) 비-Company 만
Person 4/9 · Organization 5/9 · Product 6/11 이었습니다.

**고친 것 둘**

```text
① ring_of()      한쪽 끝이 **비-Company 앵커**면 _RING_BOTH_ANCHOR(-1)
                 ★Company 앵커에는 안 건다 — 앵커가 곧 재료 기업이라 모든 관계가
                   한쪽 끝에 앵커를 두는데, 그걸 전부 올리면 워크스페이스 링이 통째로
                   죽는다. A-7 의 경계(「한쪽만 앵커면 이웃일 뿐」)가 Company 에서 그대로다
② _REL_Q         상대 끝 key 를 _GRAPH_Q 와 같은 식으로 — Person 끝이 person_key 를 싣는다
                 ★전에는 이름을 실었다. 저장소 안에서 그 쿼리 하나만 그랬고, 같은
                   relation_row() 가 먹이는 쿼리에 따라 Person key 를 다르게 냈다.
                   앵커 key 는 person_key 라 링 대조가 **원리적으로 불가능**했다
```

**실측 — 같은 48건**

```text
Company 앵커 12건        관계 목록 edge_id 순서까지 **12/12 동일**   ← 경계 보존
앵커 엣지 재료 진입        26/40 → **38/40**
   Person                4/9 → 9/9
   Organization          5/9 → 7/9
   Product               6/11 → 11/11
바뀐 12건                전부 0 → N · 후퇴 0
relations_of 지문 10곳   edge_id · Company 끝 · Organization·Product 끝 **전부 동일** ·
                        Person 끝만 이름 → person_key (43·14·13·52·10·13·13 건)
```

★**남은 2건은 이 항목 밖입니다** — 「미국 상무부」가 **「미국」(Organization)** 으로 붙었습니다.
원문에 공백이 있어 덩어리가 안 붙고(`merged_spans` 의 계약), '미국' 만 고유명사 관문을
통과합니다. §5-15 잔여(Brity Works 와 같은 종류)이고 **사실은 오답**입니다 — Coverage 는
재료가 있고 적합도가 높아 O 로 셌지만, 물은 것과 다른 대상입니다.

★**원래의 §5-17 은 그대로입니다.** 「삼성전자를 규제한 기관」(Company 앵커 + 워크스페이스)
에서 Organization 이 Ring 2 로 밀리는 것은 ③ 이 건드리지 않습니다 — Company 앵커에서는
규칙이 성립하지 않기 때문입니다. 그쪽은 여전히 후보 ①②(quota · 의도) 중 하나가 필요하고,
② 는 `_SHALLOW_KEYWORDS` 오탐 측정이 선행입니다.

### 5-18. ★claim 5종 어디에도 안 걸리는 주장이 있다 — 6번째 유형 신설 (2026-08-26)

**신규 발견 (2026-08-26 · 사례 재현 분석).** [설계서 §13-1](BizNode_Search_설계.md#13-1-답변-문장의-원천은-다섯-가지다)
...
| ★LLM judge 안 씀 | 어휘 대조 + 집합 확인뿐이다([설계서 §13-4](BizNode_Search_설계.md#13-4-배치-위치-셋)) |
...
★**여전히 표기만 하고 판정하지 않습니다**([설계서 §12](BizNode_Search_설계.md#12-사실과-추론의-경계--4등급)).
...
**A-1. ★★워크스페이스가 랭킹에 전혀 안 쓰인다** ✅ **2026-09-05 해소**
...
#### 해소 — 티어가 아니라 **몫**이었다 (2026-09-05)

★**「어느 자리에 얹을까」가 틀린 질문이었습니다.** 위 ⚠ 가 지목한 세 자리에 사전식
티어를 각각 얹어 재 봤더니 **셋이 같은 답**을 냈습니다.

```text
전역 후보 933행 · 워크스페이스(삼성·하이닉스) 188행 20%

                    최근 주요 투자   요즘 반도체   최근 리스크
  지금                 2/10        2/10       0/10
  규칙 티어 위          10/10       10/10      10/10
  규칙 아래·위험 위      10/10       10/10      10/10
  유사도 바로 위        10/10       10/10      10/10
      ★상위 3건까지 글자가 같다 — 세 안이 구별되지 않는다
```

워크스페이스가 후보의 20% 라 **이진 티어는 어느 높이에서든 상한을 채우고 남습니다.**
10/10 은 최종 설계 §19-3 이 금지한 hard filter 와 관측상 구별되지 않습니다 — 사용자가
워크스페이스 밖 사건을 영영 못 봅니다.

★**기각한 안 여섯** — 전부 실측으로 닫았습니다.

| 안 | 기각 사유 |
|---|---|
| 티어를 규칙 위 · 규칙 아래 · 유사도 위 | 셋 다 10/10 포화. 자리와 무관하다 |
| 워크스페이스 **상한**(cap) | 천장이라 방향이 반대다. 2/10 인 것을 3 으로 막아도 무변화 |
| `IMPACTS` 를 얹은 다단 티어 | 워크스페이스에 파급되는 사건이 **3.1%**(29행)뿐이고, 주체 사건만으로 이미 포화다 |
| **1홉 이웃** 티어(`_WS_OUTSIDE_COMPANY` 이식) | 워크스페이스가 무엇이든 33% 가 걸린다. 현대차·기아 워크스페이스의 T2 310행 중 **128행이 삼성전자**다 — 삼성전자가 사실상 모든 워크스페이스의 1홉 이웃이라 신호가 아니라 잡음이다 |
| 라운드로빈 몫(정렬 **전체**에서) | 규칙 티어를 깬다. 「최근 주요 투자 이벤트」에 워크스페이스 **파업 3건**이 들어왔다(그 워크스페이스에 사업확장이 2건뿐이라 모자란 몫을 노무가 채웠다) |
| 하한을 **전 질의**에 적용 | 「최근 파업」에서 담아 둔 회사의 파업이 통째로 죽는다. 하한이 「전역 상위 10만큼 유사할 것」인데 그 상위 10 이 이미 그 유형의 최상위라 **순환**이다 |

#### 채택 — 몫을 티어 **안에서** 나누고, 주제 신호가 없을 때만 하한을 건다

선례가 있습니다 — `llm/prompt.select_propagation()` 이 「앞에서부터 자르면 첫 사건이
예산을 통째로 먹는다」를 **라운드로빈**으로 풀었습니다(§5-20). 증상이 같으니 처방도 같습니다.

```text
evidence_selector.select(…, workspace_keys=frozenset())     ← 신설. 비면 지금과 동일

 ① 기존 사전식 정렬 7줄 — 한 줄도 안 고쳤다
 ② workspace_keys 가 비었거나 sims 가 비면 여기서 끝
 ③ 티어 서명 = (규칙, 위험, 최근창)     워크스페이스보다 **위**에 있는 키만
 ④ 규칙도 위험도 없을 때만 하한 = 전역 상위 limit 건의 최저 유사도 덩어리
 ⑤ 서명이 같은 덩어리 **안에서만** 워크스페이스:전역 라운드로빈
 ⑥ 출력 순서는 ① 의 정렬 그대로 — 몫만 정한다
```

★**새 상수가 하나도 없습니다.** 몫은 라운드로빈이 정하고(모자라면 그만큼만), 하한은
전역 컷에서 나옵니다. 질의마다 유사도 대역이 달라(0.1~0.2 대 0.3~0.4) **절대값 상수로는
애초에 못 씁니다.** §9 의 잠정치가 늘지 않았습니다.

★**③ 이 요점입니다.** 몫이 티어를 넘으면 질문이 부른 것을 밀어냅니다. 서명 안에 가두면
워크스페이스에 그 유형이 모자랄 때 **몫을 안 채우고 전역에 돌려줍니다.**

★**④ 는 반대 방향의 실패를 막습니다.** 규칙도 위험도 없는 질의에서는 유사도가 주제를
읽는 **유일한 신호**라, 몫이 그것을 덮으면 「반도체 업계」를 물었는데 담아 둔 자동차
회사의 파업이 들어옵니다. 규칙이 있으면 걸지 않습니다 — 걸면 위 표의 마지막 줄이 됩니다.

★**`sims` 가 비면 몫을 강제하지 않습니다.** 임베딩이 죽었거나 의도가 없어 관련도를 못
재는 상태인데, 그때 자리를 떼어 주면 근거 없이 순위를 흔듭니다([§5-27](#5-27-임베딩이-실패하면-사건-정렬이-조용히-degrade-된다-todo) 과 같은 자리입니다).

#### 실측 — 워크스페이스 3종 × 질의 6종

```text
                            없음     삼성·하이닉스    삼성 1곳    현대차·기아
  최근 주요 투자 이벤트         기준      2→5/10      1→5/10    0→2/10
  최근 리스크                 기준      0→5/10      0→5/10    1→5/10
  최근 파업이나 노사 문제        기준      1→5/10      1→5/10    1→5/10
  요즘 반도체 업계             기준      2→3/10      1→2/10    0→0/10
      ★규칙 위반 0건 · 위험 위반 0건 — 18개 조합 전부
```

★**현대차·기아 + 「투자」가 2/10 에서 멈추는 것이 ③ 이 도는 증거**입니다. 그 워크스페이스에
사업확장이 2건뿐이라 몫 5를 못 채우고 남은 8칸을 전역이 가져갑니다. 같은 워크스페이스가
「최근 파업」에서는 5/10 을 정당하게 채웁니다(노무 8건 보유).

★**「요즘 반도체 업계」 + 현대차·기아가 0/10 인 것이 ④ 가 도는 증거**입니다. 하한이 없으면
현대차 파업 5건이 반도체 질문에 들어옵니다.

★**남은 흠 하나** — 「요즘 자동차 업계」에서 삼성 워크스페이스가 0→2 로 늘며 「삼성전자
자사주 소각」이 들어옵니다. 덩어리 단위 하한이 거칠어 통과했습니다. 18개 조합 중 이 한 자리입니다.

#### 검증

```text
단위 시험    11건 신설 — 불변식 2 · 몫 3 · 티어 경계 2 · 하한 3 · sims 없음 1
배선        실 DB 4질의 — 위 실측표와 정확히 일치
불변식      워크스페이스를 안 주면 **고치기 전 알고리즘과 event_id 목록이 글자까지 같다**
기준선      ranking_baseline --compare 18 케이스 중 17 무변화
전체 시험    1,098 passed · 2 xfailed · 실패 0
```

★**기준선에서 변한 1건은 이 변경이 아닙니다** — 「최근 주요 투자 이벤트」에서 판촉 3건이
빠지고 진짜 투자 3건이 들어온 것으로, **A-3 의 라벨 수정**입니다(스냅샷이 09-02 라 그 뒤의
데이터 변경이 잡혔습니다). `_measure_anchorless` 가 `workspace_keys` 없이 부르므로 배분
계층에 아예 들어가지 않고, 고치기 전 정렬을 재현해 대조한 결과가 위 불변식 줄입니다.

★**key 공간 전제 하나를 물려받습니다** — `workspace_service.names_of()` 가 요청 key 를
그대로 키로 씁니다. 계약상 `workspace_keys` 는 `coalesce(corp_code, norm_name)` 과 같은
공간이라 맞지만, 이 전제는 [§7-4](#7-4-2026-08-25-개정이-새로-연-decide-2건) 의 열린
`[DECIDE]` 입니다. 기존 `result_ranker.workspace_priority()` 와 **같은 key 공간**을 쓰도록
배선해 새 위험을 만들지 않았습니다.
...
**A-3. 사건 라벨에 마케팅 이벤트가 섞여 있다** ✅ **2026-09-05 해소**
...
#### 해소 — 분류 프롬프트가 「이벤트」의 두 뜻을 안 갈랐다 (2026-09-05)

라벨을 붙이는 곳은 `batch/repair/event_types.py` 이고, 그 프롬프트의 정의는
「사업확장 : 증설·착공·신공장·양산 개시·수주·투자 결정」입니다. 판촉은 **그 어느
것도 아닌데** LLM 이 「신규 계좌 개설」·「신규 고객」을 사업 성장으로 읽었습니다.
한국어 「이벤트」가 **사건**과 **판촉 행사** 둘 다 뜻하는 것이 뿌리입니다.

★**데이터만 고치면 재발합니다** — 새 사건은 `event_type` 이 비어 들어오고 같은
프롬프트가 다시 분류합니다. 그래서 이 파일의 관례대로(「실측된 오분류」를 예시로
박아 두는 방식) 판촉 반례를 넣고, 그 사건들만 다시 물었습니다.

```text
--event-id 신설   이미 분류된 것 하나만 다시 묻는다
                 (--full 은 1,000건을 다시 불러 멀쩡한 라벨까지 흔든다)
```

★**대조군으로 과잉 적용을 확인했습니다.** 진짜 사업확장 12건을 옛/새 프롬프트로
각각 돌려 대조했더니 **바뀐 것 0건**입니다. 한 건(「장기공급계약 관행 변화」)이
달라 보였으나 두 프롬프트로 4회씩 다시 돌리니 **양쪽 다 「기타」로 일치** — LLM
흔들림이었습니다.

```text
「최근 주요 투자 이벤트가 뭐야?」 상위 10건
  전   신규 계좌 개설 이벤트 · 100만명 돌파 기념 포인트 적립 이벤트 ×2   판촉 3자리
  후   D램 생산 투자 · 유일로보틱스 북미 시장 공략 · 한일 제주 스타트업 펀드
       ★판촉 0건 — 열 자리가 전부 진짜 투자 사건
```

★판촉 낱말(이벤트·적립·쿠폰·경품·추첨·기념·할인·프로모·사은·증정·당첨·응모)이 든
사건을 전부 훑어 **4건이 남았고 전부 `기타`** 입니다.

> ★**곁가지 발견** — 지금 분류기는 저장된 라벨과 어긋나는 사건이 있습니다.
> 「장기공급계약 관행 변화」는 저장 `사업확장` 인데 8회 전부 `기타` 가 나왔습니다.
> `--full` 을 돌리면 **이런 것들이 함께 움직입니다** — 돌리기 전에 규모를 재야
> 합니다.
...
**A-6. ★`/retrieve` 가 앵커와 무관한 기업으로 재료를 채운다** ✅ **2026-09-05 해소**
...
`/retrieve` 에 남아 있는 셈입니다.

#### 해소 — 판정은 이미 공유하고 있었다 (2026-09-05)

★**두 경로가 다르게 판단한 게 아닙니다.** `hits_reflect_the_anchor()` 를 **이미 공유**
하고 있었고, `/retrieve` 만 그 답을 안 썼습니다.

```text
retrieve_service.py:425   use_hits=True                          ← 고정
material.py:183           use_hits=hits_reflect_the_anchor(...)   ← 판정값
```

같은 분기가 **세 벌**이었습니다 — `_assemble` · `plan_material` · `retrieve_for_ask`.
셋째는 `/ask` 가 LangGraph 로 넘어가며 **운영에서 아무도 안 부르는 죽은 입구**가 됐는데
테스트 32곳이 그리로 살아 있는 로직을 검사하고 있었습니다.

정리 순서는 **합치기 → 지우기** 였습니다(반대로 하면 그 32곳이 갈 곳을 잃습니다):

```text
① material_companies() 로 선정을 한 곳에 모은다   — 두 입구가 이것 하나를 부른다
② retrieve_for_ask() 를 지운다                  — 이제 retrieve() 와 같다
③ 시험 32곳을 retrieve() 로 옮긴다
   「unresolved 면 재료를 안 만든다」는 그래프가 이미 본다
     tests/graph/test_conditional_edges.py::test_unresolved_anchor_routes_to_halt
```

★**실측 — 두 입구가 다섯 질문에서 전부 같은 재료를 냅니다.**

```text
                              전 /retrieve                       후 (양쪽)
삼성전자 최근 리스크            00126380                           00126380     무변경
이 회사 … (기업 상세 화면)      삼성에스디에스·원익홀딩스·리노공업 …   00126380     ★고쳐짐
TSMC 최근 리스크               LX세미콘·코미코·티씨케이 …           tsmc         ★고쳐짐
최근 주요 투자 이벤트           기업 9곳                            기업 9곳      무변경
요즘 반도체 업계               기업 8곳                            기업 8곳      무변경
```

★`SEMANTIC` 은 **죽지 않았습니다.** 설계서 §14-5 가 `/retrieve` 무변경을 정한 이유가
「`SearchMode.SEMANTIC` 을 enum 에서 지우지 않는다」였는데, `match_type` 은 그대로
`SEMANTIC` 으로 나갑니다. 바뀐 것은 「의미 유사 기업을 **재료로도 쓰나**」뿐입니다.

★**백엔드 통보 필요** — `/retrieve` 의 `companies`·`events`·`relations`·`evidence` 가
위 두 경우에서 바뀝니다. §6-0 E 의 미통보 2건과 함께 나가면 됩니다.

**A-7. 두 기업을 물으면 한쪽이 사라진다** ✅ **2026-09-05 해소**
...
#### 해소 — 뿌리는 검색 계층인데, 거기 안 건드리고 됐다 (2026-09-05)

★**측정이 원인을 옮겼습니다.** `_query()` 가 하나만 담는 것은 **증상**이고, 뿌리는
`resolved_entities` 가 **애초에 한 개만 온다**는 것이었습니다.

```text
「삼성전자와 SK하이닉스는 무슨 관계야?」
  토큰               ['삼성전자', 'SK', '하이닉스']     ★Kiwi 가 사명을 쪼갠다
  resolved_entities  [('SK하이닉스', 1.0)]            ★하나뿐이다
```

`AnchorExtractor` 가 앵커를 하나만 뽑는 것이 뿌리인데, `decide_anchor()` 는 이미 질문
토큰을 들고 있어 **`search/` 를 안 건드리고** 2차 앵커를 세울 수 있었습니다.

★**증상이 하나가 아니었습니다.** 앵커를 둘로 만들어도 그것만으로는 안 됩니다 —
질문이 물은 **두 기업 사이의 엣지**가 그래프에 있는데도 재료에 안 들어옵니다.

```text
네 쌍 전부 그래프에는 직접 엣지가 있다        재료에는 4/4 가 0건
  삼성전자 ↔ SK하이닉스   3건 (공급·경쟁·소송)
  SK하이닉스 ↔ 한미반도체  4건 (TC본더 독점공급·공동개발·경쟁)
  LG전자 ↔ 삼성전자      1건 · 삼성전자 ↔ 현대차  2건
★앵커 기업의 관계가 225~566건인데 상한 10 이라 점수순에서 밀린다
```

그래서 `ring_of` 에 **앵커 축**을 더했습니다. 질문이 **명시적으로 지목한** 두 대상
사이의 관계가 곧 질문이 물은 것이므로 워크스페이스 링보다 위입니다 — 워크스페이스는
「관심 영역」이고 앵커는 「대상」입니다.

★**A-1 과 달리 티어가 맞습니다.** A-1 에서 티어가 실패한 이유는 워크스페이스가 후보의
20% 라 포화됐기 때문인데, **앵커 쌍 엣지는 566건 중 3건**이라 포화 위험이 없습니다.

★**부분 토큰 오탐을 막았습니다.** `SK` 는 **SK주식회사(00144155)로 실제로 해소됩니다.**
1차 앵커 이름의 부분 문자열인 토큰을 빼서 `SK`·`하이닉스` 를 거릅니다.

> ⚠ **반만 막은 것이었습니다** ([§5-15](#5-15-사명의-부분-토큰이-실존-기업으로-해소된다--해소-2026-09-10)
> 실측 · 2026-09-10). 저 차단은 **1차 앵커 이름의 조각**만 봅니다 — **2차 쪽이 쪼개진
> 것은 못 막았습니다.** 「삼성전자와 SK하이닉스는 무슨 관계야?」의 **2차 앵커가
> SK주식회사**로 나가고 있었고, 「SK하이닉스와 한미반도체 거래 관계」는 **1차 앵커**가
> 그랬습니다. §5-15 가 후보를 덩어리 단위로 바꾸면서 함께 닫혔습니다.

★**사건·파급 경로는 안 바뀝니다.** `ring_of` 를 부르는 곳이 전체에서 **관계 선정 두 곳**
뿐이고(`retrieve_service`·`graph_tools`), 사건은 `evidence_selector.select`, 파급은
`select_propagation` 이 따로 정합니다. 두 호출부를 **함께** 고쳤습니다 — 한쪽만 고치면
두 입구가 다른 관계를 냅니다(계약 6 파리티).

```text
실측 — 앵커↔앵커 직접 관계가 재료에 들어온다
  삼성전자와 SK하이닉스는 무슨 관계야?    0 → 3건
  SK하이닉스와 한미반도체 거래 관계        0 → 3건
  LG전자와 삼성전자 중 누가 더 위험해?     0 → 1건
  삼성전자와 현대차 중 어디가 리스크가 커?  0 → 2건

불변식 — 단일·무앵커 질의 5건이 (앵커, 기업, 관계, 사건, 근거) 전부 **동일**
조회 비용 — 남는 토큰이 없으면 2차 조회를 안 한다. 단일 대상 질의는 여전히 **1회**
기준선 — ranking_baseline 앵커 14 케이스 무변화
```

> ⚠ **재료 기업은 1곳 그대로입니다.** 2차 앵커는 **랭킹에만** 관여하고 `companies` 를
> 늘리지 않습니다. 덕분에 재료가 두 배로 불지 않아 불변식이 지켜졌지만, **비교 질의의
> 사건·근거는 여전히 한쪽뿐**입니다 → **A-9** (✅ 2026-09-13 해소).

**A-8. ★앵커가 Company 하나뿐이다 — Person·Product 를 물으면 다른 것으로 답한다** ✅ **Recommended ①② 해소 (2026-09-10)**

최종 설계 §2.3 은 앵커를 `Company·Event·Person·Organization·Product` 로 정의하고
§17.2 가 「Global Graph Entity 기준으로 확장한다」를 명시합니다. **구현은 Company
하나입니다.**

```text
"이재용 관련 최근 이슈가 뭐야?"
  query_understanding.py:196   non_company_labels() 가 Person 토큰을 **떨어뜨린다**
                               → named=[] → anchorless → 전역 사건으로 답한다
  retrieve_service.py:147      hit.entity_type != COMPANY 면 재료에서 뺀다
```

★**A-6 과 같은 종류입니다** — 물은 것과 다른 대상으로 답하는데 응답에 그 사실을
알리는 필드가 없어 **소비자가 구별할 수 없습니다.** `unresolved` 도 아닙니다 — 그건
「못 찾았다」고 말하기라도 합니다.

★**재료는 이미 있습니다.** `search/model/enums.py` 의 `EntityType` 이 5종을 갖고
...
③ `_REL_TYPES` 에 IS_EXECUTIVE_OF·DEVELOPS 를 넣을지는 **따로 판단한다**
   ...
   ★**2026-09-06 해소됨 → A-11.** ①② 는 이제 이 선행 조건 없이 들어갈 수 있다.
```
...
★**2026-09-10 — §5-15 는 닫혔지만 이 11건은 그대로입니다.** 원인이 **쪼개짐이 아니라
품사**였습니다(§11④) — 덩어리는 이제 온전히 만들어지는데 NNP·SL 관문이 거릅니다.
...
```text
① 재료 관계 화이트리스트가 그래프의 25.4% 를 버린다  ★새로 찾음 → **A-11 로 해소**(2026-09-06)
...
④ Kiwi 가 기관·제품 이름을 고유명사로 안 준다
   ...
   ★**2026-09-10 — 반만 닫혔다.** §5-15 가 후보를 덩어리로 바꿔 「김준성은」은
     「김」→「김준」이 됐지만(여전히 틀리다) **기관·제품 넷은 그대로 0개**다.
     쪼개짐이 아니라 **품사**가 원인이라 그렇다 — 덩어리는 이제 온전한데
     (`('공정거래위원회', {NNG})`) NNP·SL 관문이 거른다. ★**그 관문을 풀면
     앵커리스가 죽는다**(실측 12/12 가 `unresolved`).
   ★★**2026-09-10 닫혔다 — A-8 단계 5.** 관문을 푼 것이 아니라 **①b 3단에서만**
     푼다. 거기서는 「그래프에 정확히 같은 이름의 노드가 있는가」를 묻는 것이라
     없는 이름은 저절로 떨어지고, 남는 위험(「반도체」·「메모리」가 Product 노드로
     실재하는 것)은 **기업 이웃 수**(`MIN_ANCHOR_COMPANIES=2`)로 막았다.
     오탐 0/16 · 정탐 7/8.
```
...
선행 조건       §5-15 토큰화 — B 분류 11건은 ①② 로 안 고쳐진다
               ★2026-09-10 **반만 닫힘** — 쪼개짐은 해소, 품사(NNG)는 남았다(§11④)
범위 밖         `_REL_TYPES` 확장(§13①) · 별칭 노드 병합(A-10)
```

#### 해소 — Recommended ①② 를 구현했다 (2026-09-10)

실측이 권고한 둘을 그대로 넣었습니다. **새 숫자도 새 티어도 만들지 않았습니다.**

```text
① decide_anchor()  ①b **3단** 신설 — Company 를 먼저 보고 못 찾았을 때만 온다
                   company_service.find_non_company_by_names()
                   ★Event 는 제외한다(`_ANCHOR_LABELS`) — 사건을 대상으로 삼으면
                     재료 조립이 다른 이야기가 된다
② material_companies()  비-Company 앵커면 **Path B** 로 간다
                   companies_via_anchor() — graph_service.relations_of() 를 그대로 쓴다
                   상한은 기존 _MAX_COMPANIES(5)
③ Anchor.label     계약 추가 — 「이 답이 Person 이재용에 대한 것」임을 응답이 말한다
```

★**전에는 ①a 가 이것을 떨어뜨렸습니다.** `non_company_labels()` 가 「기업이 아닌 것을
**기업으로** 오인하지 않는다」며 Person·Product 토큰을 뺐고, 그래서 「이재용 관련 최근
이슈가 뭐야?」가 `anchorless` 로 가 **전역 사건 목록**으로 답했습니다. 이제 **Person 으로
제대로 부르므로** 그 오인이 성립하지 않습니다. 제외 자체는 **남아 있습니다** — Event
이름과 겹치는 토큰을 거르는 것은 여전히 그 함수의 일입니다.

★**새 조회를 만들지 않았습니다.** Path B 의 1홉은 `graph_service.relations_of()` 가
돕니다 — 그쪽이 이미 만료 관계 제외 · 근거 검증 판정 반영 · 점수 내림차순을 합니다.
사본을 두면 「파급은 근거 없는 엣지를 안 타는데 재료 선정은 탄다」는 상태가 됩니다.

★**한 줄만 넓혔습니다** — `graph_service._QUERY` 의 이름 절에 `person_key` 를 더했습니다.
Person 노드는 `norm_name` 이 없어(실측) 이름 절이 `norm_name` 만 보면 Person 앵커의
재료가 **통째로 0** 이 됩니다. Organization·Product 는 `norm_name` 을 들어 전부터
걸렸습니다 — 구멍이 Person 하나였습니다. `person_key` 는 `이재용@00126186` 꼴이라
값 공간이 겹치지 않고 Company 노드는 그 속성을 갖지 않습니다.

**실측 — 같은 48건을 다시 돌렸다**

```text
anchor_source        query 14 → 35 · anchorless 32 → 11 · unresolved 2 → 2
★Query Coverage      23/48 → 36/48        판정이 바뀐 13건 전부 X→O · ★후퇴 0건

  앵커 \ 의도      RELATION      EVENT       IMPACT     SEMANTIC
  Company         3/3 · 3/3   2/3 · 2/3   3/3 · 3/3   3/3 · 3/3   ← 무변화
  Person          1/3 · 2/3   1/3 · 2/3   0/3 · 2/3   0/3 · 3/3
  Organization    2/3 · 2/3   1/3 · 1/3   3/3 · 3/3   0/3 · 1/3
  Product         0/3 · 2/3   0/3 · 1/3   3/3 · 3/3   1/3 · 3/3
```

★**기준선이 17/48 이 아니라 23/48 인 이유.** 17 은 ⓐ 적합도 + ⓑ **재료에 앵커 자신이
나타난다**를 둘 다 요구한 값인데, ⓑ 는 비-Company 앵커에 **원리적으로 성립하지 않습니다** —
앵커가 Person 이고 재료는 Company 라 앵커 자신이 재료에 있을 수가 없습니다. 한쪽에만
게이트를 걸면 개선이 부풀려지므로 **양쪽에 ⓐ 만** 걸었습니다(§2 가 「ⓑ 를 빼면 23/48」
이라고 적어 둔 그 값입니다).

★**게이트를 빼면 44/48 → 43/48 로 오히려 줍니다.** 게이트 없는 수치는 앵커리스가
전역 사건 10건으로 EVENT 를 통과하는 것을 성공으로 세기 때문입니다 — §9 의
「traversal 가능 = 성공으로 판정하지 않는다」가 여기서 실제로 물었습니다.

```text
종단 확인
  「이재용 관련 최근 이슈가 뭐야?」   앵커=이재용(Person)    재료=삼성에스디에스·삼성전자·삼성물산
  「정의선 최근 동향」               앵커=정의선(Person)    재료=현대모비스·현대오토에버·현대자동차·보스턴다이내믹스·기아
  「HBM 관련 리스크는?」            앵커=HBM(Product)    재료=엔비디아·삼성전자·CXMT·마이크론·화웨이
  「카카오톡 관련 이슈」             앵커=카카오톡(Product)  재료=카카오
  「삼성전자 최근 이슈가 뭐야?」       앵커=삼성전자(Company)  재료=삼성전자              ← 무변화
  「최근 반도체 업계 주요 이슈가 뭐야?」 anchorless                                    ← 무변화
  「무한상사 실적 알려줘」            unresolved                                    ← 무변화
```

★**계약 변경 하나** — `Anchor.label` 이 늘었습니다. **기본값 `Company` 이고 `required`
에 안 들어가** 기존 소비자는 무영향입니다(OpenAPI 대조: 바뀐 스키마 `Anchor` 하나 ·
경로 전부 동일). 백엔드 통보는 §E 에 걸어 뒀습니다.

#### 단계 5 — 일반명사 덩어리도 후보로 올린다 (2026-09-10)

§13④ 가 남긴 것입니다. Kiwi 가 기관·제품 이름을 자주 `NNG` 로 줘서
**공정거래위원회·고용노동부·낸드플래시가 후보에 오르지도 못했습니다.**

★**관문을 그냥 풀면 앵커리스가 죽습니다** — §5-15 에서 12/12 로 실측된 그것입니다.
그래서 **어디서만 푸는가**를 규칙 다섯으로 재고 골랐습니다. 코퍼스는 질의 24건
(앵커 없어야 하는 것 16 · 열려야 하는 것 8)입니다.

```text
                              오탐(앵커리스 16)   정탐(열려야 할 8)
  R0 현행(NNP·SL 만)                 0                0
  R1 NNG 덩어리 전부                  6                8   ← 「반도체」·「메모리」가 가져간다
  R2 + 기업 이웃 ≥1                  2                8   ← 「메모리」가 남는다
  R3 + 기업 이웃 ≥2  ★채택            0                7
  R4 NNG 는 Organization 만          0                5   ← 낸드플래시가 죽는다
```

★**가르는 신호는 「재료를 낼 수 있는가」입니다** — 새 랭킹 티어도 유사도 임계값도
아니고 **구조적 자격 조건**입니다. 업계를 묻는 질문에 기업 한 곳을 재료로 답하는 것은
앵커리스보다 나쁩니다.

```text
  반도체    기업 0곳     메모리   기업 1곳          ← 앵커로 안 세운다
  카카오톡  기업 2곳     낸드플래시 기업 7곳
  고용노동부 기업 7곳     공정거래위원회 기업 76곳
```

★**고유명사 덩어리에는 안 겁니다**(`named`). 「문무일」은 기업 이웃이 한 곳뿐인데
질문이 고유명사로 지목했으므로 대상이 맞습니다 — 이 조건이 막는 것은 **일반명사가
우연히 노드 이름과 겹치는 것**뿐이고, 단계 3 에서 이미 나가던 앵커는 무영향입니다.

★**R3 이 못 여는 하나는 「국민연금」**인데, 그건 **A-10**(별칭이 별도 노드 —
Organization 엣지 3 vs Company 국민연금공단 엣지 34)이라 **안 여는 것이 맞습니다.**

```text
★Query Coverage   23/48 → (①②) 36/48 → (단계 5) 40/48
앵커리스 보존       16/16 · 뒤집힘 0
전체 시험          1,160 passed · 실패 0
```

> ⚠ **공정거래위원회의 적합도 1.0 을 곧이곧대로 읽으면 안 됩니다.** §5 가 이미
> 「공정위의 1홉 폐포가 전역 후보의 **48.0%** 라 아무 재료나 절반은 관련으로
> 세어진다」고 적어 뒀습니다. 76곳을 점수 상위 5로 자른 결과가 「(주)DB Inc.·
> DB하이텍·현대오토에버·효성중공업」인데, 이것이 「공정거래위원회 관련 최근 이슈」의
> 답으로 맞는지는 **이 지표로 못 가릅니다** — **허브 절단 규칙은 여전히 Deferred** 입니다.


#### 남은 8건 — 원인이 셋으로 갈린다 (단계 5 뒤)

```text
토큰 (§5-15 가 남긴 것)              3건  김준성(「김준성은」→「김준」) ×2 · Brity Works ×1
                                        ★Brity Works 는 원문에 공백이 있어 덩어리가 안 붙는다
                                          (`merged_spans` 의 계약이 「사이가 벌어져 있지
                                          않았던 것만」이다) — 단계 5 로도 안 열린다
A-10 별칭 노드                       3건  국민연금 ×3. 2단이 **Company 국민연금공단**(00706742)
                                        으로 붙는데 적합도는 Organization 국민연금(엣지 3)의
                                        폐포로 재서 0.0 이 나온다 — 측정 아티팩트가 섞여 있다
「자기 사건 0」 Path C 폴백 (Deferred)  2건  삼성에스디에스 EVENT · 문무일 EVENT
                                        ★문무일의 1홉이 삼성에스디에스 한 곳이라
                                          Company 쪽 결함을 Person 이 그대로 물려받는다
```

★**토큰 관문(§13④)은 단계 5 가 닫았습니다** — 공정거래위원회 ×2 · 낸드플래시 ×2 가
열렸습니다. 국민연금 ×1 은 열리지 않았는데, 그건 A-10 쪽이라 맞습니다.

#### ★검토 — Coverage 40/48 은 과대평가였다 (2026-09-11)

측정 기준이 「재료 기업 ≥1 ∧ 관계 ≥1」이라 **어떤** 관계든 있으면 통과합니다. 사용자가
물은 엣지 — 「앵커 ↔ 재료 기업」 — 가 실제로 재료에 들어왔는지 따로 셌습니다.

```text
Coverage O 40건 중 ★앵커를 잇는 엣지가 재료에 있는 것
  Company        11 / 11    앵커가 곧 재료 기업이라 당연하다
  Person          4 / 9
  Organization    5 / 9
  Product         6 / 11
  합계           26 / 40    ← 14건은 「기업은 맞는데 그 관계는 안 보인다」
```

**A-8 이 고친 건 「어느 기업을 볼지」이고, 「물어본 그 엣지를 보여주는가」는 안 고쳤던
것입니다.** 원인은 §5-17 — 상대가 비-Company 라 Ring 2 로 밀립니다. 재료 기업이 많고
관계가 많은 앵커(정의선 5곳)일수록 밀립니다. 이재용(3곳)은 살아남고 정의선은 죽는
이유가 그것입니다. → [§5-17 ③](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo--앵커-축은-해소-2026-09-12)
이 같은 날 **26/40 → 38/40** 으로 닫았습니다.

★**같은 검토에서 나온 것 둘 더** — 둘 다 아직 안 고쳤습니다.

```text
공정거래위원회   70곳 중 상위 5 컷이 1.14 · 1.08 · 1.035 · 0.945 · 0.945 인데
               6·7위도 0.945 로 **동점**이다 — 경계가 동점 한복판이라 사실상 임의다
               (허브 절단 규칙 · Deferred 그대로)
이재용          삼성에스디에스 · 삼성전자가 score 1.0 **동점**이라 첫 재료가 삼성에스디에스인
               건 우연이고, 그 회사는 자기 사건이 0건이다(Path C 폴백 · Deferred 그대로)
```

★**CANDIDATE 45/48 에 못 미치는 것이 예상대로입니다.** §11 이 「45/48 은 **토큰이 온다고
가정했을 때**의 상한」이라고 적어 뒀고, 실제로 못 미친 9건 중 **8건이 토큰과 폴백**입니다.


**A-9. ★다중 앵커 질의의 재료가 1차 앵커로 쏠린다** ✅ **2026-09-13 해소** (2026-09-05 · A-7 구현이 드러냄)

앵커가 둘이어도 **재료 기업은 1차 앵커 하나**입니다. 관계는 앵커 축이 살려 내지만
**사건·근거는 1차 앵커 것만** 들어옵니다. 비교·분석 질의는 두 대상의 사건을 나란히
놓아야 답이 되는데 한쪽이 통째로 빕니다.

```text
"삼성전자와 현대차 중 어디가 리스크가 커?"
  앵커       삼성전자 · 현대차          ← 둘 다 잡혔다
  재료 기업   삼성전자 1곳
  사건 10건   전부 삼성전자 · 근거 22건 전부 삼성전자 계열
  관계 10건   현대차 끝점 2건은 들어옴 (앵커 축이 살렸다)

대조 — "현대차 최근 리스크 어때?"  →  사건 9건 · 근거 19건이 **실재한다**
      ★데이터는 있는데 비교 질의에서 안 들어온다
```
...
```text
③ material_companies()
     hits_reflect_the_anchor(decision, query) = True
     companies = companies_from(result)          ← retrieve_service.py:133
        히트만 본다 · decision.anchors 를 **안 본다**
     ② Search 의 히트가 1건(1차 앵커)이라 재료 기업도 1곳이 된다
```

★**`ring_of` 로는 못 고칩니다** — 관계 **순서**만 정하고, 사건·근거는 `companies` 가 정합니다.

★**A-7 범위에 안 넣은 이유** — ① 승인 범위가 「`decide_anchor()` 에서 2차 앵커 보강」
이고 이건 **재료 조립(③) 계층**이다 ② `companies` 를 2곳으로 늘리면
`limit = MAX_RELATIONS_PER_COMPANY × len(companies)` 가 **두 배**가 되어 사건·근거가
함께 붇고, A-1 이 세운 실측 표와 프롬프트 토큰 기준선을 전부 다시 재야 한다
③ **A-7 의 실패가 아니다** — A-7 이 목표한 것(앵커↔앵커 관계 진입)은 4/4 달성했고
불변식 5/5 도 지켰다.

#### 해소 — 좁아지던 한 줄에 「지목한 대상은 전부 재료」를 붙였다 (2026-09-13)

★**두 갈래가 이미 어긋나 있었습니다.** 같은 「A와 B」 질문인데 해소 경로에 따라
재료 기업 수가 달랐습니다.

```text
hits_reflect_the_anchor → companies_from(result)      히트만 본다 · anchors 를 안 본다
else (norm_name 폴백)    → anchor_companies(decision)   ★이미 앵커 **전부**를 쓴다
```

```text
retrieve_service.with_named_anchors(companies, decision)     ← 신설 · 히트 갈래에서만
  앵커가 하나면 무동작                    §5-16 의 교환(공급사 vs 앵커)을 그대로 둔다
  둘 이상이면 빠진 앵커를 **뒤에** 붙인다   히트 순서가 이긴다 — 앞에 세우면 상한에 히트가 밀린다
  상한은 기존 _MAX_COMPANIES · 잘리면 로그  새 숫자 없음
```

`/ask` 의 `plan_material` 이 같은 `material_companies()` 를 부르므로 두 입구 파리티는
자동입니다(실측 3질의 동일). Agent 범위도 2곳이 됩니다 — A-12 실측 ③ 「범위가 둘일
때 통과」가 비교 질의에서 실제로 성립합니다.

**실측 — 두 앵커 4건 · 워크스페이스 1건**

```text
                                        기업    사건            관계    근거   글자
  삼성전자와 현대차 중 어디가 리스크가 커?   1→2   10→14 (10+4)    10→20  20→34  2,981→5,150
  LG전자와 삼성전자 중 누가 더 위험해?      1→2   10→20 (10+11)   10→20  20→42  2,686→5,753
  삼성전자와 SK하이닉스는 무슨 관계야?      1→2   10→20 (10+12)   10→20  23→45  3,563→7,018
  SK하이닉스와 한미반도체 거래 관계         1→2   10→20 (10+11)   10→20  23→43  3,625→6,891
  삼성전자와 현대차 (워크스페이스 삼성·하이닉스) 1→2   10→14           10→20  20→34  2,975→5,299
      ★기업별 사건 상한이 그대로라 사건은 최대 2배 · 근거 글자는 5~7k 로
        §5 가 겪은 34k 의 1/5 이다. 사건 합이 20 을 넘는 것(10+11)은 공유 사건 1건이
        두 기업에 다 세여서다 — 응답에는 한 번만 실린다
```

**불변식 — 단일 앵커 · 앵커리스 7건 중 5건이 글자까지 동일**

```text
동일   삼성전자에 납품하는 기업 · SK하이닉스 임원 · 삼성전자 최근 리스크(ws) ·
       최근 주요 투자 이벤트(ws · 앵커리스) · 엔비디아는 어떤가(norm_name 폴백)
변경   현대차 최근 리스크 어때?   기업 1→2 (현대자동차 + 현대차 stub)   사건 9→11 · 근거 19→30
       네이버 최근 이슈 뭐야?     기업 1→2 (NAVER + 네이버 stub)        사건 3→5  · 근거 13→25
       ★둘 다 A-10 — 별칭 stub 이 2차 앵커로 잡혀 이 규칙을 탄 것이다. 규칙이 틀린 게
         아니라 데이터 결함이 재료까지 번진 것이라 A-10 에 넘겼다(아래 정정)
ranking_baseline 18 케이스   수정 전후 차이가 **글자까지 같다** — 9-02 스냅샷 이후 그래프가
                            바뀐 것(후보 933→970)이지 코드가 아니다
전체 시험                   1,169 passed · 실패 0 (신규 4건)
```

> ⚠ **A-10 이 A-9 의 값을 깎습니다.** 「삼성전자와 **현대차**」의 2차 앵커는 별칭 stub
> `현대차`(사건 4 · 관계 32)이지 `현대자동차`(00164742 · 사건 9 · 관계 62)가 아닙니다.
> 이번 수정이 들이는 것은 stub 의 4건이고, 9건은 A-10 을 닫아야 옵니다.
> → **같은 날 A-10 을 닫았다.** 2차 앵커가 정본 `현대자동차`(00164742)로 잡힌다 — 아래 A-10 실측.

**A-10. 별칭이 별도 노드면 같은 회사가 앵커 둘로 실린다** ✅ **2026-09-13 해소 (21건 병합 · 보류 2)** (2026-09-05 · A-7 구현이 드러냄) — ★**A-9 뒤로 재료에도 번진다 (2026-09-13)**

```text
"현대차 최근 리스크 어때?"   앵커  전 ['현대자동차']  →  후 ['현대자동차', '현대차']
"네이버 최근 이슈 뭐야?"     앵커  전 ['NAVER']      →  후 ['NAVER', '네이버']
   ★재료는 4/4 동일 — 기업·관계·사건·근거가 글자까지 같다
   ★영향은 응답 `anchors[]` 한 곳 — 한 회사가 **두 대상**으로 보인다
```

부분 문자열 차단이 **별칭은 못 막습니다** — `현대차` 는 `현대자동차` 의 부분이
아닙니다. 뿌리는 두 노드가 갈라진 **데이터 결함**(노드 병합 누락)이고,
A-7 의 2차 앵커가 그것을 드러냈을 뿐입니다.
`anchors[]` 는 「무엇을 대상으로 답했나」를 화면에 내리는 **계약 필드**입니다.

#### ★정정 — 「엣지 0건 남남」이 아니라 **둘 다 엣지를 가진 갈라진 노드**다 (2026-09-13)

```text
현대자동차  00164742  Company  stub  엣지 87  사건 9    corp_code 있음
현대차      (없음)    Company  stub  엣지 41  사건 4    ★corp_code 없음 · norm_name 이 key
NAVER      00266961  Company        엣지 307 사건 3
네이버      (없음)    Company  stub  엣지 31  사건 2
company_aliases   ('현대차' → canonical_key '현대차' · source=first_seen)
                  ('네이버' → '네이버')   ★자기 자신을 가리킨다 — 정본으로 못 접는다
```

★**「재료가 안 바뀐다」는 A-9 전까지만 맞았습니다.** A-9 가 「지목한 대상은 전부
재료 기업」으로 바꾸면서 별칭 stub 의 사건·관계·근거가 **재료에 들어옵니다** —
「현대차 최근 리스크 어때?」 사건 9→11 · 근거 19→30, 「네이버 최근 이슈」 3→5 · 13→25.
내용은 같은 회사 것이라 오답은 아니지만, 한 회사가 **두 기업으로 나뉘어** 프롬프트에
실리고 `_MAX_COMPANIES` 한 자리를 먹습니다. 비교 질의에서는 반대로 **정본 쪽 재료를
못 받습니다**(위 A-9 ⚠).

★**고칠 자리는 코드가 아니라 데이터입니다** — `batch/repair/node_identity.py`(정규화키
재계산 → 병합)가 이 둘을 못 접은 이유를 보고, `company_aliases` 의 `first_seen` 자기참조
행을 정본 `corp_code` 로 돌려야 합니다. 코드 쪽 관문(`_second_anchor` 에서 별칭이면
버리기)은 그 뒤에도 필요한지 그때 잽니다 — 지금 걸면 TSMC·마이크론처럼 `corp_code`
없는 **진짜** 기업을 가를 근거가 없습니다.

#### 해소 — 코드가 아니라 데이터를 고쳤다 · 손 목록 21건 (2026-09-13)

##### 1. 측정 — 현대차·네이버 둘이 아니라 24건이었고, 열쇠 일치는 판정이 아니었다

```text
Company 3,432 = 정본(corp_code) 1,164 + 무-corp_code 2,268
company_aliases 자기참조 first_seen 2,219행 → 무-corp_code 노드로 실존 2,093
  └ block_key 가 같은 정본이 따로 있는 stub    69 (쌍 71)
       같은 법인 · 표기만 다름       24   엣지 215 · HAS_EVENT 14
       다른 법인 · 자회사·해외법인·조합·JV  41   엣지  55         ← 접으면 안 된다
       확인 필요                     4
```

★**왜 기존 배치가 못 접었나** — 둘 다 구조적이다. `node_identity` 는 `normalize_company_name`
이 같아진 stub 만 병합한다(「현대차」≠「현대자동차」). `foreign_merge` 는 후보를
`corp_code IS NULL` 로만 뽑아 **정본이 후보에 못 들어갔고**, 대신 stub 마다 「노드 자신 —
열쇠 보관」 `first_seen` 행을 남겼다. 그것이 2,219행의 정체다.

★**열쇠 일치는 판정이 아니다** — 71쌍 중 41쌍이 다른 법인이다. 모델이 `canon_name` 을 답할 때
자회사에 모회사 정식명을 줬다(「KT AMERICA, INC.」→ "KT Corporation" → 열쇠 `kt` = 케이티).
`company_registry` 가 「열쇠는 헐겁게 — 잘못 묶여도 쌍 판정이 거른다」고 못 박은 그대로다.
그래서 배치 자동 병합이 아니라 `person_merge` 와 같은 **손 목록**(`CONFIRMED` /
`REVIEWED_NOT_MERGED`)으로 갔다.

##### 2. 고친 것 — `batch/repair/company_merge.py` 두 단계

```text
1단  company_aliases 에 hand 행    stub 키 → 정본 norm_name      21행 (first_seen 자기참조를 교체)
2단  Neo4j mergeNodes(정본, stub)  관계 194 · HAS_EVENT 14 를 그대로 옮김 · merged_keys·also_names 기록
     staged_edges  src/tgt_key    stub → corp_code             316행 (안 하면 다음 적재 때 되살아남)
     company_attributes           stub 행 삭제(pg_tidy 4번 선례)  21행
```

★**대표형은 corp_code 가 아니라 정본의 `norm_name`** 이다. `apply_alias` 의 반환값이
`normalize_company_name` 의 반환값이고, 그것이 resolver 의 명부 정확 매칭 키이자 검색 fuzzy
경로의 `similarity(corp_name, norm_q)` 비교값이다 — 8자리 코드를 넣으면 후자가 0 이 된다.
기존 hand·dart 행 1,028개도 전부 norm_name 이다.

★**`mergeRels: false`** — 다른 병합 배치(true)와 다르다. stub 관계 194건 중 36건이 정본과
「같은 유형·방향·상대」인데 **subtype·evidence 가 전부 다르다**(완전중복 0). 현대로템 →SUPPLIES_TO→
코레일 이 stub 쪽 '전동차', 정본 쪽 '광역철도 전동차'. APOC 가 뭉개면 한쪽이 소리 없이 사라진다.
이 그래프에서 그런 중복은 `batch/repair/edges.py` 3단이 자기 규칙(`subtypes` 배열·근거 합집합)으로
접으므로 관계는 그대로 옮기고 정리는 그쪽 몫으로 남겼다.

★**정본이 stub 을 소유하면 접지 않는다** — 22건으로 가다가 하나가 걸렸다. 「FADU Technology
Incorporated」는 열쇠·정식명이 파두와 같아 같은 법인으로 봤는데, 그래프에
`파두 -OWNS_STAKE_IN(자회사)-> FADU Technology Incorporated` 가 있었다(미국 자회사). 1단에서
썼던 hand 행을 되돌리고 2단 관문으로 코드화했다 — 정본과 직접 관계가 있으면 다른 법인의 증거다.

##### 3. 검증 — 관계는 하나도 안 줄었다

```text
적용 전 전체 덤프의 관계 11,060건에서 stub eid → 정본 eid 치환  ==  적용 후 관계 다중집합   ✓ (유실 0 · 변조 0)
Company 3,432 → 3,411 (−21)  ·  관계 11,060 → 11,060  ·  정본 21 연결 824 → 1,018 (+194) · HAS_EVENT 65 → 79 (+14)
정본의 기존 속성이 덮인 노드 0 (discard)  ·  배열이 된 스칼라 0  ·  staged_edges stub 키 316 → 0  ·  company_attributes stub 행 21 → 0
롤백 dry-run: 21건 전부 관계 194/194 를 정본에서 식별  ·  백업 /mnt/e/2026/Hanium/backups/a10-merge-20260913/
audit/graph: 병합이 새로 만든 경고는 「중복 엣지(같은 src,tgt,type,subtype) 10건」 하나 — 아래 남은 것 ①
```

##### 4. 실측 — 앵커가 하나로 잡힌다 (LLM 없이 `RetrieveService._search/_assemble`)

```text
"현대차 최근 리스크 어때?"              앵커  ['현대자동차', '현대차'] → ['현대자동차'(00164742)]   재료 기업 1
"네이버 최근 이슈 뭐야?"                앵커  ['NAVER', '네이버']     → ['NAVER'(00266961)]
"삼성전자와 현대차 중 누가 더 위험해?"   2차 앵커  stub 현대차(사건 4) → 정본 현대자동차(00164742)   ← 위 A-9 ⚠ 해소
```

`_second_anchor` 의 별칭 관문은 **필요 없어졌다** — 별칭이 정규화 단계(`apply_alias`)에서
정본으로 접히므로 앵커가 애초에 하나만 잡힌다. 새로 들어오는 「현대차」도 같은 자리에서 접힌다.

##### 5. 남은 것

```text
① 중복 엣지 37건 (그룹 36)   같은 양끝·유형. 그래프에 그대로 있고 `edges.py` 도 손대지 않았다 — 아래 ①′
② 보류 2                     퀄컴(손 목록 qualcomm → 퀄컴 이 대표형이라 역방향 — 그 행부터 정해야 함)
                              원익PNE(corp_code_master 에 원익피앤이가 둘 · resolver 가 그래프 노드와 다른 쪽을 고름)
③ 확인 필요 4                 Kakao G Corp. · KoMiCo Technology Inc. · 삼성전자 평택캠퍼스(사업장) · 일본 도쿄일렉트론
④ 정본 16곳 sector_label 공백  stub_profiles 는 노드 entity_kind·ksic 이 있으면 대상이 아니라 재실행으로 못 채움
⑤ 기업 카드 없는 정본 5곳     stub 카드 16건은 stale_cards 로 정리했으나 company_vectors 의 stub 조건이 노드
                              sector_label(PG 로 옮겨감)을 봐서 재생성이 안 됨 — 기존 배치의 어긋남
⑥ 열쇠에 안 걸린 별칭         국민연금(Organization) ↔ 국민연금공단(Company) 처럼 라벨이 다른 것은 이번 범위 밖
```

①′ **중복 엣지는 왜 아직 안 접었나 — `edges.py` 를 그대로 돌리면 안 되고, 범위부터 정했다** (2026-09-13)

```text
그룹 36 = 완전중복(출발·도착·유형·subtype 같음) 10 + subtype 다른 클러스터 26   · 병합 전부터 있던 클러스터 13 별도
36 그룹 전부 evidence_id 가 다르다 — 같은 관계를 다른 기사·공시가 말한 것
```

★**`edges.py`(2단 완전중복 병합 · 3단 클러스터링)는 근거는 지키지만 다른 것을 조용히 버린다** — 이 36 그룹에 실측:
  · `first_seen` 이 뒤로 밀리는 엣지 8건 (그중 5건이 완전중복 10 안) — 「최초 관측일」인데 생존자 것을 그대로 둔다.
    생존자 선정(confidence 최대 → last_seen 최신)과 최초 관측일 보존은 다른 문제다: 최신 엣지가 남더라도
    `first_seen` 은 가장 이른 값(08-03), `last_seen` 은 가장 늦은 값이어야 한다
  · 삭제 엣지에만 있는 DART 상세값(amount·book_value·first_acquired·purpose·investee_*) 7건
  · 이미 클러스터링된 생존자의 `subtypes` 배열이 스칼라 합집합으로 덮여 옛 표현 유실 6건
  · 양쪽 값이 다른 것 — 지분율(현대자동차→현대차증권 39.91 vs 25.43 · 시점이 다른 공시), 계약 금액(현대로템→한국철도공사
    2,591억 vs 7,100억), 사건 역할(현대자동차→부분파업 mentioned vs subject), 방향(outbound vs symmetric) —
    한쪽을 지우면 다른 사실을 지우는 것이라 사람이 봐야 한다

★**범위 결정** — subtype 이 다른 관계는 단순 중복이라 단정할 수 없어(다른 시점·다른 사실), **A-10 이 만든 완전중복
  10 만 먼저** 걷어내고 클러스터 39(A-10 26 + 기존 13)의 의미적 통합은 별도 후속으로 둔다. 완전중복 10 중 1건
  (현대자동차→부분파업 · role 충돌)은 그때도 REVIEW 다.
  `edges.py` 보완안(판정을 파이썬으로 옮겨 first_seen 최이른·근거·subtypes 합집합·값 필드 COALESCE·충돌은 REVIEW ·
  `--only dups`)을 만들어 dry-run(49 그룹 · REVIEW 10 · 삭제 예정 39)까지 봤으나 **적용하지 않고 폐기했다** —
  다시 할 때는 이 실측이 출발점이다.

---

**A-11. ★재료 관계 화이트리스트가 그래프의 25.4% 를 버린다** ✅ **2026-09-06 해소**

`company_service._REL_TYPES` 가 8종만 담고 있어 `IS_EXECUTIVE_OF`(787)·
`DEVELOPS`(2,050)가 **어느 경로로도 재료에 못 들어왔습니다.** 사건 축(`HAS_EVENT`·
`IMPACTS`)은 `events_of()` 라는 제 경로가 있지만 이 둘은 없었습니다.

```text
엣지 11,168 = 관계 재료 6,251(56.0%) + 사건 경로 2,080 + **어디로도 못 들어옴 2,837(25.4%)**
```

★**앵커 문제가 아닙니다** — 앵커가 완벽히 잡힌 Company 질의에서도 그랬습니다.

```text
"삼성전자 임원이 누구야?"      앵커 00126380 ✅  → relations = OWNS_STAKE_IN ×10.  임원 0건
"삼성전자가 개발하는 제품은?"  앵커 00126380 ✅  → relations = OWNS_STAKE_IN ×10.  제품 0건
```

★**그래서 A-8(비-Company 앵커)과 갈립니다.** A-8 을 다 고쳐도 이건 안 고쳐집니다 —
Person 의 정의 엣지와 Product 의 정의 엣지가 하필 빠진 그 둘이라, 앵커를 확장해도
관계 질의는 △ 에서 멈춥니다(A-8 실측 §11③).

#### 해소 — 상수 한 줄인데, 고른 근거가 넷이었다 (2026-09-06)

```text
_REL_TYPES = (… 8종 …, 'IS_EXECUTIVE_OF', 'DEVELOPS')
```

★**계약을 「여집합」으로 바꿨습니다.** 열거로 두면 새 `EdgeType` 이 생겼을 때 아무도
여기를 안 고치고 그 유형은 조용히 재료 밖에 남습니다 — **이번에 실제로 그렇게
됐던 것**입니다. `tests/services/test_relation_types.py` 가
`set(_REL_TYPES) | {HAS_EVENT, IMPACTS} == EdgeType 전체` 로 묶어 둡니다.

넷 중 어느 하나로도 이 확장을 정당화할 수 있습니다.

```text
① 그래프는 **이미 그리고 있었다**
   company_graph 가 삼성전자에서 IS_EXECUTIVE_OF ×5 · DEVELOPS ×5 를 그리는데
   목록은 담을 수가 없었다 — 「목록과 그림이 각자 기준으로 뽑으면 어긋난다」는
   이 저장소의 불변식이 **그 두 유형에서만** 깨져 있었다.
② 스키마는 이미 받는다
   EdgeType 12종 · NodeLabel 5종에 다 있고 relation_row() 는 라벨·verdict·신선도를
   일반적으로 처리한다. **스키마 변경 0 · OpenAPI 지문 동일**(sha256 대조).
③ 도구 계층은 이미 대비돼 있었다
   graph_tools._caution_of 의 DEVELOPS 분기가 **한 번도 안 돌던 죽은 코드**였다.
   시험은 stub 을 먹여 통과 중이었다 — 「초록인데 기능은 죽어 있는」 상태.
④ 품질을 이유로 뺀 것이 아니다
   DEVELOPS 필터 통과 85.3% · news 출처 81%
   SUPPLIES_TO 필터 통과 85.8% · news 출처 83%   ← 이미 화이트리스트 안
   그것 때문에 뺐다면 SUPPLIES_TO 도 빠져야 한다. 뉴스 DEVELOPS 의 오추출(47%)은
   **지우는 것이 아니라 표시하는 것**이 이 저장소의 규칙이고, 그 표시가 ③ 이다.
```

**실측 — 무엇이 바뀌고 무엇이 안 바뀌었나**

```text
기업 10곳 relations_of 지문 대조   ★사라진 edge_id **0**. 추가만 있다
   삼성전자 653 → 819 (임원 22 · 제품 144)   SK하이닉스 254 → 330   CJ대한통운 5 → 8
기업 상세                          그래프 엣지 **무변화**(86→86 · 43→43 · 8→8)
                                   목록 related 70 → 80 — 그래프가 그린 관계와 **정확히 일치**
관계 상대 라벨(삼성전자)             Person 21 → 43 · Product 10 → 154 · Organization 84 무변화
/retrieve 재료 12질의               관계 구성 무변화 7 · 고쳐짐 4 · 늘어남 1
   "삼성전자 임원이 누구야?"      OWNS_STAKE_IN ×10 → **IS_EXECUTIVE_OF ×10**
   "삼성전자가 개발하는 제품은?"  OWNS_STAKE_IN ×10 → **DEVELOPS ×10**
   "삼성전자에 납품하는 기업은?"  29 → 40건. ★상한(50)에 안 닿아 **밀려난 것은 없고**
                                  물은 SUPPLIES_TO 16건은 relation_selector 가 그대로 위에 둔다
graph_tools 죽은 분기              DEVELOPS 10건이 도구에 도달, 그중 news 출처 2건에 경고가 붙었다
```

★**사건은 하나도 안 건드렸습니다** — 12질의 전부 `events` 가 글자까지 같습니다.
`_REL_TYPES` 는 관계 조회에만 쓰이고 사건은 `_EVENT_Q` 가 따로 봅니다.

#### 남은 것 — 워크스페이스가 있으면 아직 안 걸린다

```text
"삼성전자 임원이 누구야?"   워크스페이스 없음   0/10 → **10/10**  ✅
                            워크스페이스 있음   0/10 →   0/10    ❌
```

링이 의도를 이깁니다 — 링 분포가 `{0:3, 1:535, 2:281}` 인데 Person·Product 는
비-Company 라 Ring 2 이고, 상한 10 을 Ring 1 이 다 먹습니다.
**그건 이 변경이 아니라 [§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo--앵커-축은-해소-2026-09-12)
의 몫**이라 여기서 함께 고치지 않았습니다 — 랭킹 정책 변경은 회귀 범위가 다릅니다.

★라우터 키워드도 남습니다 — `_SHALLOW_KEYWORDS` 가 「임원」·「개발」만 알아서
「경영진」·「제품군」은 의도로 안 잡힙니다. 그러면 이 두 유형이 점수순으로만 겨루는데,
큰 기업에서는 상위 10에 못 듭니다(실측: 삼성전자·SK하이닉스·LG전자에서 0건).

---

**A-12. ★Agent 는 재료 기업 밖으로 한 발도 못 나간다 — 「탐색」은 없고 「조사」만 있다** `[DECIDE]` (2026-09-10 · 실측 · 코드 무수정)

「지금 구조가 Agentic GraphRAG 인가, 어디까지 더 만들어야 하나」를 판단하려고
`agent_loop → graph_tools → relation_service → graph_service` 를 되짚었습니다.
**설계 의도가 아니라 호출 관계로만** 봤습니다.

**차단 지점은 한 곳이고, 그것이 전부입니다.**

```text
plan_material        material.py:186    companies 확정 ← ★State 를 통틀어 유일한 쓰기
  ↓
agent ⇄ run_tools    agent_loop.py:246  with _scope_of(state)
                     agent_loop.py:128  anchor_scope([c.key for c in state["companies"]])
  ↓
graph_tools.get_*    tools/keys.py:53   wanted = scope.check(keys)
                                        k not in allowed → OutOfScopeKey
```

★**「초기 scope」가 아니라 요청이 끝날 때까지 고정입니다.** `companies` 를 쓰는 노드가
`plan_material` 하나뿐인데 `run_tools` 는 매 턴 그 값으로 범위를 **다시 세웁니다.**
넓히는 코드가 없습니다. 도구 7종이 전부 `keys.resolved()` 라는 같은 깔때기를 지나므로
우회로도 없습니다.

**실측 — Agent 는 상대의 key 를 보면서도 그 key 를 못 부릅니다.**

```text
① get_relations(삼성전자) → Agent 가 본 JSON
     target = SK하이닉스 / target_key = 00164779       ★key 가 그대로 보인다
② get_relations(SK하이닉스)
     {"error": "재료 범위 밖의 key: ['00164779'] (허용 ['00126380'])"}
     수집된 재료 {'get_relations': 1}                   ★거부는 재료를 안 늘린다
③ 범위가 둘일 때 get_relations(SK하이닉스) → []        통과(범위 안이므로)
```

★**의도된 설계입니다.** `RelationDTO.target_key` 를 **보여주면서** 그걸로 부르는 것은
막습니다. `tests/graph/test_agent_loop.py:306` 이 이 행동을 불변식으로 묶어 뒀습니다.
③ 이 통과하는 것은 탐색이 아니라 **프론티어 안에서의 재방문**입니다 — 앵커리스 경로처럼
`companies` 가 여러 곳일 때만 성립하고, 그 목록도 서버가 정한 것입니다.

**그래서 동적인 것과 정적인 것이 갈립니다.**

```text
동적이다  어떤 도구를 · 어떤 순서로 · 몇 번(≤12) · 언제 끝낼지(should_continue)
          search_news·search_dart 의 query 문자열 — LLM 이 자유 작성한다
          ★결과가 다음 도구 선택을 바꾼다 — get_relations 가 비면 search_news 로 간다
정적이다  ★어떤 노드를 만질 수 있는가 — 요청 시작부터 끝까지 한 벌
```

즉 **「무엇을 물을지」는 동적이고 「누구에게 물을지」는 완전 정적**입니다. 정확히 부르면
**Fixed-Frontier Agentic Retrieval over a Deterministic Graph Kernel** 이고, GraphRAG
분류로는 Graph-as-Retriever 이지 Graph-as-Agent-Environment 가 아닙니다.

#### 파급이 실제로 걷는 그래프 — 2홉 · 엣지 2종

`graph_service._PROPAGATE` 전문 기준입니다.

```text
hop 0   (parent:Company) WHERE hit.part_of_corp_code = parent.corp_code   :287
        공장·사업부 → 모회사 승격. ★감쇠 없음 — 홉이 아니라 같은 법인이다
hop 1   (e:Event)-[i:IMPACTS]->(hit:Company)                              :280
        edge type = IMPACTS 하나 · stated=True(기사가 말한 것)
hop 2   (c)-[s:SUPPLIES_TO]-(d:Company)                                   :294
        edge type = SUPPLIES_TO 하나
        ★패턴이 **무방향**이고 방향은 startNode(s)=c 로 사후 분류 → supply | demand
        decay supply 0.55 / demand 0.35 · hub penalty 40/(40+deg-1)
hop 3+  ★없다
```

★**Agent 가 보는 그래프와 파급이 걷는 그래프가 다릅니다.** `get_relations` 는 A-11 이
연 **10종**을 보는데 파급은 **2종**만 걷습니다. 그리고
`relation_service.event_impact(max_hops=3)` 의 3 은 **죽은 슬랙**입니다 —
`propagate_risk` 가 hops 2 를 넘는 값을 만들지 않아 이 필터는 한 번도 안 뭅니다.

#### ★코드가 이 확장을 위해 자리를 셋 비워 뒀다

| 예약석 | 위치 | 지금 |
|---|---|---|
| `MAX_HOPS = 6` · `hops_used` ∈ `_CAPS` | `budget.py:48,66` | 아무도 안 가산 → `0 >= 6` 이 늘 거짓 |
| `provenance: Literal["direct","explored"]` | `tools/dto.py:175,272` | `explored` 를 만드는 코드가 없다 |
| `test_no_code_path_produces_explored_yet` | `tests/graph/test_provenance.py:87` | ★트립와이어 — `dto.py` 밖에 `explored` 가 나타나면 **실패**한다 |

주석이 `explore_impact 는 2-B` 라고 이름까지 박아 뒀습니다. **미구현이지 미설계가
아닙니다** — 배선만 안 된 상태입니다.

#### 판정 — 지금은 추가하지 않는다

```text
평가셋 23 케이스(tests/agent/eval/cases.py) 중
   A→B→C 확장을 요구하는 것 ★0건 — 전부 앵커 1홉 또는 앵커리스 전역 사건
최종 설계의 가장 복잡한 시나리오
   User_Scenarios §16 「NVIDIA 투자 관련 공급망 리스크」의 흐름이
   Global Search → Contextual Ranking → Evidence → Single Agent → Answer
   ★agentic traversal 이 설계 문서 어디에도 없다
파급이 필요한 질문은 fetch_propagation 이 이미 결정론 2홉으로 답한다
```

★**지금 넣으면 계약 6(파리티)을 깨면서 평가셋 23건 중 0건을 개선합니다.**
`/retrieve` 에는 Agent 루프가 없으므로 `/ask` 재료만 늘어납니다.

★**필요한 것은 탐색권이 아니라 프론티어 확장입니다.** A-8 실측이 네 앵커 타입의 IMPACT
를 전부 ADOPT 로 낸 근거가 Path B(Anchor→Company)·Path C(Anchor→Company→Event)인데,
그것은 `plan_material` 이 재료를 정할 때 한 홉 더 들어가는 일이지 Agent 가 재량으로
파고드는 일이 아닙니다. A-8 §6 이 「무제한 graph traversal 은 금지한다」로 이미
못 박았습니다.

#### 추가한다면 — 최소 변경안과 착수 조건

```text
① graph_tools.explore_impact(key)  범위 안 key 하나에서 1홉. ★새 쿼리를 만들지 않는다
                                   edge type 화이트리스트·limit 은 내부 상수(4원칙 ③)
                                   provenance="explored" 로 찍는다
② agent_tools._EXPOSED 에 추가      인자는 key 하나
③ run_tools                        결과의 Company 끝을 State.frontier 로 승격하고
                                   spend(state, hops_used=…)  ★예약된 카운터가 여기서 산다
④ _scope_of                        anchor_scope(companies + frontier, …)   ← 한 줄
```

**`budget.py`·`scope.py`·`citation.py`·`evidence_validation` 은 무수정입니다.**
프론티어에 들어오는 것이 **서버가 이미 재료로 돌려준 엣지의 끝점**뿐이라 도구 4원칙 ①
이 그대로 지켜집니다 — Agent 가 임의 key 를 넣는 길은 여전히 없습니다.

★**유일한 실제 비용은 계약 6 입니다.** `ask_graph_parity` 가 깨지고, 해법은 대조를
`provenance == "direct"` 로 한정하는 것입니다 — **`provenance` 필드가 존재하는 이유가
정확히 이것입니다.** `test_provenance.py:87` 은 그때 **의도적으로 깨야 하는 시험**이고,
그 트립와이어를 해제하는 것이 이 작업의 정식 착수 신호입니다.

**착수 조건** — [§5-15](#5-15-사명의-부분-토큰이-실존-기업으로-해소된다--해소-2026-09-10) → A-8 구현
→ A-9 를 끝낸 뒤 **다시 잽니다.** 그때도 「A→B→C 를 Agent 가 골라야만 답이 되는 질문」이
평가셋에 남으면 그때 넣습니다. 남지 않으면 예약석 셋을 그대로 두고 닫습니다.

★**검증** — 코드 무수정. `tests/graph/test_agent_loop.py` · `test_provenance.py` ·
`test_propagation_budget.py` · `tests/tools/test_graph_tools.py` · `tests/tools/test_keys.py`
**79 passed**.
...
**A-13. Event 를 앵커로 열지 않았다 — A-8 이 좁힌 범위** `[DECIDE]` (2026-09-10)

최종 설계 §2.3 은 앵커를 `Company·Event·Person·Organization·Product` 로 정의합니다.
A-8 이 넷을 열었고 **Event 하나를 남겼습니다.** 영구 거부가 아니라 **아직 안 쟀으니
열지 않는다**입니다. 근거 셋입니다.

```text
① A-8 실측이 Event 를 안 쟀다
   매트릭스가 4타입 × 4의도다. Event 는 Node Coverage 도 Query Coverage 도 lift 도
   **한 번도 안 쟀다.** 지금 넣으면 A-8 의 첫 원칙(「실측 전에 구현 방향을 결정하지
   않는다」)을 어기는 것이고, 그 원칙이 실제로 「Person 은 Event 0이니 REJECT」를 뒤집었다.

② 재료 조립의 방향이 반대다
   Company·Person·Product   앵커 → 기업들 → ★그 기업들의 사건
   Event                    사건 → 기업들 → 그 기업들의 사건
                                            🔴 물은 그 사건이 다른 사건에 파묻힌다
   「○○ 사건 어떻게 됐어?」의 답은 **그 사건의 timeline·근거·파급**이지 경유 사건이
   아니다. 그건 `fetch_propagation` 쪽 조립에 가깝고 별도 설계다.
   ★**그래프가 막은 것이 아니다** — Event 1홉은 잘 이어져 있다(아래 실측).

③ 이름 충돌이 실재한다
   Event 이름이 다른 노드와 **정확히** 겹치는 것 5건 — 그중 하나가 Company 다.
       '고성 천연가스발전소'  ← ★Company
       '테일러 팹' · '모바일 운전면허증' · '1Tb QLC 9세대 V낸드' · '모바일 국가보훈등록증'  ← Product
   지금은 ①b 2단(Company)이 3단보다 앞이라 Company 가 이긴다. Event 를 열면 저 5건이
   **순서 하나에 「회사를 물었는데 사건으로 답하는」** 경우가 된다.
   ★게다가 `non_company_labels()` 의 Event 항목은 지금 **토큰을 억제하는** 용도다
     (§8-5 가 정확일치로 좁힌 그 자리). 같은 표를 억제와 승격에 함께 쓰면 충돌한다.
```
...
```text
Event 노드 1,074
Event 1홉 엣지   HAS_EVENT→Company 1,009 · IMPACTS→Company 981
                IMPACTS→Product 65 · HAS_EVENT→Product 25
   ★즉 Path B 자체는 구조적으로 열려 있다. 막힌 것은 그래프가 아니라 **조립 순서**다.
```

**두 겹으로 못 박아 뒀습니다**

```text
app/api/schemas.py        Anchor.label: Literal[Company, Person, Organization, Product]
company_service.py        _ANCHOR_LABELS = {Person, Organization, Product}
tests/…/test_anchor_schema.py::test_anchor_label_rejects_event
```

**열려면 필요한 것 — A-8 의 확장이 아니라 별도 항목이다**

```text
① Event × 4의도 매트릭스 실측 (A-8 과 같은 방식 · 대표 질의 12건)
② 「사건을 물었을 때의 재료」를 따로 정의
   그 사건의 timeline·근거·파급이 재료이지 기업 경유 사건이 아니다
③ Company 와 이름이 겹치는 5건의 우선순위 결정 (지금은 Company 가 이긴다)
```
...
남기므로 라벨 없이 세 갈래가 갈립니다.

★**그런데 「`trace_id != "-"` 로 배치를 뺀다」가 실측에서 안 통합니다** (2026-09-05).

```text
logs/queries.jsonl  4,192행 중 trace_id 있음 3,369행(80%)
  상위 질문 —  "q" 1,727 · "담합 소송" 492 · "삼성전자 임원" 388 · "x" 195
```

`trace_id` 는 **요청 경계에서** 발급되는데 `TestClient` 를 쓰는 테스트도 그 경계를
지납니다. `BIZNODE_QUERY_LOG` 를 끄는 곳이 `conftest.py`·`pytest.ini`·`run_test.py`
**0곳**이라, pytest 를 돌릴 때마다 같은 파일에 쌓입니다. 지금 집계하면 평가셋
질의로 만든 분포가 나옵니다.

★**이것이 B-1 의 선행 조건이었고, 닫혔습니다** (2026-09-05).

`tests/conftest.py` 에 autouse 그물을 두어 pytest 가 질의 로그를 건드리지 않습니다.
★**환경변수로는 늦습니다** — `querylog` 가 `_DISABLED`·`_PATH` 를 **import 시점에**
굳히므로 conftest 가 도는 시점엔 이미 정해져 있습니다. 모듈 속성을 직접 덮습니다.

```text
환경변수 없이 전체 시험 — 로그 4,192행 → 4,192행
  ★고치기 전 같은 실행이 약 100행을 쌓았다
```

오염된 표본은 `logs/queries.2026-09-05.jsonl` 로 옮겼습니다. **집계는 새 파일로
시작합니다** — 옛 표본 위에 얹으면 그물이 생겨도 답이 안 달라집니다.

> ⚠ **`run_test.py` 수동 시험은 아직 쌓입니다.** 실제 경로를 그대로 도는데 질의는
> 개발자가 지은 것이라, B-1 이 피하려는 편향과 **성격이 같습니다.** 끄거나 ·
> `source` 필드로 가르거나 · 집계할 때 빼거나 — **B-1 을 돌리기 전에 정합니다.**

★B-3 와 D 의 `_STRIP_UNLINKED_CLAIMS` 결정도 이 표본을 기다립니다.
...
**B-4. 답변 내용 품질을 아무도 채점한 적이 없다** `[TODO]` — §5-11 그대로. ★2026-09-13 사람이 읽은 표본 2건이 §5-11 에 있다(근거 일치 10/10 · 비교 질의의 판정 부재는 `[DECIDE]`)
...
| 무엇 | 상태 |
|---|---|
| ★**계약 변경 3건 백엔드 통보** — `Event.company` 추가 · 앵커 없는 질문의 `match_type` `EXACT`→`SEMANTIC` · **`Anchor.label` 추가**(2026-09-10 · A-8. 기본값 `Company` · `required` 아님 · OpenAPI 대조에서 바뀐 스키마는 `Anchor` 하나이고 경로는 전부 동일) · **`/retrieve`·`/ask` 관계의 Person 끝 `key` 가 이름 → `person_key`**(2026-09-12 · §5-17 ③. 기업 상세 `/companies/{key}` 의 그래프와 **같아진 것**이다 — 전에는 같은 사람이 두 응답에서 다른 key 였다. 스키마는 무변경이고 값만 바뀐다. Company·Organization·Product 끝은 글자까지 불변) | 가이드에는 적었다([연동 가이드 §10](BizNode_백엔드_연동_가이드.md)) · **통보 안 함** |
| `workspace_service._NEW_RISK` 가 **적재일**(`first_seen`)로 「최근」을 거른다 | `[TODO]` · 별건 |
| `/news` 스텁 1개 | `[TODO]` · 기존 |
| ★**대화 맥락 저장은 백엔드가 한다** — 2026-09-05 결정. §8-22 의 「소유권 미정」이 닫혔다 | 남은 것은 **계약**이다 — 후속 질문(「그럼 공급망에는?」)을 위해 `/ask` 가 이전 대화를 **받는가**. 지금 `AskRequest` 는 `{question, workspace_keys, context_keys}` 뿐이고 `app/`·`search/` 에 conversation 개념이 **0** 이다 `[DECIDE]` |

---

#### F. 운영 · 개발 기반 — **배포 전에 걸릴 것** (2026-09-05 신설)

| 무엇 | 상태 |
|---|---|
| ★**Chroma 가 없으면 앱이 import 단계에서 죽는다** — `main.py:106` 이 모듈 전역에서 `RetrieveService()` → `build_orchestrator()` → Chroma 연결. 같은 이유로 **테스트도 수집이 중단**된다(`pytest --collect-only` → 1,078/1,116 수집 후 `Interrupted: 2 errors`, `test_ask_api`·`test_retrieve_api`) | `[TODO]` |
| ★**`/health` 가 의존성을 하나도 안 본다** — `{"status":"ok"}` 고정값이라 Neo4j·PG·Chroma·OpenAI 중 무엇이 죽어도 `ok` 다 | `[TODO]` |
| **`tests/graph/test_parity.py` 가 지워진 모듈을 import 한다** — `d679e87`(2026-09-04 코드 정리)이 `batch/audit/ask_graph_parity.py` 를 지웠는데 시험은 남아 `-m needs_db` 에서 3건 `ModuleNotFoundError`. 기본 실행(`not needs_db`)에서는 안 보여 여태 몰랐다. 시험을 지우거나 기준선 대조를 살아 있는 경로로 옮기거나 — 결정 필요 (2026-09-13 발견) | `[TODO]` |
| **CI 가 없다** — `.github` 없음. 테스트 1,116건이 자동 게이트 없이 손으로만 돈다 | `[TODO]` |
| **배치 자동화 정의가 저장소에 없다** — [README §8](README.md) 이 「매일 18:00 시세 · 07:00 공시」를 규정하는데 crontab·스케줄러가 코드·문서 **0곳**이다. 「최근」이 랭킹의 축인데 신선도가 수동 실행에 달렸다. ★뉴스가 수동인 것은 구글 차단 때문이라 **의도된 결정**이고, 이건 나머지 얘기다 | `[TODO]` |
...
| **workspace anchor 질의 응답 형태 (C-3)** | 프롬프트 + 재료 구성 | `anchor_source=workspace` 면 목록형 → [설계서 §14-6](BizNode_Search_설계.md#14-6-답변-형태를-anchor_source-로-가른다-2026-08-25-확정) |
...
> [설계서 §10](BizNode_Search_설계.md#10-ask-전체-flow--10단계) 의 ①a~⑩ 은 **`/ask` flow(실행 단계)**
...
| 항목 | 내용 |
|---|---|
| 목표 | [설계서 3부](BizNode_Search_설계.md#3부--챗봇-답변을-쓴다)를 팀 합의 기준으로 확정. [§7](#7-결정해야-할-것--decide) 의 `[DECIDE]` 에 답을 붙인다 |
| 수정 대상 | **문서만** |
| 수정하지 않을 대상 | 코드 전부 |
...
| **2026-08-26 진행 (2차)** | ★**ⓐⓓ 완료 — ② 단계가 닫혔다.** `relation_selector` 신설(`app/services/relation_selector.py`)로 「삼성전자가 납품하는 기업」에서 `SUPPLIES_TO` 가 위로 온다(ⓐ). 잘라낸 관계 개수 로그에 `matched`·`direction` 을 더했다(ⓓ). ★**링을 가로지르지 않는다** — 의도는 링 **안에서만** 줄을 세운다([§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo--앵커-축은-해소-2026-09-12) 이 아직 `[DECIDE]`). `search/` 무수정 → [§5-4](#5-4-관계에는-의도-필터가-없다-사건에는-있는데--해소-2026-08-26) |
| **2026-08-26 진행 (1차)** | ★**ⓒⓔⓕⓖⓗ 완료 · ⓐⓓ 미착수.** `anchor_source` 3분법 · `unresolved` 조기 반환 · 빈 `workspace_keys` 거부 · `corp_code` → `norm_name` fallback · 앵커 기준 재료 수집 · Ring 0→1→2 정렬 · `companies=[]` 백스톱 · 답변 형태 분기 · 워크스페이스 소속 표기가 붙었다. **`relation_selector`(ⓐ)와 잘라낸 관계 개수 로그(ⓓ)는 아직**이다. 41건 전수로 불변식을 확인했다([§8-5](#8-5-anchor_source-판정-신호-2026-08-25)·[§8-7](#8-7-앵커가-재료를-정하는가--41건-전수-2026-08-26)). 테스트 487 → **579 passed · 2 xfailed**. ★`search/` 무수정. **A-3 는 🟡 진행 중**이다 — [§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo--앵커-축은-해소-2026-09-12) 과 [§7-4](#7-4-2026-08-25-개정이-새로-연-decide-2건) 가 남아 있다 |
...
| 수정 대상 | `answer_service._fact_lines()` → 블록 분할 · 프롬프트 규칙 문구(§5-9) · ~~`role`·`symmetric`·`press` 노출(§5-5)~~ **✅ 2026-08-26 완료** · 보간값 개행 제거(§5-10) · ★**워크스페이스 소속 표기**([설계서 §12](BizNode_Search_설계.md#12-사실과-추론의-경계--4등급)) · ★**§5-19 파급 경로 문자열의 라벨 누출** |
...
| **Agent Tool 연동** | `[DONE]` | — | **2026-08-28 완료**(Phase 2). 도구 7종 · 누적 예산 · 범위 방어. ★**탐색 확장은 하지 않기로 했다** — 근거는 [§6-0 A-12](#a-결함--만든-기능의-값어치를-지금-깎고-있는-것) |
...
이므로, [설계서 §14](BizNode_Search_설계.md#14-앵커-출처--무엇을-대상으로-답하는가) 의
...
| 오탐 2건을 규칙으로 보정할까 | ❌ **안 한다** | 원인은 안다(「가동이 어려워」·「신성장동력」). 다만 **n=4 에 맞춰 규칙을 얹는 것은 과적합**이다([규칙 5](BizNode_Search_설계.md#1-4-이-문서-전체를-관통하는-규칙-여섯)) |
...
| ~~**주제 질의를 목록형으로 바꿀까**~~ | ✅ **닫힘 (2026-08-25)** — `anchor_source` 로 분기한다. `query` 는 서술, `workspace` 는 **목록 + 「대상을 저희가 골랐다」 헤지**, `unresolved` 는 고정 문구 | 질문 자체가 ⓐ(주제 질의)가 아니라 **워크스페이스 앵커 질의**로 옮겨왔다 → [설계서 §14-6](BizNode_Search_설계.md#14-6-답변-형태를-anchor_source-로-가른다-2026-08-25-확정). **화면 표현은 여전히 프론트와 맞춘다** |
...
| ★**Ring 2 가 답인 질의의 관계 우선순위** | 링별 quota 냐 질의 의도별 우선순위냐. **둘 다 재 본 적이 없다** → [§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo--앵커-축은-해소-2026-09-12) |
...
| **norm_name 정규화의 권위** | `corp_code` 가 없는 기업이 `workspace_keys` 에 들어올 때, 백엔드가 우리 `normalize_company_name()` 과 **같은 정규화**를 돌릴 수 있는가. 못 하면 **우리 경계에서 한 번 더 정규화**해야 하고, 그러면 「받은 key 를 변환하지 않는다」와 충돌한다. 어긋나면 Neo4j 조회가 **조용히 0건**이 된다 | **팀 확인** → [§10](#10-확인하지-못한-것) · [설계서 §16-1](BizNode_Search_설계.md#16-1-key-는-corp_code-또는-norm_name-이다) |
...
[설계서 §14](BizNode_Search_설계.md#14-앵커-출처--무엇을-대상으로-답하는가)가 직접 든 예 3.
워크스페이스는 삼성전자·SK하이닉스, ①b 해소는 `SearchOrchestrator.search()` 의 분기를
그대로 흉내 냈습니다(`search/` 무수정).

★**잰 이유** — [설계서 §14-3](BizNode_Search_설계.md#14-3-해소-실패는-워크스페이스로-떨어지면-안-된다)
...
근거가 아니라 **더 재야 한다는 신호**입니다([규칙 5](BizNode_Search_설계.md#1-4-이-문서-전체를-관통하는-규칙-여섯)).
...
([§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo--앵커-축은-해소-2026-09-12)).

### 8-8. 재료 정합성 규칙 실측 (2026-08-26)

**규칙을 짐작으로 정하지 않으려고 잰 것**입니다([규칙 5](BizNode_Search_설계.md#1-4-이-문서-전체를-관통하는-규칙-여섯)).
...
[§14-3](BizNode_Search_설계.md#14-3-해소-실패는-워크스페이스로-떨어지면-안-된다)
...
| `graph_tools` 의 링 정렬 후 절단 | 위와 같은 이유. Ring 3 가 상한에서 잘리는 것은 [§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo--앵커-축은-해소-2026-09-12) 과 같은 자리다 |
| **Anchor 를 Company 밖으로** (Event·Person·Organization·Product) | 해소기 신설 + Neo4j 이름 인덱스가 필요하다 → **[§6-0 A-8](#6-0-지금-기준--여기부터-읽으세요-2026-09-02) 로 올렸다**(2026-09-05) |
...
| Conversation | 코드에 개념이 0 이고 소유권(Backend 인가 우리인가)이 미정이다 → **소유권은 백엔드로 닫혔다**(2026-09-05). 남은 계약은 §6-0 E |
...
→ [§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo--앵커-축은-해소-2026-09-12) 은 `41bb1bb` 로
...
| ★**`workspace_keys` 의 `key` 규약** | `corp_code` 가 없는 기업은 어떤 문자열로 오는가. 그 값이 우리 `normalize_company_name()` 출력과 **같은가** → [설계서 §16-1](BizNode_Search_설계.md#16-1-key-는-corp_code-또는-norm_name-이다) |
...
| 날짜 | 변경 | 왜 |
|---|---|---|
| 2026-09-13 | **답변 품질 첫 채점 표본 2건을 §5-11 에 기록**(코드 무수정) | A-9 를 손으로 돌려 본 답변 둘을 사건 노드·근거 원문과 전건 대조했다 — 「LG전자 최근 리스크」 3/3 · 「LG전자와 삼성전자 중 누가 더 위험해」 7/7 일치, 지어낸 것·오귀속 0. 뒤엣것은 A-9 전에는 삼성 쪽 4건이 나올 수 없던 답이다. ★비교 질의가 **판정을 안 낸다** — 프롬프트를 읽으니 금지도 요구도 없고, 「근거 결합 금지」·「[사실]에 계산된 결과가 있을 때만」·「최종 검증 1·4」가 사실상 판정을 막는다. [사실] 블록에 기업 단위 집계가 없어서다. 그대로 두기 · 기업별 집계를 계산해 싣기 · 프롬프트에 비교 규칙 넣기 셋을 `[DECIDE]` 로 적었고 셋째 먼저를 추천했다 |
| 2026-09-13 | ★**질문이 지목한 대상은 전부 재료 기업이다 — A-9 해소**(§6-0 A-9) | A-7 이 2차 앵커를 세웠는데 **랭킹에만** 걸었다. 히트 갈래는 `companies_from()` 이 히트만 보므로 「삼성전자와 현대차 중 어디가 리스크가 커?」의 사건·근거가 전부 삼성전자 것이었다 — 현대차 사건은 실재하는데도. ★**두 갈래가 이미 어긋나 있었다** — norm_name 폴백 갈래(`anchor_companies`)는 앵커 전부를 쓰고 있었다. `with_named_anchors()` 를 히트 갈래에 붙였다: 앵커가 하나면 무동작(§5-16 의 교환 보존) · 둘 이상이면 빠진 앵커를 **뒤에**(히트 순서가 이긴다) · 상한은 기존 `_MAX_COMPANIES`. 두 입구는 같은 `material_companies()` 라 파리티 자동(실측 3질의 동일). ★실측 — 두 앵커 4건 전부 기업 1→2 · 사건 10→14~20 · 근거 글자 3k→5~7k(§5 의 34k 의 1/5). 단일·앵커리스 7건 중 5건 글자까지 동일, ranking_baseline 18 케이스 수정 전후 차이 동일(그래프 변화뿐). ★**변한 2건은 A-10 이다** — 「현대차」·「네이버」의 별칭 stub 이 2차 앵커라 재료에 들어온다. A-10 의 「엣지 0건 남남」을 실측으로 **정정**했다: 현대차 stub 엣지 41·사건 4, 네이버 stub 엣지 31·사건 2, `company_aliases` 가 자기 자신을 가리킨다. 재료에도 번지므로 A-10 의 급함이 올라갔다. ★별건 — `tests/graph/test_parity.py` 3건이 `d679e87` 이 지운 `batch/audit/ask_graph_parity` 를 import 해 `-m needs_db` 에서 ERROR 다(수정 전에도 같다 · F 에 적었다). **1,169 passed · 44 deselected · 1 xfailed · 실패 0**(신규 4건) |
| 2026-09-12 | ★**비-Company 앵커를 잇는 관계를 링 위로 올린다 — A-8 검토가 드러낸 14/40 을 닫는다**(§5-17 ③) | A-8 을 사용자 시나리오로 검토하다 잡았다. Coverage 기준이 「관계 ≥1」이라 **어떤** 관계든 있으면 통과하는데, 물어본 엣지(「앵커 ↔ 재료 기업」)가 실제로 들어왔는지 세니 **40건 중 26건**뿐이었다 — Company 는 11/11 인데 Person 4/9 · Organization 5/9 · Product 6/11. A-8 이 고친 건 「어느 기업을 볼지」이고 「물어본 그 엣지를 보여주는가」는 §5-17(비-Company 관계가 Ring 2 로 밀린다)에 남아 있었다. ★**§5-17 의 후보 ①②와 다른 지렛대다** — 질의 의도(키워드)가 아니라 **앵커**만 본다. A-7 이 `_RING_BOTH_ANCHOR` 로 「앵커 ↔ 앵커」를 올린 것과 같은 종류의 신호라 `_SHALLOW_KEYWORDS` 오탐 측정이라는 선행 조건이 없다. ★**Company 앵커에는 안 건다** — 앵커가 곧 재료 기업이라 모든 관계가 한쪽 끝에 앵커를 두는데, 그걸 전부 올리면 워크스페이스 링이 통째로 죽는다. 앵커 쪽 끝이 Company 가 아닐 때만 보고, A-7 의 경계(「한쪽만 앵커면 이웃일 뿐」)는 Company 에서 그대로다 — Company 앵커 12건의 관계 목록이 **edge_id 순서까지 동일**하다. ★**`_REL_Q` 를 `_GRAPH_Q` 와 맞췄다** — 상대 끝 key 가 Person 에서 이름이었다. 저장소 안에서 그 쿼리 하나만 그랬고 (`_GRAPH_Q`·`graph_service._QUERY`·GraphSearcher 는 전부 `person_key`), 같은 `relation_row()` 가 먹이는 쿼리에 따라 Person key 를 다르게 냈다. 앵커 key 는 `person_key` 라 이 불일치가 있는 한 링 대조가 **원리적으로 불가능**했다. 기업 10곳 지문에서 edge_id·Company 끝·Organization·Product 끝은 전부 동일하고 Person 끝만 바뀌었다. **계약 값 변경**이라 §E 에 걸었다 — 상세 그래프와 같아진 것이다. ★실측 — 같은 48건에서 앵커 엣지 진입 **26/40 → 38/40**, 바뀐 12건 전부 0→N · 후퇴 0. ★**남은 2건은 오답이다** — 「미국 상무부」가 **「미국」(Organization)** 으로 붙는다. 공백 때문에 덩어리가 안 붙어 '미국' 만 관문을 통과한다(§5-15 잔여 · Brity Works 와 같은 종류). Coverage 는 O 로 셌지만 물은 것과 다른 대상이다. ★**원래의 §5-17 은 그대로다** — Company 앵커 + 워크스페이스에서 Organization 이 밀리는 것은 ③ 이 안 건드린다. ★검토에서 둘을 더 봤다 — 공정위 상위 5 컷이 **동점 한복판**(0.945 가 5·6·7위)이고, 이재용 재료의 첫 자리가 score 1.0 동점에서 우연히 나온 삼성에스디에스(자기 사건 0)다. 둘 다 Deferred 그대로. **1,165 passed · 44 deselected · 1 xfailed · 실패 0**(신규 6건 · needs_db 1) |
| 2026-09-10 | ★**앵커가 Company 하나가 아니게 됐다 — Person·Organization·Product 를 대상으로 답한다**(A-8 Recommended ①②) | 실측이 권고한 둘을 그대로 넣었다. **새 숫자도 새 티어도 만들지 않았다.** ★**전에는 ①a 가 이것을 떨어뜨렸다** — `non_company_labels()` 가 「기업이 아닌 것을 **기업으로** 오인하지 않는다」며 Person·Product 토큰을 빼서, 「이재용 관련 최근 이슈가 뭐야?」가 `anchorless` 로 가 **전역 사건 목록**으로 답했다. 이제 Person 으로 제대로 부르므로 그 오인이 성립하지 않는다. 제외 자체는 **남는다** — Event 이름과 겹치는 토큰을 거르는 일은 그대로다. ★**①b 에 3단을 신설했다** — Company 를 먼저 보고 못 찾았을 때만 온다. 순서가 계약이다(「국민연금」이 Organization 과 Company 국민연금공단으로 갈려 있어 A-10, 여기가 앞이면 재료가 얇은 쪽에 붙는다). `Event` 는 앵커가 될 수 없다 — 사건을 대상으로 삼으면 재료 조립이 다른 이야기가 된다. ★**재료는 앵커에서 한 홉이다**(Path B) — `graph_service.relations_of()` 를 그대로 쓴다. 그쪽이 이미 만료 관계 제외·근거 검증 판정 반영·점수 내림차순을 하므로 사본을 두면 「파급은 근거 없는 엣지를 안 타는데 재료 선정은 탄다」가 된다. 상한은 기존 `_MAX_COMPANIES`. ★**한 줄만 넓혔다** — `graph_service._QUERY` 이름 절에 `person_key`. Person 은 `norm_name` 이 없어(실측) 그게 없으면 재료가 통째로 0 이다. 값 공간이 겹치지 않아 기존 호출은 무변화다. ★**계약 하나가 늘었다** — `Anchor.label`. 「이 답이 Person 이재용에 대한 것」임을 응답이 말할 수 있어야 A-6 과 같은 종류의 오답을 소비자가 구별한다. 기본값 `Company`·`required` 아님이고 OpenAPI 대조에서 바뀐 스키마는 `Anchor` 하나·경로는 전부 동일하다. ★**실측 — 같은 48건을 다시 돌렸다.** `anchor_source` query 14 → 35 · anchorless 32 → 11. **Query Coverage 23/48 → 36/48**, 바뀐 13건이 **전부 X→O 이고 후퇴 0건**이다. ★**기준선이 17 이 아니라 23 인 이유를 적어 둔다** — 17 은 「재료에 앵커 자신이 나타난다」(ⓑ)를 함께 요구한 값인데 ⓑ 는 비-Company 앵커에 **원리적으로 성립하지 않는다**(앵커가 Person 이고 재료는 Company 다). 한쪽에만 게이트를 걸면 개선이 부풀려지므로 양쪽에 적합도 게이트만 걸었다. ★**게이트를 빼면 44 → 43 으로 오히려 준다** — 앵커리스가 전역 사건 10건으로 EVENT 를 통과하던 것을 성공으로 세기 때문이다. §9 의 「traversal 가능 = 성공으로 판정하지 않는다」가 여기서 실제로 물었다. ★**남은 12건의 원인은 넷이다** — 토큰 관문(NNG) 5 · §5-15 잔여 토큰 3 · 「자기 사건 0」 Path C 폴백 2(문무일의 1홉이 삼성에스디에스 한 곳이라 Company 쪽 결함을 그대로 물려받는다) · A-10 별칭 2. CANDIDATE 45/48 에 못 미친 9건 중 **8건이 토큰과 폴백**이라 §11 이 「45/48 은 토큰이 온다고 가정했을 때의 상한」이라 적어 둔 그대로다. ★**단계 5 도 같은 날 넣었다** — 「일반명사 덩어리도 후보로 올린다」. Kiwi 가 기관·제품 이름을 자주 NNG 로 줘서 공정거래위원회·고용노동부·낸드플래시가 **후보에 오르지도 못했다.** 관문을 그냥 풀면 앵커리스가 죽으므로(§5-15 에서 12/12 실측) **어디서만 푸는가**를 규칙 다섯으로 재서 골랐다 — 질의 24건에서 NNG 전부는 오탐 6/16, 기업 이웃 ≥1 은 2/16, **기업 이웃 ≥2 가 오탐 0/16 · 정탐 7/8** 이다. ★가르는 신호는 **재료를 낼 수 있는가**다 — 「반도체」는 기업 0곳 · 「메모리」는 1곳인데 낸드플래시는 7곳 · 공정위는 76곳이다. 새 랭킹 티어나 유사도 임계값이 아니라 **구조적 자격 조건**이고(`MIN_ANCHOR_COMPANIES`), **고유명사 덩어리에는 안 걸어** 단계 3 에서 이미 나가던 앵커는 무영향이다(문무일은 기업 1곳인데 그대로 앵커다). 못 연 하나는 「국민연금」인데 A-10(별칭이 별도 노드)이라 **안 여는 것이 맞다.** ★**Query Coverage 23/48 → 36/48(①②) → 40/48(단계 5)**, 앵커리스 16/16 보존. ★남은 8건은 토큰 3 · A-10 3 · Path C 폴백 2 다. ⚠**공정위의 적합도 1.0 은 곧이곧대로 읽으면 안 된다** — §5 가 「공정위 폐포가 전역의 48%」라 적어 뒀고, 76곳을 상위 5로 자른 결과가 맞는지는 이 지표로 못 가린다. **허브 절단 규칙은 여전히 Deferred** 다. ★**Event 앵커는 A-13 으로 갈라 냈다** — 최종 설계 §2.3 이 다섯을 정의하는데 A-8 이 넷만 열었다. 안 쟀고(매트릭스가 4×4다) · 재료 조립의 방향이 반대이고 · Event 이름이 다른 노드와 정확히 겹치는 것이 5건(그중 하나가 Company)이라 **A-8 의 확장이 아니라 별도 항목**이다. **1,160 passed · 43 deselected · 1 xfailed · 실패 0**(신규 19건) |
| 2026-09-10 | ★**사명을 조각이 아니라 덩어리로 본다 — §5-15 해소, 그리고 A-7 의 숨은 오답이 함께 드러났다** | 「토큰 병합·구간 커버리지 중 무엇이 맞는지 재 본 적이 없다」가 이 항목이 2주 넘게 열려 있던 이유였다. **질의 758건**으로 규칙 다섯을 같은 코퍼스에 걸었다 — 그래프 Company 3,451곳에서 뽑은 실존 사명 58곳 × 질의틀 7(★띄어쓰기·조사를 일부러 갈랐다) + 합성 사명 348 + 허구 4. ★**두 후보가 실측으로 탈락했다** — 조각 폴백(R2)은 정답률이 가장 높지만 **오탐이 하나도 안 줄고**(35.6% 그대로) 그게 §5-15 자체다. 구간 커버리지(R3)는 「SK하이닉스에」에서 조각 어느 것도 덩어리를 못 덮어 **조사 붙은 사명이 통째로 죽는다**(정답률 57.1%). ★**병합만으로도 안 됐다** — 덩어리를 그냥 얹었더니 「최근」·「업계」·「가격」이 대상이 되어 **앵커 없는 질의 12건이 12건 다 `unresolved`** 로 뒤집혔다. Global Event Search 가 통째로 막힌다. ★**채택한 것은 「고유명사를 품은 덩어리만」**이다. 앵커리스가 보존되는 근거가 여기 있다 — 덩어리는 고유명사를 품어야 남으므로 **「고유명사가 없다」가 전과 같은 뜻**이고, 질의 774건 전수에서 **공집합 동치가 한 건도 안 어긋났다.** ★**붙이는 규칙은 한 벌로 뒀다** — `token_overlap._merge_adjacent()` 가 이미 이 문제를 위해 있었는데 **글자만** 돌려주어 품사로 거를 수가 없었다. `merged_spans()` 로 품사를 함께 내보내고 `_merge_adjacent()` 를 그 위에 다시 세웠다. `sentence_tokens` 와 `_GROUND_THRESHOLD = 0.34` 가 그 동작에 묶여 있어 **두 출력을 마주 세우는 시험**을 걸었다. ★**곁가지 — A-7 이 절반만 맞았다.** 비교 질의의 **2차 앵커가 SK주식회사**였다(「삼성전자와 SK하이닉스는 무슨 관계야?」). `_second_anchor` 의 부분 문자열 차단이 **1차 앵커 이름의 조각**만 보기 때문인데, 덩어리로 보면 애초에 안 쪼개진다 — 3건이 함께 고쳐졌다. ★실측 — ①b **2단으로 내려오는 539건에서 오탐 121 → 12**, 정답 111 → 110(미탐 +1). 앵커리스 12/12 유지, 「명시했으나 못 찾음」 4건 전부 그대로, 바뀐 판정은 **TSMC 합성 하나**다. ★**남긴 것** — 붙여 쓴 「사명 + 일반명사」(「삼성전자주가」)는 합성 사명과 구별할 신호가 없어 미탐으로 둔다. 잔여 길이 임계값은 실측 없는 값이라 얹지 않았다. 이 형태 58건 중 **26건은 ①b 1단(`corp_code`)이 흡수**한다. ★**A-8 의 선행 조건은 반만 닫혔다** — 공정거래위원회·낸드플래시류 B 분류 11건은 원인이 **쪼개짐이 아니라 품사**라 그대로다. 덩어리는 이제 온전한데 NNP·SL 관문이 거르고, **그 관문을 풀면 앵커리스가 죽는다**(12/12 실측). 비-Company 앵커에서만 NNG 덩어리를 보는 것은 A-8 의 설계 문제로 넘긴다. ★랭킹 기준선은 무영향이다 — `ranking_baseline.py` 가 `query_understanding` 을 **한 번도 부르지 않고**(import 0) 기업 key 를 직접 받아, OpenAI 임베딩을 태워 다시 뜨지 않았다. **1,143 passed · 43 deselected · 1 xfailed · 실패 0**(신규 6건 · `xfail(strict)` 1건을 정상 시험으로 뒤집음) |
| 2026-09-10 | ★**Agent 가 그래프를 동적으로 탐색하는가를 코드로 되짚었다 — 탐색은 없었다**(A-12 · 코드 무수정) | 「Agentic GraphRAG 를 실제로 구현하고 있나」를 설계 의도가 아니라 **호출 관계로만** 판정했다. ★**차단 지점이 한 곳이고 그것이 전부다** — `state["companies"]` 를 쓰는 노드가 `plan_material` 하나뿐인데 `run_tools` 는 매 턴 그 값으로 범위를 다시 세운다(`agent_loop.py:246→:128`). 「초기 scope」가 아니라 **요청이 끝날 때까지 고정**이고, 도구 7종이 전부 `keys.resolved()→scope.check()` 라는 같은 깔때기를 지나 우회로가 없다. ★**실측** — 스텁으로 재현했더니 `get_relations(삼성전자)` 가 `target_key=00164779` 를 **보여주는데** `get_relations(SK하이닉스)` 는 `재료 범위 밖의 key` 로 거부되고 재료도 안 는다. 보여주면서 막는 것이 의도이고 `tests/graph/test_agent_loop.py:306` 이 그 행동을 불변식으로 묶어 뒀다. ★**정확한 이름은 Fixed-Frontier Agentic Retrieval** — 「무엇을 물을지」는 동적(도구·순서·횟수·종료·질의문)이고 「누구에게 물을지」는 완전 정적이다. Graph-as-Retriever 이지 Graph-as-Agent-Environment 가 아니다. ★**파급이 걷는 그래프는 2홉·엣지 2종**(`IMPACTS`→`SUPPLIES_TO`)이라 `get_relations` 가 보는 10종(A-11)과 다르고, `event_impact(max_hops=3)` 의 3 은 **죽은 슬랙**이다(`propagate_risk` 가 2를 넘는 hops 를 안 만든다). ★**코드가 이 확장을 위해 자리를 셋 비워 뒀다** — `budget.MAX_HOPS=6`·`hops_used`(아무도 안 가산) · `provenance` 의 `explored`(만드는 코드 없음) · `test_provenance.py:87`(★`dto.py` 밖에 `explored` 가 나타나면 실패하는 트립와이어). 주석이 `explore_impact 는 2-B` 라고 이름까지 박아 뒀다 — **미구현이지 미설계가 아니다.** ★**판정은 「지금 넣지 않는다」** — 평가셋 23 케이스 중 A→B→C 확장을 요구하는 것이 **0건**이고, 최종 설계의 가장 복잡한 시나리오조차 흐름에 traversal 이 없다. 지금 넣으면 **계약 6(파리티)을 깨면서 23건 중 0건을 개선**한다. 필요한 것은 탐색권이 아니라 **프론티어 확장**이고 그건 A-8 의 몫이다. ★**추가한다면 4곳**(`explore_impact` 신설 · `_EXPOSED` 등록 · `run_tools` 승격 · `_scope_of` 한 줄)이고 `budget`·`scope`·`citation`·`evidence_validation` 은 무수정이다. 유일한 실제 비용인 파리티는 대조를 `provenance == "direct"` 로 한정해 푼다 — **그 필드가 존재하는 이유가 정확히 이것이다.** 착수 조건은 §5-15 → A-8 → A-9 뒤 재측정. ★**낡은 행 둘을 함께 정정했다** — §1-3 의 「Agent Tool 연동 🔴 없음」과 §6-3 의 같은 항목 `[TODO]` 는 **Phase 2(2026-08-28)에 이미 닫힌 것**인데 A-12 와 정면으로 어긋나 있었다. **코드·테스트 무수정 · 문서만 변경 · 관련 시험 79 passed** |
| 2026-09-06 | ★**재료 관계 화이트리스트를 열었다 — 임원·제품 질의가 처음으로 답을 얻는다**(A-11) | ★**앵커 문제가 아니었다.** `company_service._REL_TYPES` 가 8종만 담아 `IS_EXECUTIVE_OF`(787)·`DEVELOPS`(2,050)가 **어느 경로로도 재료에 못 들어왔다** — 엣지 11,168 중 2,837(25.4%). 앵커가 완벽히 잡힌 「삼성전자 임원이 누구야?」가 `OWNS_STAKE_IN ×10` 을 냈다. ★**계약을 열거에서 여집합으로 바꿨다** — 「`EdgeType` 에서 사건 축 둘을 뺀 전부」. 열거로 두면 새 유형이 생겨도 아무도 여기를 안 고치고, 이번이 바로 그렇게 된 경우다. ★**고른 근거가 넷** — ⓐ `company_graph` 는 이미 그 둘을 그리고 있어 「목록과 그림은 같은 목록」 불변식이 그 두 유형에서만 깨져 있었다 ⓑ `EdgeType`·`NodeLabel` 이 이미 받아 **스키마 변경 0 · OpenAPI 지문 동일** ⓒ `graph_tools._caution_of` 의 `DEVELOPS` 분기가 **한 번도 안 돌던 죽은 코드**였다(시험은 stub 으로 초록이었다) ⓓ 품질이 이미 편입된 유형과 같다(DEVELOPS 85.3% vs SUPPLIES_TO 85.8%). ★실측 — 기업 10곳 지문 대조에서 **사라진 edge_id 0**(추가만), 상세 그래프 무변화, 목록이 그래프가 그린 관계와 정확히 일치하게 됐다. `/retrieve` 12질의 중 관계 무변화 7 · 고쳐짐 4 · 늘어남 1이고 **사건은 12/12 동일**하다. ★**남은 것은 링이다** — 워크스페이스가 있으면 Ring 1 이 상한을 다 먹어 여전히 0/10 이다. 그건 §5-17 의 몫이라 함께 고치지 않았고, 대신 §5-17 에 「원인이 둘이었다」를 정정해 뒀다. **1,111 passed · needs_db 5 · 2 xfailed · 실패 0** |
...
| 2026-09-05 | ★**기능·코드를 훑어 남은 작업을 다시 셌다 — 새로 4건, 목록에서 빠져 있던 것 2건**(코드 무수정) | A-1·A-7 외에 무엇이 남았나를 코드로 확인했다. ★**새로 찾은 것 넷** — ① `logs/queries.jsonl` 4,192행 중 trace_id 보유 3,369행인데 상위 질문이 `q` 1,727 · `x` 195 라 **B-1 의 전제가 이미 깨져 있다**(테스트가 같은 파일에 쓰고 `BIZNODE_QUERY_LOG` 를 끄는 곳이 0곳이다). B-1·B-3·strip 결정 셋의 **선행 조건**이라 §6-0 B-1 에 적었다 ② Chroma 가 없으면 `main.py:106` 모듈 전역 조립이 터져 **앱 기동도 테스트 수집도 중단**된다(1,078/1,116 수집 후 `Interrupted`) ③ `/health` 가 의존성을 하나도 안 본다 ④ README §8 이 규정한 매일 배치의 스케줄러 정의가 저장소에 0곳. ②~④ 를 **F 절**로 신설했다. ★**목록에서 빠져 있던 것 둘** — 앵커가 Company 하나뿐인 것(최종 설계 §2.3·§17.2)과 Conversation 이다. 둘 다 §8-22 뒤풀이 표에 「이번 범위 밖」으로만 있고 **§6-0「지금 기준」에는 없었다**. 앞의 것은 **A-8** 로 올렸다 — `non_company_labels()` 가 Person 토큰을 떨어뜨려 「이재용 관련 이슈」가 조용히 anchorless 로 흐르는, **A-6 과 같은 종류**의 탐지 불가능한 오답이고, A-7 과 **같은 반환 구조**를 고치는 수술이라 함께 연다. 뒤의 것은 **저장을 백엔드가 맡기로 결정**돼 소유권이 닫혔고, 남은 계약(`/ask` 가 이전 대화를 받는가)만 E 표에 남겼다. ★**규모 미측정** — Docker 가 이 WSL 에서 안 잡혀(`docker: command not found`) 실 DB 조회를 못 했다. A-8 은 코드 경로로만 확정했다. ★곁가지로 30초 요약이 **해소된 A-2 를 아직 🔴 로** 달고 있어 고쳤다 |
| 2026-09-05 | ★**합본을 편 경계에서 세 파일로 갈랐다 — 현황은 계층별로 안 가른다**(코드 무수정) | 같은 날 설계·현황·Agent 를 한 파일로 합쳤다가 **7,231줄 / 587KB** 가 됐다. 갱신 주기가 다른 셋(설계·Agent 설계는 저빈도, 현황은 매 작업)을 한 파일에 둔 대가라 편 경계에서 다시 갈랐다: `BizNode_Search_설계.md`(1,860줄) · `BizNode_Agent_설계.md`(1,379줄) · `BizNode_검색챗봇_현황.md`(3,978줄). ★**넷으로 가르지 않은 이유 — 현황은 계층별로 안 갈라진다.** 현황 14개 절 중 **7개가 두 계층 심볼이 섞인 절**이고 그것이 **3,386줄 / 3,946줄 = 86%** 다. 결정적으로 **Agent 전용 절이 0개**라 「Agent 현황」을 만들면 통째로 옮길 절이 하나도 없다 — 섞인 절에서 문장 단위로 뜯어내는 것은 분할이 아니라 재집필이다. §12 는 97줄 중 **5줄이 한 줄 안에서 두 계층을 함께 기록**한다(예: 09-02 Global Event Search 는 `evidence_selector`·`AnchorExtractor` 와 `ask_graph` 를 같이 건드렸다) — **한 커밋이 두 계층을 같이 고치기 때문**이고, 가르면 같은 항목이 양쪽에 나뉜다. 이 저장소가 2026-08-23 에 이미 밟은 함정이다. ★**검증에서 내 기준선이 틀렸던 것을 잡았다** — 처음엔 분할 후 숫자끼리 비교해 「새로 깨뜨린 것 0」이라고 읽었는데, 편 헤딩을 없애면서 `#설계편--…`·`#현황편--…` 링크 **12개가 실제로 깨져** 있었다. 저장소 이력에서 합본을 꺼내 기준선으로 삼아 다시 재고 그 12개를 형제 파일 링크로 돌렸다. 고친 뒤 **합본 17건/11종 → 분할 후 17건/11종 · 새로 깨뜨린 것 0**. 남은 11종은 이 세션 이전부터 있던 것(제목이 바뀌었는데 링크가 안 따라간 자리)으로 범위 밖이다. ★**되돌아온 것** — 편을 넘나들던 내부 링크 **93개**가 다시 파일 간 링크가 됐다. 합쳤던 이유가 그것이었으므로 대가를 정확히 되돌려 받은 셈이다. 대신 각 파일이 **1,400~4,000줄**로 읽을 만해졌다. **docs 12 → 14개**(이 세션 시작 시 16개) |
...
| 2026-09-05 | ★**A-3 해소 — 판촉 「이벤트」를 사업 사건으로 읽던 분류를 고쳤다** | 「최근 주요 투자 이벤트」 상위 10건 중 3자리를 판촉이 먹고 있었다(신규 계좌 개설 이벤트 · 100만명 돌파 기념 포인트 적립 이벤트 ×2). 데이터로는 `사업확장` 448건 중 **3건(0.7%)** 인데 최근에 몰려 있어 결과에서 **40배로 증폭**됐다. ★뿌리는 `batch/repair/event_types.py` 의 분류 프롬프트다 — 한국어 「이벤트」가 사건과 판촉 둘 다 뜻하는데 정의(증설·착공·수주·투자 결정)가 그것을 안 갈랐다. **데이터만 고치면 재발한다**(새 사건은 같은 프롬프트가 다시 분류한다). 이 파일의 관례대로 판촉 반례를 프롬프트에 박고, 이미 분류된 것만 다시 묻는 `--event-id` 를 신설해 3건을 되돌렸다 — `--full` 은 1,000건을 다시 불러 멀쩡한 라벨까지 흔든다. ★**과잉 적용 없음 실측**: 진짜 사업확장 12건을 옛/새 프롬프트로 대조해 바뀐 것 0건(달라 보인 1건은 4회씩 재시행에서 양쪽 다 「기타」로 일치 — LLM 흔들림). ★결과: 상위 10건에서 판촉 0건, 빈자리를 D램 생산 투자·유일로보틱스 북미 공략·제주 스타트업 펀드가 채웠다. 판촉 낱말 12종으로 전수 조사해 남은 4건은 전부 `기타` |
| 2026-09-05 | ★**A-6 해소 — 재료 선정을 한 곳으로 모으고 죽은 입구를 지웠다** | 같은 질문이 입구에 따라 다른 재료를 냈다. ★**두 경로가 다르게 판단한 게 아니다** — `hits_reflect_the_anchor()` 를 이미 공유하고 있었고 `/retrieve` 만 그 답을 안 썼다(`use_hits=True` 고정). 같은 분기가 **세 벌**이었고 그중 `retrieve_for_ask()` 는 `/ask` 가 LangGraph 로 넘어간 뒤 **운영에서 아무도 안 부르는 죽은 입구**였는데 테스트 32곳이 그리로 살아 있는 로직을 보고 있었다. **합치고 나서 지웠다** — 반대 순서면 그 32곳이 갈 곳을 잃는다. `material_companies()` 를 뽑아 `_assemble` 과 `plan_material` 이 함께 쓰고, 앵커리스에서 고른 사건을 그대로 넘겨 두 입구가 다른 사건을 고르지 않게 했다. 「unresolved 면 재료를 안 만든다」는 그래프 조건부 엣지가 이미 보고 있어 잃는 것이 없다. ★**실측: 다섯 질문에서 두 입구가 전부 같은 재료**를 낸다. 기업 상세 화면의 「이 회사 리스크 어때?」가 삼성에스디에스·원익홀딩스에서 삼성전자로, TSMC 가 LX세미콘·코미코에서 TSMC 로 돌아왔고 나머지 셋은 무변경. ★`SEMANTIC` 은 안 죽는다 — `match_type` 은 그대로 나간다. ★**백엔드 통보 필요** |
...
| 2026-08-26 | **신규 결함 3건** — 부분 토큰 오탐([§5-15](#5-15-사명의-부분-토큰이-실존-기업으로-해소된다--해소-2026-09-10)) · 비-Company 엣지 재료 0([§5-16](#5-16-비-company-엣지-질의는-재료가-0-이-된다--해소-2026-08-26), **해소**) · Ring 2 잘림([§5-17](#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo--앵커-축은-해소-2026-09-12)) | §5-16 만 고쳤다. 나머지 둘은 **표시만** — 규칙을 얹으려면 실측이 먼저다. §5-15 는 `xfail(strict=True)` 회귀 케이스로 고정 |
...
| 2026-08-25 | ★**정정 — `workspace_id` 동기화를 도입하지 않는다. `workspace_keys` 계약 유지** (문서만) | 실제 계약을 다시 확인하니 `AskRequest` 는 `{question, workspace_keys}` 이고 `workspace_keys` 는 백엔드가 **요청마다** 주는 **워크스페이스 기업 `corp_code` 배열**이었다. 워크스페이스 동기화 API·AI 쪽 상태 저장·push/pull 재동기를 **전부 걷어냈다.** 그날 확정한 **A-3(링)·B-2(unresolved)·C-3(anchor_source 답변 형태)·E(corp_code → norm_name)는 그대로 유지**한다 → [설계서 §3](BizNode_Search_설계.md#ask-는-workspace_keys-를-받는다-2026-08-25-정정)·[§16](BizNode_Search_설계.md#16-워크스페이스-기업-식별--workspace_keys) |
