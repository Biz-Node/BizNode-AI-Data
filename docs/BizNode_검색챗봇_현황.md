...
앵커 확장 ───────────── 🟡 반만 해소   2026-09-10. Person·Organization·Product 를 대상으로
   └ 재료 경로 ──────── 완료          앵커 → 1홉 기업(Path B). Coverage 23/48 → 36/48. §6-0 A-8
   └ 토큰 관문 ──────── 🔴 남음        Kiwi 가 기관·제품을 NNG 로 준다 — 후보에 안 온다. §13④
...
④ Kiwi 가 기관·제품 이름을 고유명사로 안 준다
   ...
   ★**2026-09-10 — 반만 닫혔다.** §5-15 가 후보를 덩어리로 바꿔 「김준성은」은
     ...
     앵커리스가 죽는다**(실측 12/12 가 `unresolved`). 관문을 푸는 것이 아니라
     **비-Company 앵커에서만 NNG 덩어리를 보는 것**이 A-8 의 설계 문제다.
```
...
```text
토큰 관문 (§13④ · Kiwi 가 NNG 로 준다)        5건  공정거래위원회 ×2 · 낸드플래시 ×2 · 국민연금 ×1
...
A-10 별칭 노드 + 측정 아티팩트                 2건  국민연금 ×2 — 2단이 **Company 국민연금공단**
                                                 (00706742)으로 붙는데, 적합도는 Organization
                                                 국민연금(엣지 3)의 폐포로 재서 0.0 이 나온다
```

★**CANDIDATE 45/48 에 못 미치는 것이 예상대로입니다.** §11 이 「45/48 은 **토큰이 온다고
가정했을 때**의 상한」이라고 적어 뒀고, 실제로 못 미친 9건 중 **8건이 토큰과 폴백**입니다.

★**단계 5 를 분리했습니다** — 「비-Company 앵커를 찾을 때만 NNG 덩어리도 본다」(§13④).
관문을 그냥 풀면 앵커리스가 죽는 것이 §5-15 에서 12/12 로 실측됐으므로, **어디서만
푸는가**를 재고 나서 넣습니다.
