...
> 지금 어디까지 됐는지·무엇이 고장났는지는 [현황](BizNode_검색챗봇_현황.md) 을 보세요.
> `/ask` 안의 실행 그래프는 [Agent 설계](BizNode_Agent_설계.md) 입니다.
>
...
마지막 갱신 마지막 갱신 **2026-09-05** · 기준 커밋 `6c39289`(브랜치 `yun`)

> 📌 **파일 이름이 `Search` 지만 내용은 검색 + 챗봇 둘 다입니다.** 챗봇(`/ask`)이
> 같은 파이프라인 위에 얹혀 있어 따로 떼면 경계가 안 보입니다.

> ★**본문 안의 「1부 — 검색」·「2부 — 재료」·「3부 — 챗봇」은 이 문서 내부의
> 구분**입니다.
...
| 당신이 | 이 순서로 |
|---|---|
| **백엔드 개발자** — API 를 붙여야 한다 | [1절](#1-큰-그림) → [현황서 §3 계약](BizNode_검색챗봇_현황.md#3-백엔드프론트가-알아야-할-것) → [15절](#15-답변-규약--인젝션-방어--실패-처리) |
| **검색 품질을 만질 사람** | [1절](#1-큰-그림) → [1부 전체](#1부--검색-무엇이-관련-있는가) → [현황서 §4 결함](BizNode_검색챗봇_현황.md#4-알려진-결함--검색) |
| **챗봇/프롬프트를 만질 사람** | [1절](#1-큰-그림) → [3부 전체](#3부--챗봇-답변을-쓴다) → [현황서 §5 결함](BizNode_검색챗봇_현황.md#5-알려진-결함--챗봇) |
| **다음에 뭘 할지 정해야 하는 사람** | [현황서 §6 개발 순서](BizNode_검색챗봇_현황.md#6-다음-개발-순서) → [현황서 §7 결정 대기](BizNode_검색챗봇_현황.md#7-결정해야-할-것--decide) |
...
구조는 [Agent 아키텍처 §5](BizNode_Agent_설계.md) 를 보세요.
...
| 5 | **실측 근거 없는 숫자를 새로 만들지 않는다** | 잠정치는 잠정치라고 적고 [현황서 §9](BizNode_검색챗봇_현황.md#9-실측-근거-없는-잠정치)에 모은다 |
...
있습니다. 41건 전수 검증은
[현황서 §8-7](BizNode_검색챗봇_현황.md#8-7-앵커가-재료를-정하는가--41건-전수-2026-08-26).

> ⚠ **알려진 빈틈** — 상대 노드가 답인 질의(「삼성전자를 규제한 기관」)에서 그 상대가
> **Ring 2 로 밀려 상한에서 잘립니다**
> ([현황서 §5-17](BizNode_검색챗봇_현황.md#5-17-비-company-관계가-ring-2-로-밀려-상한에서-잘린다-todo)).
...
> 선행 측정은 [현황서 §6-3](BizNode_검색챗봇_현황.md#6-3-병행-가능한-독립-작업) 의
> **N+1 실측**과 **prompt token 크기** 입니다 — 둘 다 이미 `[MEASURE]` 로 올라가 있고,
> **이제 그 둘이 이 결정의 선행 조건입니다**
> ([현황서 §7-4](BizNode_검색챗봇_현황.md#7-4-2026-08-25-개정이-새로-연-decide-2건)).
...
**실측 사례 둘** — [현황서 §5-12](BizNode_검색챗봇_현황.md#5-12-event-라벨이-근거와-정반대인데-낱말-겹침이-못-잡는다-todo)·[§5-14](BizNode_검색챗봇_현황.md#5-14-기사의-배경절이-event-라벨이-된다-추출-파이프라인-measure).
...
**1차 정책을 뒤집었습니다**([현황서 §8-8](BizNode_검색챗봇_현황.md#8-8-재료-정합성-규칙-실측-2026-08-26)).
...
[현황서 §5-14](BizNode_검색챗봇_현황.md#5-14-기사의-배경절이-event-라벨이-된다-추출-파이프라인-measure) 의
...
([현황서 §7](BizNode_검색챗봇_현황.md#7-결정해야-할-것--decide)). 전체 규모 측정은
[현황서 §6-3](BizNode_검색챗봇_현황.md#6-3-병행-가능한-독립-작업) 의 **B2-2** 항목입니다.
...
[현황서 §6-2](BizNode_검색챗봇_현황.md#6-2-순서--목적에서-역산한-6단계) ⑥ 뒤입니다.
...
**발생률과 오탐률**을 잰 뒤에 정합니다([현황서 §5-18](BizNode_검색챗봇_현황.md#5-18-claim-5종-어디에도-안-걸리는-주장이-있다--6번째-유형-신설-2026-08-26)).
...
| **ⓒ query anchor 는 있는데 못 뽑음** | 「NAVER에 …」 (별칭·조사 문제) | 앵커 해소 실패 | `SEMANTIC` | **다른 문제다** — [현황서 §4](BizNode_검색챗봇_현황.md#4-알려진-결함--검색) `AnchorExtractor` 항목 |
...
([현황서 §8-5](BizNode_검색챗봇_현황.md#8-5-anchor_source-판정-신호-2026-08-25)).
...
[현황서 §4-5](BizNode_검색챗봇_현황.md#4-5-동음이의-사명은-원리적으로-못-가른다)
동음이의. **규칙을 더 얹지 않았습니다** — 실측으로 확인한 것만 둡니다.

★**`anchor_source` 는 서버가 아는 결정론적 값입니다.** 이것이
[현황서 §5-8](BizNode_검색챗봇_현황.md#5-8-askresponse-에-match_type-이-없어-결정론적-백스톱이-없다-todo)
이 없다고 지적한 **백스톱**입니다.
...
> ★**「`/retrieve` 무변경」은 2026-09-05 에 끝났습니다** (현황서 §6-0 A-6).
>
> 위 표의 **검색 분기**는 그대로입니다 — `match_type` 은 여전히 `SEMANTIC` 으로
> 나가고 enum 도 살아 있습니다. 끝난 것은 **「의미 유사 기업을 재료로도 쓴다」**
> 쪽입니다. 그 조항 때문에 같은 질문이 입구에 따라 다른 재료를 냈고, 특히 기업
> 상세 화면의 「이 회사 리스크 어때?」가 `/retrieve` 에서 **묻지 않은 기업들**로
> 답을 받았습니다.
>
> 재료 기업 선정은 이제 `retrieve_service.material_companies()` **한 곳**이고,
> `/retrieve` 와 `/ask` 그래프가 그것을 공유합니다. 판정 함수
> (`hits_reflect_the_anchor`)는 **처음부터 공유하고 있었고** `/retrieve` 만 그
> 답을 쓰지 않던 것이었습니다.

★**[현황서 §5-9](BizNode_검색챗봇_현황.md#5-9-시스템-프롬프트의-semantic-설명이-실제-동작과-어긋난다-todo)
...
| **[현황서 §4-5](BizNode_검색챗봇_현황.md#4-5-동음이의-사명은-원리적으로-못-가른다) 동음이의** 「대상」·「동남」 | 원리적으로 못 가름 | 워크스페이스에 「대상」이 있으면 **해소 후보의 prior** 로 쓴다. ★hard filter 가 아니다 |
...
[현황서 §4-3·§4-6](BizNode_검색챗봇_현황.md#4-3-anchor-에-조사가-붙어-나오는-경우가-남았다--해소-2026-08-23) 수정으로
ⓒ(못 뽑음)가 크게 줄었습니다.
...
> 입니다. 현재 노드 배선은
> [Agent 아키텍처 §5](BizNode_Agent_설계.md), 폐기 경위는
> [현황서 §12](BizNode_검색챗봇_현황.md) 2026-09-04 항목입니다.
...
([현황서 §5-16](BizNode_검색챗봇_현황.md#5-16-비-company-엣지-질의는-재료가-0-이-된다--해소-2026-08-26)).
...
| 문서 | 무엇이 있나 |
|---|---|
| [현황서](BizNode_검색챗봇_현황.md) | **짝 문서.** 어디까지 됐나 · 무엇이 고장났나 · 다음에 무엇을 하나 · 결정 대기 목록 |
