...
★**`propagations_used` 의 303 은 계측 결함의 값입니다** — 2026-08-29(Phase 10)에 세는 단위를 사건 수로 고쳤습니다. **④ 재측정(2026-08-29)에서 최대 12 로 확인**됐고 「상한을 넘긴 카운터」 경고도 사라졌습니다. 같은 실행에서 `budget_exhausted` 플래그는 **9/20 → 0/20**(Phase 12 — 소진 판정에서 제외). 위 ①②③ 수치는 고치기 전의 기록으로 남깁니다(§9-2).
...
| ★**단서 (2026-08-29 · Phase 11)** | 이 **38건은 순수하지 않았습니다.** 당시 `cited_without_ring` 한 통에 ⓐ 관계가 아닌 근거(정상)와 ⓑ 관계인데 되짚기가 끊긴 것(결함)이 섞여 있었습니다(§9-8) |
| ★**재측정 결과 (2026-08-29 · 2회)** | 계수를 가르고 다시 재니 **40건 중 33건(82.5%)** · 2회차 **36건 중 29건(80.6%)** 이고 `cited_relation_without_ring` 은 두 번 다 **0** 입니다 — 즉 **33건은 순수하게 관계가 아닌 근거**입니다. 「인용의 대부분이 관계가 아니다」는 결론은 **그대로**이고, 링 랭킹 개선의 기대 효과 상한도 그대로입니다 |
...
| **재측정 (2026-08-29)** | `cited_relation_without_ring` **0건** — 「관계를 인용했는데 되짚기가 끊긴 것」은 없습니다. ★단 **되찾기의 효과는 귀속하지 못했습니다**(아래 §9-8-2) |
| **상태** | ★**해소** (계수 분리는 유지 · 배열 2차 조회는 Phase 15 에서 제거 — §9-8-1) |

#### 9-8-1. ★배열 근거는 관계에 귀속되지 않습니다 — **한계가 아니라 계약입니다**

엣지의 근거는 단수 `evidence_id` 말고 `evidence_ids` **배열**에도 있습니다. 그런데
`graph_tools._relation_dto:116` 은 **단수만** 싣습니다. 이것을 한때 「되짚기 구멍」으로
보고 관측 계층에서 2차 조회를 넣었다가(Phase 13) **뺐습니다**(Phase 15).

★**`app/tools/dto.py` 의 `RelationDTO.evidence_id` 가 못 박고 있었습니다:**

> "근거 원문의 id. **엣지의 스칼라 `evidence_id` 다** — `evidence_ids` 배열은 여러
> 근거의 합집합이라 **이 관계 하나의 출처가 아니다**"

즉 배열을 안 싣는 것은 결함이 아니라 **결정**입니다. 저장소 전체가 **「수집은 넓게,
귀속은 좁게」** 로 일관돼 있습니다:

| 층 | 배열 | 근거 |
|---|---|---|
| 수집 (검색) | **포함** | `graph_searcher.py:139-150` |
| 수집 (상세) | **포함** | `relation_service.py:117-121` |
| 화이트리스트 진입 | **포함** | `agent_loop.py:275-276` |
| DTO 귀속 | ★**단수만** | `dto.py:110-115` |
| 프롬프트 귀속 | ★**단수만** | `prompt.py:148-162` — 나머지는 `about="미연결"` |
| 응답 귀속 | ★**단수만** | `prompt.py:265-271` → `Source.edge_id` |

★**관측만 배열로 되짚으면 응답과 갈립니다.** 배열 근거가 인용되면 `Source.edge_id` 는
`None` 인데 `cited_rings` 만 링을 붙이게 됩니다. `observe.py` 는 **재기만 하는 자리**이고
재는 대상은 실제로 일어난 일이어야 합니다.

★**그리고 잘린 관계를 오염시킵니다.** `ring_by_edge` 는 자르기 **전**(`by_ring` 전체)을
담으므로 2차 조회는 **잘린 관계**의 링까지 인용 분포에 넣습니다 — 아래 §9-3 의 표에서
「본 것 / kept / 인용」 세 열이 같은 모집단이라는 전제가 깨집니다(kept 0 인데 인용 > 0).

★**별개로 남는 실제 이슈 하나** — 검색 히트가 들고 오는 근거의 **63~78%가 배열
전용**입니다(실측: 「삼성전자 규제 이슈」 45건 중 **35건**). 그것들이 프롬프트에서
`about="미연결"` 로 나갑니다. 고친다면 자리는 **응답 계약**(`_edge_id_for` · `about` ·
`RelationDTO`)이지 관측이 아닙니다. `[DECIDE]`

#### 9-8-2. ★되찾기의 효과를 **귀속하지 못했습니다** (2026-08-29 재측정에서 드러난 구멍)

Phase 13(배열 근거 되짚기)을 넣고 평가셋을 다시 돌렸는데 **관계 인용이 전과 같은 7건**이었습니다.

| | 2026-08-28 | 2026-08-29 |
|---|---:|---:|
| 최종 인용 (관계) | 7 (R0 1 · R1 6) | 7 (R0 **2** · R1 **5**) |
| 링 없는 인용 | 38 | **33** |
| 총 인용 | 45 | **40** |

**총 인용이 45 → 40 으로 달라져 직접 비교가 안 됩니다.** LLM 이 근거를 몇 개 드는지가
실행마다 흔들리기 때문입니다. 그래서 「되찾은 것이 0건」인지 「되찾았는데 총량이 줄어
상쇄됐는지」를 **가를 수가 없습니다.**

★**원인은 되찾은 건수를 세지 않은 것입니다.** `observe.cited_recovered_by_evidence` 를
신설하고 **한 번 더 돌렸습니다.** (★이 계수와 되찾기 자체는 이후 Phase 15 에서 **제거**됐습니다 — §9-8-1)

#### 답 — 되찾기는 **0건**입니다 (2회차 · 29 passed)

| | 1회차 | **2회차** |
|---|---:|---:|
| 도구 호출 총계 | 36 | 35 |
| 링 본 것 / kept / 인용 | 1008 / 110 / 7 | ★**1008 / 110 / 7 (동일)** |
| 링 없는 인용 | 33 | 29 |
| `cited_relation_without_ring` | 0 | **0** |
| ★`cited_recovered_by_evidence` | (계수 없었음) | ★**0** |

★**세 실행 연속 링 수치가 미동도 없습니다**(1008 / 110 / 7). 도구 호출은 35 → 36 → 35
로 흔들리는데도 그렇습니다 — `e6c70f4`(dedup 계측)가 제대로 작동한다는 증거입니다.

#### 왜 0인가 — 네 조건이 **동시에** 성립해야 합니다

되찾기 1건이 나오려면:

| # | 조건 | 실측 |
|---|---|---|
| ① | Agent 가 그 기업으로 `get_relations` 를 부른다 | 20 케이스 중 **3건** |
| ② | 그 배열 근거가 **다른 경로로** 화이트리스트에 들어온다 | 평가셋 범위 되찾기 대상 372건 중 Event 경로로 가능한 것 **14건**. 나머지는 검색 히트 의존 |
| ③ | LLM 이 하필 그 근거를 인용한다 | 케이스당 인용 1~5건 |
| ④ | 그 근거가 kept 관계의 **단수**가 아니다 | 맞으면 부르는 쪽이 이미 되짚습니다 |

★**원문 부재 때문은 아닙니다.** 배열 전용 근거 200건 표본에서 Chroma 원문이 없는 것은
**0건**이었습니다(단수 근거 대조군도 0건). 인용 자체는 가능합니다.

#### ★수치 정정 — 「1,631개」는 상한이었습니다

Phase 13 을 정당화할 때 든 「단수로 못 닿는 근거 1,631개」는 **되찾기 대상의 상한**이지
인용까지 도달하는 수가 아닙니다. 정확히는:

| 층 | 값 |
|---|---:|
| 배열 항목 중 단수와 다른 것 (연 수) | 1,631 |
| 그 distinct | **1,462** |
| 그 중 Event 경로로 화이트리스트에 들어올 수 있는 것 (전 그래프) | **46** |
| 평가셋 워크스페이스 범위의 되찾기 대상 | 372 |
| 그 중 Event 경로로 가능한 것 | **14** |

#### 그럼 Phase 13 은 값어치가 있었나 — ★**아니오. 제거했습니다**(Phase 15)

0건이라서가 **아닙니다.** 코드를 다시 읽어 보니 **계약을 우회한 것**이었습니다(§9-8-1) —
`RelationDTO.evidence_id` 가 「배열은 이 관계 하나의 출처가 아니다」로 못 박고 있었고,
프롬프트·응답 귀속도 모두 단수만 봅니다. 관측만 다르게 두면 **응답과 갈립니다.**

★**canonical 경로는 원래부터 완전했습니다** — `cited_edges ⊆ kept ⊆ by_ring` 이고
`edge_id` 는 `elementId(r)` 라 빈 값이 없어 조회 실패가 **구조적으로 불가능**합니다.
평가셋 실측이 뒷받침합니다: `get_relations` 를 부른 3 케이스에서 인용 8건 중 **7건이
canonical 경로로 링을 받았고**, 못 받은 1건은 사건 근거였습니다.

★**남긴 것** — 되돌린 이유를 계약으로 못 박는 테스트 2건
(`test_array_evidence_is_not_attributed_to_the_edges_ring` ·
`test_a_cut_relations_ring_never_enters_the_citation_distribution`). 다시 넣으려는
사람이 거기서 멈춥니다.

★**이것은 Phase 11 이 고친 것과 같은 종류의 교훈입니다** — 「무엇이 일어났나」를 세지
않으면 효과를 귀속할 수 없습니다. 계측을 고칠 때는 **그 고침 자체의 효과를 재는 계수**도
함께 두어야 합니다.
...
| ② | `EMBED_CACHE_STRICT=1` 로 재측정 | 캐시 빗나감 17건이 실행마다 값을 흔듭니다 | 미착수 — ★**커버리지가 부분적입니다**([§10-7](#10-7-모델-교체-검토에서-실측한-제약-2026-08-29)) |
...
| ⑥ | 동일 실행 **N회 반복**으로 변동폭 확정 | 33~37 이 관측 4회의 범위일 뿐입니다 | ★**완료** — N=5 실측 **35~36**([§10-8](#10-8-n5-베이스라인--줄일-분산이-없다-2026-08-29)) |
| ⑦ | LLM **토큰** 계측 | 비용 추정치를 검증할 방법이 없었습니다 | ★**완료** (`Observation.llm_*`) |
| ⑧ | `uncited` 를 구조화된 값으로 | 로그 한 줄에만 있어 20 케이스를 못 모았습니다 | ★**완료** (`Observation.claims_*`) |

### 10-7. 모델 교체 검토에서 실측한 제약 (2026-08-29)

「Agent 만 `gpt-5.6-luna` 로 올려 도구 선택 분산이 줄었는지 본다 → 줄면 답변을
`terra` 로 올려 uncited 비율을 본다」는 계획을 검토하며, **평가셋을 돌리기 전에**
API 로 직접 확인한 것들입니다. 셋 다 계획을 그대로 실행하면 드러나지 않고
**결과를 오독하게 만드는** 종류입니다.

| # | 실측 | 계획에 미치는 영향 |
|---|---|---|
| ① | 모델 상수가 **하나**였습니다 — `adapter.DEFAULT_MODEL` 을 Agent 루프와 답변 생성이 공유 | 「Agent 만 바꾼다」가 **구조적으로 불가능**했습니다 |
| ② | `gpt-5.6-luna`·`terra`·`sol` 은 **`temperature=0.0` 을 거부**합니다 (`Only the default (1) value is supported`) | 모델을 바꾸면 **temperature 0 → 1 이 강제로 딸려옵니다.** 분산을 재려는데 분산을 **올리는** 변수가 같이 움직입니다 |
| ③ | 같은 모델들이 `/v1/chat/completions` 에서 **function tools + 추론을 함께 못 씁니다** (`use /v1/responses or set reasoning_effort to 'none'`) | 세 번째 변수입니다. 추론을 끄거나(`none`) **전송 경로를 Responses API 로 바꾸거나** 둘 중 하나를 골라야 합니다 |

실측 환경은 핀 그대로입니다 — `openai 1.109.1` · `langchain-openai 1.1.9`.
`reasoning_effort="none"` 과 `use_responses_api=True` **양쪽 다 도구 호출이
동작하는 것까지 확인**했으므로, 라이브러리 업그레이드는 필요 없습니다.

★**그래서 「luna 의 효과」라는 문장은 원리적으로 쓸 수 없습니다.** 정직한 비교
문구는 **「gpt-4o-mini@temp0 vs luna@temp1(추론 none)」이라는 묶음 비교**입니다.
제품 결정으로는 유효하지만, 계획이 말한 「깨끗한 단일 변수 읽기」는 아닙니다.

**`EMBED_CACHE_STRICT=1` 의 커버리지도 부분적입니다.** `_default_embed` 를 타는
경로만 걸립니다:

- `search_news`·`search_dart` 의 질의는 `ChromaStore.query()` → `self.embed()` 로
  **캐시를 우회**합니다. 그 질의문은 **LLM 이 매번 새로 씁니다**
- `claim_check` 은 `EmbeddingCacheMiss` 를 **삼킵니다**(「관측이 답변을 막지
  않는다」). 켜면 실행이 죽는 대신 `semantic_mean` 이 blank 가 됩니다

즉 STRICT 는 노이즈를 없애는 장치가 아니라 **고정된 사용자 질문 쪽 한 갈래를
없애고 나머지를 보이게 하는** 장치입니다. 베이스라인 문구를 그렇게 적어야
하고, 패스마다 `embed_calls`·`misses` 를 함께 읽어야 합니다.
...
### 10-8. N=5 베이스라인 — ★줄일 분산이 없다 (2026-08-29)

`--agent-eval-repeat=5` 로 **같은 코드 5회**를 돌린 결과입니다. 20/20 PASS ·
10분 52초 · `gpt-4o-mini` · `temperature=0.0`.

| 지표 | N=5 실측 |
|---|---|
| 도구 호출 총계 | **35~36** · 평균 35.8 · 편차 **0.45** |
| ★**도구 조합 안정성** | **0.97** (1.00 = 매번 같은 조합) |
| 완전 결정론인 케이스 | **18 / 20** |
| uncited 비율 | 0.0~2.9% · 평균 **2.2%** |
| 임베딩 캐시 빗나감 | 3~10 · 평균 7.2 |
| 입력+출력 토큰 | 256,512~263,147 · 평균 261,390 |

**★첫째 — 옛 근거 `33~37` 은 재현되지 않았습니다.** 동일 코드 5회의 폭은
**1**(편차 0.45)이고, 옛 값의 폭은 4였습니다. 그 4는 Agent 의 변동이 아니라
**그 사이 코드가 바뀐 것이 섞인 값**일 가능성이 큽니다 — 관측 4회가 같은
코드에서 나온 것이 아니었기 때문입니다. 임베딩 드리프트(§4-5)·링 계측기(§4-9)에
이은 **같은 귀속 문제의 세 번째 사례**이고, 이번에는 반복 측정이 그것을
잡았습니다.

**★둘째 — 흔들리는 것은 20 케이스 중 2개뿐입니다.**
...
**★그래서 모델 교체 계획의 전제가 둘 다 깨집니다.**

- **1단계**(luna 로 도구 선택 분산 감소): 노릴 수 있는 폭이 **0.97 → 1.00**,
  3%p 입니다. 그 대가로 `temperature 0 → 1`(분산을 **올리는** 방향)과 추론
  설정 변경이 **강제로 딸려옵니다**(§10-7). 순효과가 음수일 가능성이 큽니다
- **2단계**(terra 로 uncited 개선): uncited 가 이미 평균 **2.2%** 입니다.
  바닥에 붙은 지표라 모델을 올려도 움직일 자리가 없습니다

★**모델을 바꿀 이유가 실측으로 사라졌습니다.** 값이 있는 다음 수는 흔들리는
**두 케이스를 직접 들여다보는 것**(공짜입니다)이거나, 아직 아무도 안 재 본
축 — **답변 내용 품질의 사람 채점**(§5-2 · 현황서 §0 「🟡 미검증」)입니다.

### 10-9. 모델 교체 실험 — 무엇이 확실하고 무엇이 아닌가 (2026-08-29)

사용자 지시로 **실제로 바꿔서 돌렸습니다.** 셋 다 N=1 · 같은 코드 · 같은 DB.

| | Agent/답변 둘 다 `gpt-4o-mini` | 둘 다 `gpt-5.6-sol` | **분리**(mini / sol) |
|---|---:|---:|---:|
| 판정 | 20/20 | ★**2건 실패** | 20/20 |
| 도구 호출 총계 | 36 | **96** | 30 |
| 케이스당 최대 | 4 | ★**14 (상한 12 초과)** | 3 |
| 소요 | 134초 | 310초 | — |
| 입력 토큰 | 251,034 | 508,151 | 219,952 |
| 출력 토큰 | 12,113 | 15,137 | 16,612 |
| uncited | 0 | 0 | 0 |
| unlinked | 4 | 2 | **8** |

**★확실한 것 — `sol` 을 Agent 에 쓰면 나쁩니다.** 예산 계약을 깨고(아래),
도구를 **2.7배** 부르고, **2.3배** 느립니다. 효과가 크고 방향이 일관됩니다.

**★불확실한 것 — `sol` 을 답변에 쓰는 효과는 측정되지 않았습니다.** 같은
`unlinked` 가 세 번의 n=1 실행에서 **4 → 2 → 8** 로 움직였습니다. 실행 간
흔들림이 모델 간 차이를 **압도**합니다. 우호적 신호는 「무관한 근거로 인과를
지어내는가」 프롬프트 하나뿐인데(현행 `gpt-4o-mini` 는 지어냈고 `sol` 은 그
근거를 인용 목록에서도 뺐습니다) 그것도 n=1 입니다.

★**이 절이 남기는 교훈은 수치가 아니라 그 두 번째 항목입니다.** 옛 `33~37`
(§10-8)·링 계측기(§4-9)·임베딩 드리프트(§4-5)에 이어 **네 번째로 같은 함정**이
나왔고, 이번에는 계측기가 그 자리에서 잡았습니다. 답변 모델의 효과를 말하려면
`--agent-eval-repeat=N` 으로 **양쪽 다 N회씩** 재야 합니다.

#### 10-9-1. ★드러난 결함 — 예산은 **턴 경계에서만** 검사된다

`ws-semantic-capital-trend` 가 도구를 **14회**(상한 12) 불렀습니다. `sol` 의
버그가 아니라 **잠복해 있던 결함이 드러난 것**입니다:

```text
should_continue   budget.is_exhausted(state)   ← 턴과 턴 **사이**에만 검사
run_tools         budget.spend(tool_calls_used=len(calls))   ← 한 턴을 통째로 씀
```

`gpt-4o-mini` 는 턴당 1~2개만 요청해 한 번도 안 걸렸습니다. `sol` 은 **한 턴에
여러 개를 병렬로** 요청하므로, 카운터가 11일 때 한 턴이 3개를 부르면 14가
됩니다. **「자르는 단위와 세는 단위가 다르다」** — `propagations_used` 에 대해
§4-4 가 적어 둔 것과 **같은 종류**입니다.

★**고치지 않았습니다.** 턴 안에서 자르려면 요청된 `tool_call` 일부를 버려야
하는데, LangChain `ToolNode` 는 **모든 `tool_call_id` 에 대응하는 `ToolMessage`**
를 요구하므로 순진한 절단은 다음 API 호출을 깨뜨립니다. 상한값 조정은 문서가
`[DECIDE]` 로 사람 몫이라 못 박은 항목이기도 합니다(§5-3 ③). 선택지는 셋입니다:

1. `run_tools` 가 남은 예산만큼만 실행하고 **나머지에는 「예산 소진」 `ToolMessage`
   를 채워 넣는다** — 계약을 지키면서 메시지 짝도 맞는다
2. 상한을 올린다 — ★모델이 바뀔 때마다 다시 올려야 하므로 근본 해결이 아니다
3. 그대로 둔다 — 단, **턴당 병렬 호출이 적은 모델에서만** 계약이 성립한다고
   문서에 명시해야 한다

★**1번으로 고쳤습니다**(2026-08-29). `run_tools` 가 `budget.remaining()` 만큼만
실행하고, 남는 호출에는 도구가 거부할 때와 **같은 모양**(`{"error": ...}`)의
`ToolMessage` 를 채웁니다. 짝을 맞추는 것과 자르는 것은 **함께** 해야 합니다 —
`tool_call_id` 에 짝이 없으면 OpenAI 가 그 대화의 다음 요청을 거부해서, 상한은
지켰는데 대화가 깨집니다.

**깨졌던 그 설정(Agent = `gpt-5.6-sol`)으로 다시 재서 확인했습니다:**

| | 수정 전 | 수정 후 |
|---|---:|---:|
| 판정 | **2건 실패** | **29 passed** |
| 케이스당 최대 도구 호출 | **14** (상한 12) | **12** (정확히 멈춤) |
| 예산이 막아 실행 안 한 호출 | — | 1 |
| 예산으로 잘린 케이스 | — | 1 / 20 |

★거부 수는 `tool_errors` 와 **갈라 셉니다**(`Observation.tool_calls_denied_by_budget`).
저건 **도구가** 거부한 것(범위 밖 key — 프롬프트 문제)이고 이건 **예산이** 막은
것(상한 문제)이라, 한 통에 담으면 어느 쪽을 고쳐야 하는지 알 수 없습니다.

---

### 10-10. ★머지 후 첫 재측정 — `_STRIP_UNLINKED_CLAIMS` 는 **켜면 안 됩니다** (2026-08-29)

세 브랜치를 합친 뒤(`9b6b6be` · `6d73800`) 사용자 승인을 받고 평가셋을 다시 만들었습니다.
**20/20 PASS · 29 passed · 3분 58초 · N=1.**

★**이전 수치와 직접 비교하지 마세요.** 답변 모델도 코드도 다릅니다:

| | 이번 (2026-08-29 · 머지 후) | 직전 (§10-8 N=5 베이스라인) |
|---|---|---|
| `ANSWER_MODEL` | ★`gpt-5.6-sol` | `gpt-4o-mini` |
| `AGENT_MODEL` | `gpt-4o-mini` (temp 0.0) | `gpt-4o-mini` |
| 코드 | Phase 15 + context_keys + 턴 내 예산 | Phase 13 · context_keys 없음 |
| 반복 | N=1 | N=5 |

#### 답 — 6건 전부 **오탐**입니다

`claim_check.unlinked()` 가 고른 6건을 한 문장씩 읽었습니다. **하나도 질문과 무관하지
않습니다.**

| 케이스 | 지워질 뻔한 문장 | 실제 |
|---|---|---|
| `query-news-citable` ×3 | 「의혹은 소송과 금융당국 조사로 이어졌습니다」·「증권집단소송이 1년 8개월째 지연」·「자본시장법 위반 2차 공판」 | 「파두 실적 논란 어떻게 됐어?」에 **정확히 답한 문장들** |
| `query-multi-company-suit` | 「D램 가격 담합 의혹으로 피소됐으나 최종 승소」 | 「둘 다 관련된 소송 있어?」가 **물은 바로 그것** |
| `ws-relationship-regulator` ×2 | 「로펌이 솔리다임 지분 조사 착수」·「그건 규제당국 조사가 아니라 민간 로펌 조사입니다」 | 뒤엣것은 모델의 **자기 정정** — 오히려 바람직한 문장 |

★**원인은 주장이 아니라 라벨입니다.** `intent_linked is False` 는 「근거가 **사건
종류(event_type)** 를 갖는데 질문의 의도와 안 맞는다」는 뜻입니다. 파두 질의의 의도가
매핑한 종류에 **소송·공판이 없어서** 걸린 것이지, 문장이 딴소리를 해서가 아닙니다.
재고 있는 것은 **주제 적합성이 아니라 라벨 일치**입니다.

★**그래서 플래그를 켜면 안 됩니다.** 오탐률이 이 표본에서 **6/6 = 100%** 입니다.
그래프 경로에 strip 배선이 아직 없는 것(`answer.py` 의 `strip_not_wired` 경고)이
결과적으로 다행이었습니다 — 배선부터 하고 켰다면 **맞는 답이 지워졌을** 것입니다.

★**고칠 자리는 `_STRIP_UNLINKED_CLAIMS` 가 아니라 `intent → event_type` 매핑**입니다
(`evidence_selector.matched_event_types`). 「논란」이 소송·공판을 안 부르는 것이 실제
결함입니다. `[DECIDE]`

#### 원인을 되짚었더니 **셋**이었습니다 — 그중 하나는 고쳤습니다 (2026-08-29)

6건을 실 Neo4j 로 되짚어 사건 종류를 확인했습니다. ★**키워드는 다 맞았습니다** —
「실적」도 「소송」도 「규제·조사」도 전부 `matched_event_types` 에 잡혔습니다. 위 진단의
「매핑이 못 부른다」는 절반만 맞았습니다.

| # | 원인 | 건수 | |
|---|---|---:|---|
| ① | ★**`기타` 는 구조적으로 절대 매치되지 않는다** — 패턴이 `re.compile(r"(?!)")` 라 `matched_event_types` 가 영원히 안 담는데, `_intent_linked` 가 그것을 「종류를 안다」로 읽고 대조해 **늘 `False`** 였다 | 2 | ★**해소** (2026-08-29) |
| ② | **인접 종류** — 「D램 담합 승소」는 `규제수사` 인데 질문은 `분쟁소송`. 한 사건에 종류가 **하나뿐**이라 생긴다 | 1 | `[DECIDE]` |
| ③ | **질문이 한 종류만 지목** — 「실적 논란」이 `실적` 만 부르는데 답은 소송·공판으로 이어진다 | 3 | `[DECIDE]` |

★**①은 결함이지 튜닝 문제가 아닙니다.** 「기타」는 「분류를 못 했다」는 뜻이라 **판정
불가(`None`)** 여야 하는데 「연결 없음(`False`)」으로 떨어졌습니다 — `_intent_linked` 의
docstring 이 「판정 불가를 연결 없음으로 떨어뜨리면 관계 질의가 통째로 차단된다」고
**바로 그것을 하지 말라고** 적어 둔 자리입니다. 「없는 것과 모르는 것은 다르다」는
`_own_evidence_ids` · `evidence[].missing` · `llm_calls_without_usage` 가 반복해 지킨
선입니다(현황서 §5-25).

★**②③ 은 안 고쳤습니다.** `_EVENT_TYPE_KEYWORDS` 나 종류 묶음을 손대야 하는데, 그 표는
`evidence_selector.select()` 가 **사건을 고를 때** 쓰므로 **재료가 바뀌고 답이 바뀝니다**.
①은 관측 계층(`claim_check`)만 만져 재료·답변·기준선이 **안 움직입니다** — 다음 실행의
§4-1 계수로만 나타납니다.

★**베이스라인이 그 사이 바뀌었습니다**(`6d672d1` · 보고 있는 기업 케이스 추가). 위
건수는 `35ac6a5` 실행(연결 없음 6건) 기준이고, 새 베이스라인은 **연결 없음 5건 · 판정
불가 18건**이며 그중 `기타` 는 **1건**(같은 `ev_a00a445e309a3c4f`)입니다 → 다음 실행에서
**연결 없음 4 · 판정 불가 19**. ★사라진 것은 결함이 아니라 **모델 출력이 바뀐 것**입니다
(자기 정정 문장이 이번 실행에 없었습니다) — `기타` 원인은 그대로입니다.

★**오탐률 판정은 그대로입니다** — 남은 것도 위 표에서 전부 오탐입니다. 켜면 안 됩니다.

#### 함께 확인된 것

| 항목 | 값 | 읽는 법 |
|---|---|---|
| **링 본 것 / kept / 인용** | 1008 / 110 / 7 | ★**네 실행 연속 미동**. 도구 호출은 35→36→35→36 으로 흔들리는데도 그렇다 |
| `cited_relation_without_ring` | **0** | 되짚기가 끊긴 인용 없음 — Phase 11·15 가 물고 있다 |
| `uncited` 주장 | **0 / 42 (0.0%)** | ★모든 주장이 근거를 달았다. `gpt-5.6-sol` 로 올린 뒤 처음 재는 값이다 |
| ★`claims_unlinked` 6 ↔ §4-1 표 **6행** | 일치 | ★**이중 계수 수정이 실제로 물었다는 증거다.** 개수(`record_claims`)와 본문(`record_claim_links`)이 다른 함수에서 오는데 값이 같다 — 고치기 전이었다면 개수만 12 로 찍혔다 |
| `tool_calls_denied_by_budget` | **0** | 턴 내 예산(`da2b53c`)은 이번엔 안 물었다 — 최대 4 회(상한 12) |
| `propagations_used` 최대 | 12 · 3케이스 | 소진 신호가 **아니다**(`_CAPS` 밖) — 관측치다 |
| LLM 토큰 | `gpt-4o-mini` 43호출 113k/7.9k · `gpt-5.6-sol` 18호출 142k/9.1k(추론 3,481) | 추론 토큰이 출력의 38%다 |

★**§3-1(앵커별 링 분포)은 안 찍혔습니다.** `if len(by_source) > 1` 가드는 통과할
조건인데도 출력이 없습니다 — 평가셋 20 케이스에 `context_keys` 를 쓰는 것이 아직
없어서입니다. 그 절이 경고하려던 「`workspace_keys=[]` → 전부 R3」 상황은 **재현되지
않았습니다.** 평가셋에 workspace-less 케이스를 넣는 것은 별개 작업입니다. `[DECIDE]`
