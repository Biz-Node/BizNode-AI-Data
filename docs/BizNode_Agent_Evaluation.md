...
| ② | **`propagations_used` 단위 불일치 발견** — 상한 12 에 실측 303 | §9-2 |
...
| | 시점 | 커밋 | 상태 |
|---|---|---|---|
| **Baseline (before)** | 2026-08-28 21:40 | `7e91679` | ★**링 안 의도 정렬이 꺼진 상태** |
| **After** | 2026-08-28 23:08 | `709496a` | 정렬 복구 (`41bb1bb`) 이후 |
...
| | before (`7e91679`) | after (`709496a`) |
|---|---|---|
| 케이스 | 20 | 20 |
| **PASS** | **20** | **20** |
| FAIL | 0 | 0 |
| Agent 가 불린 케이스 | 18 / 20 | 18 / 20 |
| 집합 수준 판정 | 전부 통과 | 전부 통과 (28 passed) |
...
| 카운터 | 상한 | before 최대 | **after 최대** | 상한에 닿은 케이스 | 읽는 법 |
|---|---:|---:|---:|---:|---|
| `tool_calls_used` | 12 | 5 | **3** | 0 | 한 번도 안 뭅니다. **12 는 넉넉합니다** |
| `events_used` | 40 | 20 | **20** | 0 | 한 번도 안 뭅니다 |
| `propagations_used` | 12 | 303 | ★**303** | **9** | ↓ **상한을 25배 넘겼습니다** |
| `hops_used` | 6 | 0 | **0** | 0 | 아무도 안 씁니다 (`explore_impact` 미구현) |
...
|---|---:|---:|
| 총 호출 | 37 | **34** |
| 케이스당 (최소·중앙·최대) | 1 · 2 · 5 | **1 · 2 · 3** |
| 거부(`tool_errors`) | 0 | **0** |

**도구별:**

| 도구 | before 호출 / 쓴 케이스 | after 호출 / 쓴 케이스 | 인용 |
|---|---:|---:|---|
| `get_relations` | 7 / 3 | 3 / 3 | 불가 |
| `get_events` | 11 / 9 | 11 / 9 | 불가 |
| `search_news` | 4 / 4 | 5 / 5 | ★**가능** |
| `search_dart` | 2 / 2 | 2 / 2 | 불가 |
| `get_business_overview` | 2 / 2 | 2 / 2 | 불가 |
| `get_market` | 5 / 4 | 5 / 4 | 불가 |
| `get_filings` | 6 / 5 | 6 / 5 | 불가 |
...
| | before | after |
|---|---:|---:|
| **Agent 루프가 예산으로 잘린** 케이스 | **0 / 20** | **0 / 20** |
| 최종 `budget_exhausted` 플래그 | 9 / 20 | 9 / 20 |
...
| 링 | 본 관계 (before → after) | 상한에 남은 것 (before → after) | 최종 인용 (before → after) |
|---|---:|---:|---:|
| **R0** 양끝 모두 워크스페이스 안 | 9 → 9 | 9 → 9 | 1 → 1 |
| **R1** 워크스페이스 ↔ 바깥 기업 | 754 → **746** | 109 → **101** | 4 → **5** |
| **R2** 워크스페이스 ↔ 비-Company | 203 → 203 | **0 → 0** | 0 → 0 |
| **R3** 워크스페이스와 안 닿음 | 63 → **50** | 8 → **0** | 0 → 0 |
| **합계** | 1029 → 1008 | **126 → 110** | 5 → 6 |

- 관계 kept **126 → 110** · cut **903 → 898**
- 최종 인용 39건 중 **33건이 관계가 아닙니다** (사건·뉴스 근거 — 링이 없습니다)
...
★**빗나감 15건은 「그 실행에서 직접 계산했다」는 뜻**이고, 그 값은 실행마다 흔들립니다. 기준선으로 쓰려면 `EMBED_CACHE_STRICT=1` 로 다시 재야 합니다.
...
| **조치** | ★**Phase 8 에서 일부러 안 고쳤습니다** — 현재 동작을 고정한 채 재는 단계라, 고치면 무엇을 쟀는지가 흐려집니다 |
| **상태** | **미수정 · 알려진 문제** |
...
| **현상** | R2 는 203건을 봤는데 **kept 0** · R3 는 50건 봤는데 **kept 0** (after) |
...
### 9-4. 발견 ③ — 인용의 85%가 관계가 아님

| 항목 | 내용 |
|---|---|
| **분류** | **관측 사실** (결함 아님) |
| **현상** | 최종 인용 39건 중 **33건이 사건·뉴스 근거** — 링이 없습니다 |
...
| **Instrumentation 문제** | 1 | ① `propagations_used` 단위 불일치 |
| **외부 모델 의존 문제** | 1 | ④ 도구 선택 변동 |

★**Agent 구현 자체의 결함은 0건입니다.** 발견된 것은 계측·랭킹·평가셋·모델 의존이며, 이것은 **Agent 를 감싼 결정론 방어선이 설계대로 작동했다**는 뜻이기도 합니다.
...
**★R2 는 전후 모두 0 입니다.** 이유는 구조적입니다 — **정렬은 링 *안에서만* 순서를 바꾸고, 자르기는 링 *순서대로* 먹습니다.** R1 이 혼자 746건이라 상한을 다 채우고 끝나므로 **R2 는 애초에 차례가 오지 않습니다.**

**결론 셋:**

1. ★**§5-17(비-Company 관계가 Ring 2 로 밀려 잘린다)은 이 수정으로 닫히지 않았습니다.** 링 **사이** 배분(fair-share) 문제로 남습니다
2. `41bb1bb` 의 **실제 효과는 kept 구성 변화**입니다 — 자르는 규칙은 그대로인데 **무엇이 살아남는지**가 바뀌었습니다 (개선이 아니라 1.5차 동작 복구)
3. 도구 호출 분포는 원리적으로 **안 바뀝니다** — 도구 스키마·시스템 프롬프트가 동일하고, 실측 총 호출 34 는 노이즈 범위(33~37) 안입니다

> ★**이것이 이 평가셋이 만든 가장 값어치 있는 결과입니다.** 「고쳤으니 좋아졌겠지」가 아니라 **「고쳤는데 안 바뀌었고, 그 이유가 다른 곳에 있다」**를 수치로 확인했습니다. 재평가가 없었다면 §5-17 을 닫힌 것으로 오해했을 것입니다.
...
```text
Agent 평가셋   28 passed  (케이스 20 + 집합 판정 8)
전체 스위트    953 passed · 기존 실패 7건 그대로 · 신규 0
```
...
| # | 무엇 | 왜 |
|---|---|---|
| ① | `EMBED_CACHE_STRICT=1` 로 재측정 | 캐시 빗나감 15건이 실행마다 값을 흔듭니다 |
| ② | `propagations_used` 단위 수정 후 재측정 | 「막는다」가 실제로 막는지 확인 |
| ③ | 링 사이 배분 규칙 결정 후 재측정 | R2·R3 가 살아나는지 |
| ④ | 답변 **내용 품질** 채점 설계 | ★현재 평가셋이 안 재는 축 |
| ⑤ | 동일 실행 **N회 반복**으로 변동폭 확정 | 33~37 이 관측 4회의 범위일 뿐입니다 |
