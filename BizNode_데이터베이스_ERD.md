...
### ★ 개인 주주 필터 — 지분율 단일 축의 함정 (2026-07-27 개선)

방법서 §10[2]의 "개인은 5% 이상만"을 **지분율만으로 적용하면 총수를 놓친다.**
한국 재벌은 낮은 지분으로 순환출자를 통해 지배하기 때문(이재용 삼성전자 **1.65%**).

→ **2축 필터**: `지분 ≥5%` **OR** `relate가 최대주주 계열`(DART 최대주주 현황의 관계 필드)

| relate | 판정 | 예 |
|---|---|---|
| `최대주주 본인`·`본인`·`최대주주의 특수관계인`·`최대주주의 자` | **유지**(지분 무관) | 이재용 1.65%, 홍라희 1.49% |
| 일반 `특수관계인`·`미등기임원`·`계열회사 임원`·`姉` | 5% 미만이면 제외 | 친인척 보유자 |

**임계값 민감도 실측:** 1%로 낮춰도 +36명뿐(154 vs 111)이고 대부분 단순 보유 친인척.
→ **5% 유지가 적절**하며, 놓치면 안 될 인물은 임계값이 아니라 **relate(신분)** 가 잡는다.
지분율="얼마나 소유하나", relate="지배구조상 누구인가" — 두 축이 상호보완.

### Person 식별자 — 소속 제거 + 전역 생년월 보강
...
**전역 생년월 인덱스 ([person_index.py](pipeline/normalizer/person_index.py)):** 최대주주 API엔 생년월이 없어 주주가 `name@corp`로 폴백되고 회사마다 분열된다. → 시드 **전체 임원 데이터**에서 이름→생년월 인덱스(1,168명)를 만들어 주주에도 주입. 동명에 생년월이 2개 이상이면 모호하므로 **제외**(동명이인 오병합 방지).
효과: 그룹 오너 8명이 계열사 간 병합(정의선=현대오토에버+현대모비스, 최창호=하나마이크론+하나머티리얼즈 등).

**한계 — 총수 분열은 P2 ER로:** 어느 계열사에서도 등기임원이 아닌 총수는 생년월 소스가 없어 여전히 분열된다(`이재용@삼성전자` / `이재용@삼성SDS`). P1(DART 단독)엔 동일인 판정 **근거가 없다**. P2에서 뉴스 문맥·기업집단 정보가 들어오면 근거를 갖고 `apoc.refactor.mergeNodes`로 병합(§2-6). **지금 근거 없이 합치지 않는 것이 안전**하다.

### P1 개체해소(ER) 규칙 ([person_er.py](pipeline/importer/person_er.py))

**"확실한 것만 병합"** — 근거가 명확한 케이스만 P1에서 처리하고 나머지는 P2로 남긴다.
`apoc.refactor.mergeNodes`로 **엣지를 보존**하며 병합(§2-6 원칙 2·3).

| 규칙 | 판정 | 실제 사례 |
|---|---|---|
| **R2. 생년월 상이** | ⛔ **절대 병합 금지**(동명이인) | 이승훈(1962-12/1965-05), 김근태, 김병수 |
| **R3-a. 같은 회사 공유** | ✅ 병합 | 정의선·이상현·이은지·최창호 |
| **R3-b. 양쪽 '최대주주 본인'** | ✅ 병합 | (동명이인이 둘 다 최대주주일 확률 희박) |
| 그 외 크로스-회사 | ⬜ **P2 이관** | 이재용·홍라희(삼성), 이준호(한쪽만 최대주주) |

### ⚠️ person_key는 자연키 — P2에서 대리키로 전환 (결정 2026-07-27)

현행 `person_key = "이름|생년월"`(없으면 `"이름@corp_code"`)은 **자연키**라 구조적 한계가 있다.

| 문제 | 내용 |
|---|---|
| **불완전** | DART 최대주주 현황엔 **생년월 필드 자체가 없다**(전 API 실측 확인). 이재용처럼 등기임원이 아닌 총수는 어디서도 생년월을 못 얻어 `@corp` 폴백 → 회사마다 분열 |
| **충돌 가능** | 동명+동생년월이 이론상 가능(생'일'은 DART에 없음). 현재 1,255조합 중 충돌 0건이나 규모 확대 시 위험 |
| **불안정** | 나중에 생년월을 알게 되면 **키가 바뀐다** → 노드 재생성 필요 |

**→ 근본 해결은 대리키(surrogate key):** `person_id = "p_<hash>"` + name·birth_ym·aliases는 속성.
Company가 `corp_code`(정부 발급 대리키)를 쓰는 것과 같은 원리다. 속성이 나중에 채워져도 ID는 불변.

**규모는 문제가 아니다** — 짧은 ASCII 대리키가 가변길이 한글 자연키보다 저장·인덱스 모두 유리하고,
노드 수백만은 Neo4j에서 일상적이다. 자연키가 오히려 규모에 취약(충돌 확률↑).

**전환 시점 = P2:** 지금 바꾸면 동작하는 코드를 고치는 순수 리팩토링 비용만 든다.
P2에서 뉴스 개체가 대량 유입되며 **어차피 ER 인프라(블로킹·유사도·Union-Find)를 구축**하므로,
그 작업의 일부로 흡수되어 추가 비용이 거의 없다.

**ER 신호 우선순위 — 뉴스가 주 근거가 아니다:**
```
1순위 생년월 일치/불일치     ← 결정적 (동명이인 배제)
2순위 같은 회사 / 최대주주 신분 ← P1에서 처리 가능
3순위 같은 기업집단 + relate   ← P2 (홍라희·중소기업 오너)
4순위 뉴스 문맥               ← 보조 확증
```
중소기업 오너는 지분이 높고(26~38%) relate가 명확해 **대기업 총수보다 판정이 쉽다**.
