...
def events_of(key: str) -> list[Event]:
    ...
    - `role`: `subject` 당사자 · `mentioned` **이름만 나온 것** · `related`.
      `mentioned` 를 당사자로 세면 집계가 부풀려진다.
    """
...
# ══════════════════════════════════════════════════════════════════
#
# ★이건 **프론트엔드가 아니다.** 디자인은 팀원 소관이고, 여기서 확인하는 건
#   데이터다. JSON 만 봐서는 안 보이는 것들이 있다 —
#
...
#       null 이 「—」로 얌전히 비나
#
# ★같은 출처에서 서빙하므로 **CORS 설정이 필요 없다.** 아티팩트로 만들면
#   외부 호스트 접근이 막혀 localhost 를 못 부르고 데이터를 박아 넣어야 한다 —
#   그러면 API 를 고칠 때마다 다시 만들어야 한다. 이건 새로고침이면 된다.
