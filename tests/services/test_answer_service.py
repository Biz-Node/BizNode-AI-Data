...
def test_system_prompt_tells_the_model_to_respect_evidence_about():
    ...
    ★문구가 두 번 바뀌었다. 옛 프롬프트는 규칙 번호로 말했고(「19번 규칙은
    14번 규칙을 이깁니다」 · `1b7fbaa`), 그다음은 [워크스페이스] 절 끝의
    단서였다. 지금은 [대상 지정 없음] 절이고 문구도 짧아졌다 — 「담아 둔
    기업이라는 이유만으로 그 evidence 를 사용하지 않습니다」.

    ★**이기는 쪽은 세 번 다 같다** — 근거의 귀속(`about`)이 워크스페이스 소속
    보다 우선한다. 반대로 가면 근거가 실제로 말하는 기업이 아니라 담아 둔
    기업으로 귀속되는 오답이 난다."""
    assert "about" in as_module._SYSTEM_PROMPT
    assert "담아 둔 기업이라는 이유만으로" in as_module._SYSTEM_PROMPT
